...
**Module ABI**:
The contracts between modules — **implementation-defined by design**, not a protocol property. In the reference implementation these are **compile-time contracts**: a `concept` shape plus `constexpr` traits per seam, dispatched through the target's §module set ([ADR-0047](docs/adr/0047-build-time-closed-module-sets-compile-time-seams.md), superseding the earlier runtime-vtable spelling of ADR-0016 §3). Two conforming nodes interoperate over the **wire**, never via a shared ABI ([ADR-0013](docs/adr/0013-v1-scope-boundaries.md)).
_Avoid_: "the protocol defines the module ABI", "modules are binary-portable across implementations", "the seam is a C vtable" (superseded — compile-time concept + module set).

**Module set (build-time-closed)**:
The per-target, compile-time-closed **set of module types** at a pluggable seam — e.g. the backend set (L0) and the transport set (net plane) — declared in the target's build configuration and dispatched by tag (single-member sets fold to direct calls; multi-member sets keep heterogeneous coexistence, e.g. a mixed heap+GPU rope). Adding a platform module means appending its type to the target's set, never editing core. **Types** close at build time; **instances** stay runtime (a connection vertex is still created by an in-band `:children[]` write). ([ADR-0047](docs/adr/0047-build-time-closed-module-sets-compile-time-seams.md))
