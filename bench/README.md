...
- **The multi-link rope arm.** A rope source's sub-span count is the sender's choice and is
  ...
  injected receive source (`core/src/fwd_router.cpp:2431`). Nothrow (ADR-0065 — exhaustion
...
### `bench_source_role` — the retention-role write asymmetry (#1505)

```
cmake --build bench/build --target bench_source_role bench_source_role_alloc -j
./bench/build/bench_source_role        # timing:  RESULT / RESULT_TAIL rows
...
per-edge clone of its own in `dispatch_edge_target` that swamps the term in question. Nothing
swept the role of the vertex being WRITTEN. This bench does, and subscribes by CALLBACK so the
...
touches no allocator and cannot fail, while one past it grows a `std::vector<view_t>` on every
...
never registered — a quarter of the deliveries lost, with every `write()` still returning
success — and requires the counted figure to come out strictly below `publishes x fan-out`
and land exactly on what arrived. The healthy twin must then agree with the arithmetic, so
a regression to the derived figure fails the first case instead of passing both. A test
that only built a working topology would guard nothing: on an idle host the two figures are
identical at every width the sweep runs.
...
> (`bench_libtracer.cpp:16`, `:1470`); `bridge_t`, `router_wrap`, `router_unwrap`, `kMaxHops`,
