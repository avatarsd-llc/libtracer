...
void run_sub(std::uint16_t port) {
    auto session = Session::open(make_config(true, port));
    net::SubState state("zenoh");
...
void run_pub(std::uint16_t port) {
    auto session = Session::open(make_config(false, port));
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::fprintf(stderr, "usage: %s <pub|sub> <port>\n", argv[0]);
        return 2;
    }
