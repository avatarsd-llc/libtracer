 ...
 *
 * A global heap-allocation counter — the "measured, not asserted" instrument the
 * 16KB-RAM zero-heap gate requires (ADR-0038 §16KB-RAM feasibility gate: "zero heap
 * allocations and zero heap growth, measured (an allocator counter / __wrap_malloc)").
 *
...
// The live counters (defined in bench_forward_heap.cpp alongside the operator news).
...
// Zero the counters (call before arming a fresh window).
inline void reset() {
...
// Begin / end counting. seq_cst so the arm strictly precedes the measured region.
...
// Read the counters (typically after disarm()).
...
// RAII window: reset + arm on construction, disarm on scope exit; read via .result().
class window_t {
   public:
    window_t() {
        reset();
