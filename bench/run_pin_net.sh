...
# publisher's send rate. Arms rotate per round, and arm A (untouched origin/main) alternates
# position against the implementing binary, for the same reason as run_pin_ratio.sh.
#
...
#
#   run_pin_net.sh <impl-bench-dir> <control-bench-dir> [rounds] [payload] [slot] [slots]
#
...
for ((r = 0; r < ROUNDS; ++r)); do
    for V in $VERTEX_SET; do
        n=${#ARMS[@]}
        for ((j = 0; j < n; ++j)); do
            IFS=: read -r label bin k <<<"${ARMS[$(((r + j) % n))]}"
