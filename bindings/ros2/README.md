...
| subscription | `subscribe(path)` — an edge appended to the *producer* vertex; delivery is a push, and the retained history is that **vertex's** STREAM ring (owner-sized and with no wire surface at all — `core/include/libtracer/graph.hpp:1272-1280`), never a per-subscriber queue |
...
`SCHEMA_NOT_FOUND`, caller-independently (`core/src/graph.cpp:3108-3114`). The `:settings`
**read** container survives with its shape and none
of its knobs — `SETTINGS{ [NAME "app" SETTINGS{…}] }`, `core/src/graph.cpp:3465` — so
...
| --- | --- | --- |
| `reliability`, `durability` (libtracer packs a third bit-field, `priority`, that ROS has no profile member for) | the **subscription's** packed 16-bit `delivery_policy`, set at `rmw_create_subscription` time and carried in the `SUBSCRIBER`'s existing `SETTINGS` child as `NAME "delivery_policy" VALUE u16` | `core/include/libtracer/subscriber.hpp:104`, decoded at `core/src/graph.cpp:2450`, RFC-0022 §3.A |
| `history` + `depth` | **owner-side** ring depth, declared independently at each end — the subscription's own target vertex is a STREAM whose depth `rmw_tracer` sets locally (that ring is what `rmw_take` pops); what a subscriber cannot set is the *producer's* depth, which has no wire surface | `core/include/libtracer/graph.hpp:1280`, `core/include/libtracer/vertex.hpp:207` |
...
- **A libtracer-only delivery mode is `rmw_tracer`-local, not a QoS extension.**
  ...
  (`core/src/graph.cpp:2353`) with no wire spelling. `rmw_tracer` may set it on vertices it
