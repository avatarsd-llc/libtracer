...
| subscription | `subscribe(path)` — an edge appended to the *producer* vertex; delivery is a push, and the retained history is that **vertex's** STREAM ring (owner-sized and with no wire surface at all — `core/include/libtracer/graph.hpp:779-787`), never a per-subscriber queue |
...
of its knobs — `SETTINGS{ [NAME "app" SETTINGS{…}] }`, `core/src/graph.cpp:2060` — so
...
| `history` + `depth` | **owner-side** ring depth — the topic's owner calls `graph_t::set_history_depth`; a remote subscriber cannot set it | `core/include/libtracer/graph.hpp:787` |
...
- **A libtracer-only delivery mode is `rmw_tracer`-local, not a QoS extension.**
  ...
  (`core/src/graph.cpp:1386`) with no wire spelling. `rmw_tracer` may set it on vertices it
