...
`SCHEMA_NOT_FOUND`, caller-independently (`core/src/graph.cpp:2392-2398`). The `:settings`
**read** container survives with its shape and none
of its knobs — `SETTINGS{ [NAME "app" SETTINGS{…}] }`, `core/src/graph.cpp:2747` — so
...
| --- | --- | --- |
| `reliability`, `durability` (libtracer packs a third bit-field, `priority`, that ROS has no profile member for) | the **subscription's** packed 16-bit `delivery_policy`, set at `rmw_create_subscription` time and carried in the `SUBSCRIBER`'s existing `SETTINGS` child as `NAME "delivery_policy" VALUE u16` | `core/include/libtracer/vertex.hpp:271`, decoded at `core/src/graph.cpp:1870`, RFC-0022 §3.A |
...
- **A libtracer-only delivery mode is `rmw_tracer`-local, not a QoS extension.**
  `delivery_mode_t` (`vertex.hpp:668` — `IF_NEWER` / `UNCONDITIONAL` / `EXPLICIT`; there is
