...
/*!
 * @brief Protocol ERROR registry + STATUS (Unit 3).
 *
...
/**
 * @brief A registered protocol error code (RFC-0002 §D) — the u16 wire identity of a
 * built-in `tr::…` error path, carried LE in the ERROR TLV's first-child VALUE.
 */
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u16)]
pub enum ErrCode {
    /** @brief `tr::frame::truncated` */
    FrameTruncated = 0x0001,
    /** @brief `tr::frame::invalid` */
    FrameInvalid = 0x0002,
    /** @brief `tr::frame::crc_fail` */
    FrameCrcFail = 0x0003,
    /** @brief `tr::tlv::nesting_too_deep` */
    TlvNestingTooDeep = 0x0010,
    /** @brief `tr::path::not_found` */
    PathNotFound = 0x0020,
    /** @brief `tr::path::invalid` */
    PathInvalid = 0x0021,
    /** @brief `tr::path::in_use` */
    PathInUse = 0x0022,
    /** @brief `tr::schema::type_mismatch` */
    SchemaTypeMismatch = 0x0030,
    /** @brief `tr::schema::not_found` */
    SchemaNotFound = 0x0031,
    /** @brief `tr::flow::backpressure` */
    FlowBackpressure = 0x0040,
    /** @brief `tr::flow::timeout` */
    FlowTimeout = 0x0041,
    /** @brief `tr::flow::address_shift_gap` */
    FlowAddressShiftGap = 0x0042,
    /** @brief `tr::access::denied` */
    AccessDenied = 0x0050,
    /** @brief `tr::transport::down` */
    TransportDown = 0x0060,
    /** @brief `tr::version::mismatch` */
    VersionMismatch = 0x0070,
}

/**
 * @brief Registry-side severity of a protocol error (RFC-0002 §D) — advisory metadata
 * that never travels on the wire.
 */
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Severity {
    /** @brief Expected-in-operation outcome (e.g. a missing path). */
    Warn,
    /** @brief A genuine protocol-level failure. */
    Error,
    /** @brief The peer relationship itself is unsound. */
    Critical,
}

/**
 * @brief Registry-side disposition of a protocol error (RFC-0002 §D) — what the caller
 * should do next; never travels on the wire.
 */
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Disposition {
    /** @brief Retry may succeed (congestion, timing, lossy link). */
    Transient,
    /** @brief Don't retry this request as-is. */
    Permanent,
    /** @brief Tear down the peer relationship. */
    Fatal,
}

/**
 * @brief The 15-entry registry table: `(code enum, u16 code, tr:: path, severity,
...
impl ErrCode {
    /** @brief The u16 wire code of this error (the ERROR TLV's VALUE payload, LE). */
    pub const fn code(self) -> u16 {
        self as u16
    }

    /** @brief The registered code for a raw u16, or `None` if unregistered. */
    pub fn from_code(code: u16) -> Option<ErrCode> {
        REGISTRY.iter().find(|e| e.1 == code).map(|e| e.0)
    }

    /** @brief The registered code for a `tr::…` path, or `None` if unregistered. */
    pub fn from_path(path: &str) -> Option<ErrCode> {
        REGISTRY.iter().find(|e| e.2 == path).map(|e| e.0)
    }

    /** @brief The canonical `tr::<concept>::<error>` path (the string-form identity). */
    pub fn path(self) -> &'static str {
        REGISTRY[self.index()].2
    }

    /** @brief The registry severity of this error. */
    pub fn severity(self) -> Severity {
        REGISTRY[self.index()].3
    }

    /** @brief The registry disposition of this error. */
...
/** @brief The two identity forms of an ERROR TLV's first child (RFC-0002 §C). */
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ErrorId {
    /**
     * @brief A registered code (`VALUE` first child, u16 LE). Carries the raw u16 so an
     * unrecognized-but-numeric code round-trips; use [`ErrCode::from_code`] to
     * resolve it to a known entry.
     */
    Code(u16),
    /**
     * @brief A string-form identity (`NAME` first child, UTF-8 `tr::…` path) — used by
     * third-party extensions outside the frozen registry.
     */
    Path(String),
}

/** @brief A decoded ERROR TLV: its identity plus any optional human `DESCRIPTION` detail. */
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ParsedError {
    /** @brief The error identity (registered code or string path). */
    pub id: ErrorId,
    /** @brief The optional human-readable `DESCRIPTION` (`0x03`) detail, if present. */
    pub description: Option<String>,
}

impl ParsedError {
    /**
     * @brief The resolved registry code, when the identity is a registered numeric code
...
/**
 * @brief Build an ERROR TLV (`type=0x08`, `PL=1`) in registered-code form:
...
/**
 * @brief Build an ERROR TLV in raw registered-code form from an arbitrary u16 (for a
...
/**
 * @brief Build an ERROR TLV (`type=0x08`, `PL=1`) in string form:
...
/**
 * @brief Parse an ERROR TLV into its identity and optional description.
 *
 * # Errors
...
/**
 * @brief Build the empty STATUS=OK TLV (`09 00 00 00`) — `opt.PL=0`, zero length. The
...
/**
 * @brief Build a non-empty STATUS TLV (`type=0x09`, `PL=1`) carrying one or more ERROR
...
/**
 * @brief The ERROR children of a STATUS TLV (empty for STATUS=OK).
 *
 * # Errors
