...
/*!
 * @brief `:field` selector — FIELD TLV (`0x10`) build/parse (Unit 5).
 *
...
/** @brief Max levels in a `:field` selector (`core` `kMaxFieldDepth`). */
pub const MAX_FIELD_DEPTH: usize = 8;

/**
 * @brief The index mode of one `:field` level, matching the FIELD wire `index_mode`
 * VALUE (RFC-0004 §C).
 */
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FieldMode {
    /** @brief No brackets — a plain scalar field (`index_mode` absent / 0). */
    Scalar,
    /**
     * @brief `[N]` element or `[]` append (`index_mode=1`); the index is carried
     * separately when present.
     */
    Element,
    /** @brief `[*]` wildcard (`index_mode=2`) — subscriber-path targets only. */
    Wildcard,
}

/** @brief One level of a `:field` selector chain. */
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FieldLevel {
    /** @brief The level's field name (e.g. `"subscribers"`, `"settings"`). */
    pub name: String,
    /**
     * @brief The `[N]` index when this is an element selector; `None` for `[]` append,
     * scalar, or wildcard.
     */
    pub index: Option<u32>,
    /** @brief The level's mode. */
    pub mode: FieldMode,
}

impl FieldLevel {
    /** @brief A scalar level (`name`, no brackets). */
    ...
    /** @brief An indexed element level (`name[index]`). */
    ...
    /** @brief An append element level (`name[]`). */
...
/**
 * @brief Parse a `:field` selector string into its levels. Accepts an optional leading
 * `:`; levels split on `.`; each level may carry a trailing `[N]`, `[]`
 * (append), or `[*]` (wildcard).
 *
 * `":subscribers[]"` → one append level; `":subscribers[3]"` → one indexed
 * level; `":settings.history_keep_last"` → two scalar levels.
 *
 * # Errors
...
/**
 * @brief Build a FIELD TLV node (`type=0x10`, `PL=1`) from its levels. Each level emits
...
/**
 * @brief Build a FIELD TLV from a `:field` selector string. Vector-pinned:
 * `field-append`, `field-indexed`, `field-nested`.
 *
 * # Errors
...
/**
 * @brief Parse a FIELD TLV back into its levels (the inverse of [`field_tlv`]).
 *
 * # Errors
