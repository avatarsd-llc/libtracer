...
//! Typed TLV builders and child-lookup accessors (Unit 1).
//!
...
/// Maximum bytes in a single path/name segment (reference/03 §path syntax,
/// `core` `kMaxSegmentBytes`).
pub const MAX_SEGMENT_BYTES: usize = 64;
/// Maximum number of segments in a path (`core` `kMaxSegments`).
pub const MAX_SEGMENTS: usize = 32;
/// Maximum total encoded NAME-child bytes in a PATH payload (`core`
...
/// A typed-tier build/parse failure. Kept SEPARATE from the codec-fault
...
pub enum BuildError {
    /// A segment was empty or exceeded [`MAX_SEGMENT_BYTES`] (`tr::path::invalid`).
    SegmentLength,
    /// A segment held a reserved character `/ : . [ ] * ?` (`tr::path::invalid`).
    ReservedChar,
    /// A path had zero segments where at least one is required (`tr::path::invalid`).
    EmptyPath,
    /// A path exceeded [`MAX_SEGMENTS`] segments (`tr::path::invalid`).
    TooManySegments,
    /// A path's encoded NAME payload exceeded [`MAX_PATH_BYTES`] (`tr::path::invalid`).
    PathTooLong,
    /// A path string was not rooted at `/` (`tr::path::invalid`).
    NotRooted,
    /// A `:field` selector was empty or malformed (`tr::path::invalid`).
    InvalidField,
    /// A structured TLV did not match its expected shape / a child had the wrong
    /// type code (`tr::schema::type_mismatch`).
    TypeMismatch,
    /// Bytes that should be UTF-8 (a NAME/DESCRIPTION payload) were not
    /// (`tr::path::invalid`).
    InvalidUtf8,
}
...
/// Validate a single path/name segment against the addressing rules: 1..64 UTF-8
...
impl Opt {
    /// A structured option byte: only `PL=1` (bit 6) set, all else clear — the
...
/// Build an [`Opt`] from explicit flags, mirroring the TypeScript `opt()` helper.
...
/// Option selectors for [`value_opts`]. Mirror the wire opt/trailer fields the
...
pub struct ValueOptions {
    ...
    pub crc: bool,
    /// With [`ValueOptions::crc`], use CRC-16-CCITT (`opt.CW=1`) instead of CRC-32C.
    pub crc16: bool,
    /// Absolute u64 wire timestamp (ns since epoch) → `opt.TS=1, TF=0`.
    pub timestamp_ns: Option<u64>,
    /// Relative i32 wire timestamp (ns) → `opt.TS=1, TF=1`. Ignored if
    /// [`ValueOptions::timestamp_ns`] is set.
    pub timestamp_rel_ns: Option<i32>,
}

/// Build a plain opaque VALUE TLV (`type=0x01`, no trailer). Vector-pinned:
/// `value-bool-true`.
pub fn value(payload: &[u8]) -> Tlv {
    value_opts(payload, &ValueOptions::default())
}

/// Build a VALUE TLV (`type=0x01`) with length-width / CRC / wire-timestamp
...
pub fn value_opts(payload: &[u8], opts: &ValueOptions) -> Tlv {
    ...
    Tlv {
        ...
        opt: opt(false, has_ts, opts.crc, opts.long_length, opts.crc16, has_rel),
...
/// A 1-byte VALUE TLV node (e.g. a FWD `op`/`kind` discriminant, an ACE flag).
pub fn value_u8(v: u8) -> Tlv {
    value(&[v])
}

/// A little-endian u16 VALUE TLV node (e.g. an ACL `access_mask`, an ERROR code).
pub fn value_u16(v: u16) -> Tlv {
    value(&v.to_le_bytes())
}

/// A little-endian u32 VALUE TLV node (e.g. a FIELD `[N]` index).
pub fn value_u32(v: u32) -> Tlv {
    value(&v.to_le_bytes())
}

/// A little-endian u64 VALUE TLV node (e.g. an AWAIT timeout, an ACE `expires_ns`).
...
/// Build a PATH TLV (`type=0x06`, `PL=1`) from segments — one validated NAME
...
impl Tlv {
    /// The first child with the given `type_code`, or `None`.
    ...
    /// Iterate the children with the given `type_code`.
    pub fn children_of_type(&self, type_code: u8) -> impl Iterator<Item = &Tlv> {
        self.children.iter().filter(move |c| c.type_code == type_code)
    }

    /// The opaque payload interpreted as UTF-8 (for NAME / DESCRIPTION).
    ///
    /// # Errors
    ...
    /// The opaque payload interpreted as a little-endian unsigned integer (up to
