...
## Status: v0.7.0 — M1 (codec) + M2 (substrate) + M3 (L4 graph) + M4 (loopback) + M5 (UDP transport) + M6 (TCP stream transport) all landed, plus the RFC-0004 remote-operation plane, the WS/CAN transports, and the ADR-0043 Phase A QUIC module

> The C++ API is versioned independently of the wire format it implements: `core/` is at **0.7.0** (pre-1.0, still implementation-defined per [ADR-0013](../docs/adr/0013-v1-scope-boundaries.md)), while **protocol-v1 itself is still `DRAFT`** and becomes immutable on release, not before — see [`docs/spec/v1.md`](../docs/spec/v1.md). "M1–M6 landed" describes the rebuild below, not a frozen surface.
...
- **RFC-0004 remote-operation plane (landed).** `fwd_router_t` (+ `child_registry_t`, `op_resolver_t`, `route_handle_t`): path-addressed `read`/`write`/`await`/`subscribe` over `FWD`, with connections exposed as `/net/<conn>` vertices (`transport_vertex_t`, ADR-0027). A forward hop of a **contiguous (single-link)** frame allocates nothing of its own — offset-dispatch + stack header buffers + a stack `iov` array — and `bench_forward_heap` CI-gates *that arm* at 0 allocations (ADR-0038/0039). The gate is not a whole-wire claim: it drives a stub link, so it cannot see the shipping transports' `::iovec` table, which spills to the heap above 16 spans (`bench_transport_iov` measures the boundary at 17 caller spans / ~288 B, against a structural `kFwdMaxIov` of 9). A **multi-link rope** frame gathers into a `mem::block_array_t` drawn from the injected nothrow receive source (`src/fwd_router.cpp:1691`, ADR-0065) — it cannot abort, but it is not allocation-free either. The terminus reads the ADR-0041 arena: `wire::decode_into`/`tlv_arena_t` (flat pre-order span-nodes over the inbound frame), a span-aliased `path_key` (a canonical PATH body *is* the vertex-map key — zero materialization), trailer-sliced stores (stored WRITE values are header+body only), and a direct-emitted `FWD{REPLY}` head in one exactly-sized segment. A remote subscriber's `return_route` is copied once at subscribe into a refcounted segment and refcount-cloned per delivery. The arena draws from the `fwd_router_t` constructor's **third** parameter, a nothrow `tr::mem::block_source_t*` (defaulted to the platform heap) — split from the `std::pmr::memory_resource*` that still serves the label tables, because the arena is built from a peer's frame and must report exhaustion by value rather than by throwing (ADR-0065). The library holds no internal buffer. A host bounds the terminus by injecting a bounded **recycling** block source; note that the shipped `bump_source_t` is scope-lifetime only and is not one.
...
- **ADR-0043 Phase A — QUIC transport, a separate module (landed).** `quic_transport_t` — msquic behind the same seam, in its own library target **`libtracer_quic`**: a host that talks QUIC links the module and registers `quic_transport_factory()` via the `register_transport_type` extension seam; a host that doesn't never compiles these sources — the core has no msquic reference, no feature macro, no `quic` builtin (open/closed), and the 16 KB MCU story is untouched. One connection, ONE bidirectional stream carrying the SAME length-prefix framing as `tcp_transport_t` — plus what QUIC adds: TLS 1.3 (the first real link-security story), connection migration, no TCP head-of-line blocking. Dial (synchronous handshake; the system trust store by default, a CA bundle, or a DEV-ONLY no-verify flag for self-signed certs) and listen (PEM `cert`/`key` paths, one inbound peer; `tools/gen-dev-cert.sh` emits a dev pair); the factory consumes four kind-private config keys — `cert`/`key` on LISTEN and the DIAL-side trust pair `ca` (a PEM CA-bundle path) / `insecure` (u8, default 0) — parsing them MODULE-SIDE from the raw config TLV, so the shared `conn_settings_t` stays lean with only the universal keys (the ADR-0043 §5 leanness ruling). **A SPEC-created dialer authenticates its peer**: with neither trust key it verifies against the system trust store and refuses a certificate that does not chain to it, so reaching a self-signed peer takes an explicit `ca` or `insecure = 1`. ADR-0042 owning delivery + backpressure drain, as tcp. Validated raw and end-to-end over localhost QUIC (`tests/quic_test.cpp`, the dedicated `quic` CI workflow); TSan/ASan/UBSan clean. Per-flow streams, RFC 9221 datagram mode, and WebTransport (Phase B, #92) are staged follow-ons.
...
├── examples/             in_process_pubsub.cpp pubsub_fanout.cpp rope_scatter.cpp
│                         tree_of_ropes.cpp two_node_fwd.cpp wire_codec.cpp
│                         wire_roundtrip.cpp   (seven compile-tested programs)
