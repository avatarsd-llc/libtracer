...
- **M3b — subscriptions + dispatch (in progress).** Field-write (`:subscribers[]`, `:settings.*`), SUBSCRIBER target-path fan-out on `SegmentPtr` clone + a `subscribe(src, callback)` helper, a dispatch-depth cycle cap, a minimal `:schema`, and an in-process pub/sub example — completing the P0 node.
