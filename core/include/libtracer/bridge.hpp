...
class bridge_t {
   public:
    bridge_t(graph::graph_t& graph, transport_t& transport, peer_id_t peer);
    ...
    [[nodiscard]] graph::result_t<void> export_vertex(const graph::path_t& src);
    ...
    void set_mount(const graph::path_t& mount);
