...
struct default_config_t {
    ...
    /**
     * @brief How many reader threads the ADR-0072 reclamation domain reserves announcement
     *        storage for **statically** — `0` (the default) reserves none at all.
     *
     * A distinct knob from @ref kHazardReaderSlots, which sizes the LKV slot's own registry
     * and is emitted only when @ref lkv_slot_t is `hazard_slot_t`. The reclamation domain,
     * by contrast, is built into every `graph_t`, so a table sized off that knob would be
     * unconditional `.bss` on a target that never runs a second reader thread — 4 KB of a
     * ~16 KB budget on the ESP32-C6 half of the dual target, which is a hard-constraint
     * violation rather than a cost to weigh.
     *
     * So the table is **not** a table by default. A thread claims its participant (a
     * cache-line-isolated group of announcement words) on its first protected read and
     * releases it at thread exit; unclaimed participants do not exist, and a participant a
     * departed thread released is reused by the next one. What N buys is *determinism*, not
     * capacity: the first N participants are reserved in `.bss` at link time, so a target
     * that must not touch the allocator on a reader path sets N to its reader-thread count
     * and never does. Past N — and at the default 0, from the first thread — a participant
     * costs one `sizeof(participant_t)` allocation per thread, once, on a cold path.
     *
     * Undersizing is not a correctness problem and not a capacity problem: participants are
     * unbounded either way (the no-synthetic-limits rule — the bound is the allocator's, and
     * nesting depth is unbounded too, through a per-thread grow-only chain).
     *
     * CMake: `-DLIBTRACER_RECLAIM_ANNOUNCE_SLOTS=8`. Costs
     * `N * sizeof(tr::mem::detail_hz::participant_t)` bytes of `.bss` — which
     * @ref kCacheLineBytes dominates, exactly as it does the lock stripes.
     */
    static constexpr std::size_t kReclaimAnnounceSlots = 0;
...
/** @brief @ref default_config_t::kReclaimAnnounceSlots for this build. */
inline constexpr std::size_t kReclaimAnnounceSlots = config_t::kReclaimAnnounceSlots;
