 ...
 *
 * Sizing: one index per such thread. The domain costs `(N + 1) * 128` bytes of `.bss`, and
 * **nothing at all** unless `lkv_slot_t` is bound to `hazard_slot_t` — the default binding
 * never references the registry, so it is never emitted. Undersizing is not a correctness
 * problem: threads past the bound share one reserved index under a spin lock, so they
 * serialize with each other and with nobody else.
 */
inline constexpr std::size_t kHazardReaderSlots = 64;
