 ...
 *
 * Sizing: one index per such thread, and **nothing at all** unless `lkv_slot_t` is bound to
 * `hazard_slot_t` — the default binding never references the registry, so it is never
