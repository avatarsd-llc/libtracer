...
class key_view_t {
   public:
     ...
     * @return 0 when the bytes at @p at are ragged: no room for a header, or a length that
     *         runs past the key's end. A well-framed record ends at @p at + 4 or later, so 0
     *         is unambiguous. Every walk here stops there — a malformed tail is never
     *         half-decoded.
     *
    ...
    [[nodiscard]] std::size_t record_end(std::size_t at) const noexcept {
        ...
        return at + 4 + len > key_.size() ? 0 : at + 4 + len;
    }
