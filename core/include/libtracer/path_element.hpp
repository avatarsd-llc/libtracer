...
struct path_element_t {
    ...
    /** @brief The decoded label, meaningful only when @ref kind is @ref LABEL. */
    path_label_t label{};
 ...
 * @ref path_element_kind_t::FOREIGN, and only a `kind = 0x16` record whose payload is exactly
 * one *mintable* label reads as @ref path_element_kind_t::LABEL.
 *
 * @note The reserved zero generation reads @ref path_element_kind_t::MALFORMED, not a label
 *       that will fail a lookup. `0` is the value a host never mints (`path_label.hpp`), so a
 *       record carrying it is an address no table could ever answer for — refusing it in the
 *       grammar keeps a lookup off a path that can only fail, and matches `path_label_at`,
 *       which has answered `nullopt` for it since car 2.
 */
 ...
 *         §11.2's rule in the codec: layering a label over an element that is already a
 *         compressed spelling spends two mechanisms on bytes one of them already saved.
 */
