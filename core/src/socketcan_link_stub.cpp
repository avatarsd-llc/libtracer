/**
 * @file
 * @brief socketcan_link_t stub — the non-Linux implementation of the SocketCAN link: every
 *        operation is a no-op and ok() stays false, so a program referencing socketcan_link_t still
 *        links everywhere while a platform port (e.g. the ESP-IDF component's twai_link_t) supplies
 *        the real can_link_t.
 *
