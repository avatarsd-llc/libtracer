 ...
 *
 * Copied from `mesh_node.cpp` / `transport_vertex_test.cpp` rather than shared —
 * `conn_spec` is deliberately a test-local helper, not a shipped header.
 */
...
std::vector<std::byte> conn_spec_bytes(std::string_view type, std::string_view name,
    ...
    const view_t v = conn_spec(type, name, role, port, kind, addr);
...
int main() {
    ...
    (void)net_a.register_module(std::string(tr::net::kWsServerSuggestedModule), "ws",
                                conn_role_t::LISTEN);
    {
        const auto w = graph_a.write(path_t("/net:children[]"),
                                     conn_spec("listener", "l", conn_role_t::LISTEN, kPortA, "ws"));
    ...
    {
        const auto w =
            graph_b.write(path_t("/net:children[]"),
                          conn_spec("listener", "ctrl", conn_role_t::LISTEN, kPortBCtrl, "ws", {},
    ...
    std::uint32_t kAfter = 0x0491'C0DE;
    {
            ...
            &c_misroutes);
        {
            const auto w = graph_c.write(
                path_t("/net:children[]"),
                conn_spec("client", "b", conn_role_t::DIAL, b_port, "ws", "127.0.0.1"));
