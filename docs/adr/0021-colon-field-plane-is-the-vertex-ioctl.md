...
Status: accepted. **Refined by [ADR-0059](0059-creator-endpoint-creation-and-removal-are-writes-to-a-vertex.md)**, which retracts exactly two claims below and leaves the rest of this ADR standing as the operative record. Retracted: (1) §Decision's "*Creation is one such optional standard field*" — creation moves off the `:` plane onto a **creator endpoint vertex**; and (2) §Decision 3's claim that the standard-field model "*subsumes the 'device-bounded creation endpoint' alternative without leaving the protocol*" — this ADR considered that alternative and rejected it, and ADR-0059 §Considered options answers the rejection on its own terms. A creator endpoint is a **distinct identity**, so it does not violate this ADR's own rule (*control facet ⇒ `:`, distinct identity ⇒ `/`*) — which is why the refinement is consistent with, not a departure from, the ioctl framing. **Untouched and still operative:** the ioctl framing itself, the optionality rule (`SCHEMA_NOT_FOUND` = the `ENOTTY` of an unsupported field), and the standard-vs-device-private two-tier model — these are precisely what ADR-0059 leans on to reject the full-sysfs "every knob is a file" alternative.
