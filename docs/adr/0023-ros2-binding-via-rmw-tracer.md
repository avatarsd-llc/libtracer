...
**QoS maps cleanly because both are DDS-derived:** `RELIABLE/BEST_EFFORT` → the subscription's `reliability` (bits 0–1); `TRANSIENT_LOCAL/VOLATILE` → its `durability_request` (bit 5); `KEEP_LAST(depth)` → the producer's STREAM ring depth, which is owner-side state (`graph_t::set_history_depth`, §3.C) with no wire surface — an rmw layer sets it locally on the vertex it owns, and cannot request it of a remote producer. A ROS QoS profile is a *reader/writer* profile, which is exactly the split [RFC-0022](../spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md) made.

> **Erratum (2026-08-01), [RFC-0022](../spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md):** this paragraph originally mapped every DDS knob onto a per-**vertex** `:settings.*` name — `:settings.reliability`, `:settings.durability`, `:settings.priority` — and mapped `deadline`/`liveliness` onto `:settings.deadline_ns`. Those four knobs no longer exist. The first three moved to the subscription's packed delivery policy (§3.A), which is *closer* to DDS, not further: DDS puts reliability, durability and priority on the reader/writer pair for the same reason RFC-0022 did. `deadline_ns` and `queue_max_bytes` were deleted as inert (§3.E), so `deadline`/`liveliness` and `lifespan` now have **no** libtracer mapping at all — an rmw layer must either enforce them itself or decline to offer them, which is the honest state and was already the *effective* one (nothing enforced the knob).
...
- QoS round-trips through `:settings`; any libtracer QoS not in the ROS profile (e.g. `delivery_mode=ON_CHANGE`) is exposed as a `rmw_tracer`-namespaced QoS extension, ignored by stock ROS tools.
  ...
  > **Erratum (2026-08-03), [RFC-0022](../spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md) §3.B:** this bullet names a carrier that no longer exists — the same correction the §QoS-mapping erratum above already made to the mapping table, missed here in the same sweep. `settings_t` was **deleted outright** (see [ADR-0058](0058-vertex-ext-storage-classes-borrowed-declarations-and-group-split.md)'s 2026-08-01 erratum); there is no `:settings` for QoS to round-trip through and no wire surface for a QoS *extension* to ride. What the mapping actually is: the reader/writer knobs (reliability, durability, priority) ride the **subscription's** packed `delivery_policy` (`core/include/libtracer/vertex.hpp:204,697`), and the STREAM ring depth is owner-side `graph_t::set_history_depth` (`graph.hpp:769`). The vertex-side `delivery_mode_t` (`vertex.hpp:553`) is also **owner-side and wiring-time** (`set_delivery_mode`) with no wire spelling and no `ON_CHANGE` member — so a libtracer-only delivery mode is an `rmw_tracer`-local concept it configures on vertices it owns, not something round-tripped to a remote peer.
