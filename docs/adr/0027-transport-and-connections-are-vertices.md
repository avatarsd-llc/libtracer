...
- **Dial direction is a deployment knob**, not a protocol rule; the consumer-dials default + forwarding-hop fallback covers leaf-out, fan-out-server, and folded-mesh topologies. There are **no `router`/`orchestrator` roles** — only transient hats any peer wears; the network is pure-decentralized and self-healing. The end-to-end flow is in [reference/13](../reference/13-network-formation.md).
