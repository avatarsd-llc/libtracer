...
The profile of the originating production firmware (an ESP32-C6 smart-agriculture node) is **CAN + a reliable IP transport** ([reference 12](../reference/12-deployment-profiles.md)). The open question was which reliable transport to build first — and whether board-to-board (controller-to-controller) links should be the same one. [The M6 research note](../research/m6-transport-and-batching.md) left **TCP-vs-QUIC** open; the maintainer asked specifically about **WebSocket vs QUIC**, flagging QUIC's **connection migration** as the one real reason to prefer it (roaming). This ADR settles the choice without locking the project in, because a transport is a swappable seam ([ADR-0027](0027-transport-and-connections-are-vertices.md)).

## Decision

**Ship WebSocket as the reliable transport for the origin firmware now — for the web-UI link *and* board-to-board** — and **defer QUIC** as a per-link, gateway-and-up upgrade behind the same `transport_t` seam.
...
QUIC is pulled forward **only if L3 roaming becomes the norm** for origin-firmware boards (cellular, vehicle-mounted, multi-subnet mesh).
...
- Confirms reference 12's origin-firmware = CAN + WS, now with the board-to-board + roaming rationale recorded so it isn't re-litigated.
