...
Status: **superseded by [ADR-0037](0037-net-side-channels-dissolve-into-vertex-tree-compositor.md).** The decision to dissolve the net side-channels into the vertex tree stands, but grilling the code surfaced that (a) the codebase carries a *second* side-channel — `fwd_router_t`'s children-table, isomorphic to a transport-vertex's child-vertices — which this ADR did not name, and (b) "relocate `bridge_t`'s ROUTER logic verbatim" is wrong: the connection-vertex's `forward` is `fwd_router_t`'s source-routing model, and `bridge_t`'s ROUTER egress is *deleted*, not relocated. ADR-0037 broadens the scope to both side-channels, replaces the relocate-verbatim framing with a compositor split (transport-vertex = composite, connection-vertex = leaf, root = resolver), and pins a five-invariant lean contract gated on a 16KB-RAM zero-heap build. Read 0037 as the operative record; this ADR is kept as the first-cut history.
