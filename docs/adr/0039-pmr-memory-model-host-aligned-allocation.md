...
freed it, i.e. a source-carrying deleter on `std::unique_ptr<vertex_t>` (`graph.hpp:1102`). Sizes
...
**Trap for any future attempt.** Do not "just route the root allocation first": `graph.hpp:1102`
declares `root_`, `graph.hpp:1148` declares `ctl_`, and C++ initialises members in **declaration**
