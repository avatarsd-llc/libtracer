...
- **`fwd_router_t`** (RFC-0004) — a *source-router that cannot loop*. A `FWD` frame carries its own route (`dst` shrinks per hop, `src` grows); a hop strips its segment and forwards the rest. A `dst` revisiting a node is malformed (`ERROR=INVALID_PATH`), so it is **loop-free by construction** — no dedup, no `hop_count`, no state.
  ...
  > **Erratum ([#445](https://github.com/avatarsd-llc/libtracer/issues/445), following [#420](https://github.com/avatarsd-llc/libtracer/issues/420) / [#444](https://github.com/avatarsd-llc/libtracer/issues/444)).** The `ERROR=INVALID_PATH` clause above is wrong, and it contradicts the rest of its own sentence: detecting that a `dst` revisits a node requires a visited-set, and the same sentence ends "no state". `fwd_router_t` has no visited-set, no hop counter, and never emits `INVALID_PATH` for a revisit — verified in `core/src/fwd_router.cpp`.
  >
  > **Loop-freedom comes from monotonic `dst` consumption, not from a revisit check.** Each hop strips one segment, so a route is finite and a delivery travels exactly as far as its route names and no further. A `dst` that spells out a physical cycle simply routes around it as many times as the route says, then stops. That is protection by construction; the error was never the mechanism, and no implementation ever emitted it.
  >
  > The conclusion of this bullet — no dedup, no `hop_count`, no state — is unchanged and is what the code does. Only the stated reason was wrong. [reference/05](../reference/05-protocol-tlvs.md) already says this correctly; [RFC-0004](../spec/rfcs/0004-remote-operation-addressing.md) §A still carries the phantom and is the last site, pending its instrument.
...
4. **No loss of loop safety.** FWD's `dst`-monotonicity is the loop bound for the source-routed plane, already in the code; deleting `bridge_t` removes no protection FWD relies on. (Per the erratum in §Context, the `INVALID_PATH`-on-revisit this item originally also credited never existed — the monotonic consumption is the whole mechanism, so the conclusion is unchanged.)
