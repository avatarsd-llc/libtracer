...
On a view-delivered terminus frame, a WRITE whose payload TLV (`node.wire`) is **≥ the vertex's `store_ref_min_bytes`** stores a **subview of the frame** (refcount bump, zero copy) instead of the ADR-0041 one-copy `own_tlv`; smaller payloads keep the copy (a copy under ~a few hundred bytes beats pinning — the Brick-2 lesson). The subview **pins the whole frame** until the next LKV write; the amplification is `frame_size / payload_size`, which for the dominant case (a delivery FWD whose payload is most of the frame) is ≈1.

`store_ref_min_bytes = 0` (the default) **disables referencing** — behavior is byte-identical to today. Opting in is a per-vertex owner-side call because pinning amplification is a deployment property, not a library presumption. A span-delivered frame ignores the setting (nothing to reference).
...
> **Erratum (2026-08-01), [RFC-0022](../spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md) §3.B:** the threshold's *value* and *predicate* are unchanged; its *home* is not. `settings_t` is deleted, so the knob is now owner-side vertex state set by `graph_t::set_store_ref_min_bytes(v, bytes)` and read by `graph_t::store_ref_min_bytes(v)` — it has **no remote write surface**, because a copy/pin trade is a deployment decision rather than a quality-of-service property a peer may request. Read `:settings.store_ref_min_bytes` below as that host call. RFC-0022 §3.D additionally proposes replacing the absolute threshold with an amplification ratio; that change is gated on §6's dual-target measurement and has **not** landed.
>
> **Erratum 2 (2026-08-02), RFC-0022 §3.D + Amendment 2, [#774](https://github.com/avatarsd-llc/libtracer/issues/774):** §3.D's amplification predicate DID land with §6's bench — the store decision is now `payload_bytes * K >= segment_bytes`, not `payload_bytes >= N` — and the owner-side declaration was renamed to match its value: `graph_t::set_pin_payload_ratio(v, k)` / `graph_t::pin_payload_ratio(v)`. Same u32, same owner-side home, no wire surface either way; only the name and the predicate changed. Amendment 2 fixes the shipped default at the sentinel (`kPinNever`) on both targets, so pinning is off unless a deployment declares a non-sentinel `K`. Read every `store_ref_min_bytes` in this ADR's body as that ratio, and its "≥ the threshold" wording as the ratio predicate.
