...
5. **Factory + docs**: a `quic` transport-factory builtin (registered only when the module is compiled in); reference/module docs present QUIC as what it is — the hosted, secure, migrating link — with the MCU exclusion stated plainly.
