...
> **Erratum (2026-08-03): §2 over-states what landed — two of its three clauses never shipped, and one of them must not.**
> What DID ship: `needs_cache_ops` and `is_isr_safe` as `static constexpr` traits consumed by the tag-dispatch fast path (`core/src/backend_set.cpp:48`), `tr::mem::backend_set` as the per-target module set, and `mem::transfer` with the memcpy default.
> What did NOT: **`mem_backend` never became a `concept`** (there is no backend concept anywhere under `core/`), and **`alignment()` is still a virtual** on the abstract `mem_backend_t` (`core/include/libtracer/backend.hpp:156`) — not a `static constexpr` trait, and no concrete backend declares one.
> This is not an outstanding task. Every seam ADR since injects a *runtime* `mem::mem_backend_t*` chosen at wiring time ([ADR-0039](0039-pmr-memory-model-host-aligned-allocation.md), [ADR-0060](0060-lkv-copy-store-injected-value-backend.md), [ADR-0065](0065-failable-allocation-gets-its-own-seam-block-source.md), [ADR-0067](0067-bounded-recycling-source-and-per-owner-topology.md), [ADR-0074](0074-terminus-reply-egress-is-its-own-injected-backend.md)), so the vtable is load-bearing and the alignment virtual is what a heterogeneous rope reads. §1's own rule is what settled it: a backend's identity arrives as **runtime wiring data**, so the seam stays runtime-dispatched and the tag-dispatch fast path **coexists with** the vtable rather than replacing it. A reader must not act on "is gone" by deleting the virtual.
