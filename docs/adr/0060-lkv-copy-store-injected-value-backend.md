...
- Follow-ups recorded: (i) `httpd_ws_link` owning delivery, which unlocks the subview store on the board — **landed 2026-08-28 as an opt-in per-link mode** ([#1565](https://github.com/avatarsd-llc/libtracer/issues/1565): name a bounded, integrator-injected `mem_backend_t` and an unfragmented inbound frame is recv'd into it and delivered owning; borrowed stays the default); (ii) collapsing the wrapper + value into a single per-write `segment` to retire `mr_`.
