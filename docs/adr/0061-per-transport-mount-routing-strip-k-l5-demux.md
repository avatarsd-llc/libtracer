...
- **The forward decision cannot move onto the L4 graph.** A `vertex_t` may not carry a `transport_t*`, and `graph.find(child) → transport` was litigated-and-rejected, both in [ADR-0038](0038-net-plane-performance-model-two-plane-forwarding-and-buffer-lifetime.md) §3b.1 (grounded in `vertex.hpp`'s own *"L4 never depends on `tr::net`"*). Earlier revisions of this ADR cited ADR-0016 for the rule; that citation is corrected by ADR-0038's 2026-07-30 erratum. Putting an "is-mount" marker on the vertex and injecting an L4→L5 forward sink re-courts exactly that inversion.
- **Bus peers have no vertex at all.** A multi-peer `ws`/`tcp`/`can` peer is synthesized on read; `transport_vertex.cpp:277-278` — "NO vertex is ever created for a peer." Pure graph-descent black-holes every peer. The registry's `by_name`/`peer_link` fallback (`child_registry.hpp:520-544`) is the only thing that reaches them, and `by_name` is used pervasively on the reply/advertise paths (`fwd_router.cpp:489,697,715,1264,1296,1498,1575,1673`) with no graph substitute.
...
**Spec status (adversarially confirmed against the artifacts):** `v1.md` pins **no MUST** on the first-`dst`-segment shape (it incorporates only the reference/03 path-syntax constraints + reference/05); `reference/03:216` marks `/net/<conn>/…` a *convention*, not normative. The normative conformance vector `fwd-routed-multihop/expected.json` (renamed `fwd-routed-mount-residual` by #419 on 2026-08-02, bytes unchanged) **already encodes the mount form** (`dst=/net/board/can0/ow/sensor`, `net` as `segment[0]`), consumed as one whole `net/<module>/<name>` run. Its sibling `fwd-src-accumulated` is the mid-route companion (`dst=/net/uplink/d/sensor/temp`, `src=/net/downlink/a/net/downlink/cli/reply-ep`) — `src` grown by two FULL mount runs, six segments, not two bare NAMEs; **that vector's bytes were rewritten on 2026-08-02** under maintainer ruling (b) on [#419](https://github.com/avatarsd-llc/libtracer/issues/419), because as authored it decayed segment-by-segment, a pre-S2a reading that does not compose. So the flat bare-name impl is the divergence, and aligning it to the mount form is a **conformance fix (#419), not an RFC**. The per-transport two-level nesting is governed by the **already-accepted RFC-0014** ([#492](https://github.com/avatarsd-llc/libtracer/issues/492)); its only spec cost is rewriting the vector *bytes* under #492's draft-spec authority.
...
4. **Layering and intra-device invariants are upheld.** `transport_t*` stays in L5 (ADR-0038 §3b.1; the general layering principle is ADR-0016 Decision 2, which scopes itself to L0↔L1). A local `dst` still descends to a local vertex and terminates — the forward arm is reached only for a `dst` that names a transport mount (ADR-0038 §3a, a hard invariant).
...
- **Vector byte rewrite is RFC-0014-governed → S7.** Strictly circular as a gate: S7 is four slices downstream of the S2 this ADR blocks. The wire-visible `dst` becomes `/net/<module>/<name>[/<peer>]/residual`; rewrite `fwd-routed-multihop` (now `fwd-routed-mount-residual`) + `fwd-src-accumulated` under RFC-0014/[#492](https://github.com/avatarsd-llc/libtracer/issues/492)'s "proposed pending" clause-kind authority on the DRAFT spec — **not silently as a #419 tooling change** (`v1.md` §conformance: adding a vector is free, changing existing bytes is a spec change). **DONE, 2026-08-02 ([#419](https://github.com/avatarsd-llc/libtracer/issues/419)):** `fwd-routed-multihop`'s bytes needed no change at all — it always encoded the mount form, so it was renamed `fwd-routed-mount-residual` bytes-untouched (ruling (a)). `fwd-src-accumulated` needed a byte rewrite and got one under an explicit maintainer ruling (b); a third ruling (c), the same day, applied the identical fix to the two vectors that carry the *reverse* leg of the same route, `fwd-reply-result` and `fwd-reply-error` — their `dst` was the same pre-S2a `/via_board/via_net/reply-ep`. Those three are the only conformance vectors whose bytes have ever changed, and after them no vector carries the pre-S2a form. **Keep the two gaps separate:** mount-vs-bare-name (pure impl + doc + vector conformance, no RFC) and one-level→two-level nesting depth (RFC-0014-governed).
...
- Retires the #373 `has_first_level_child` shadow-guard once connection names are no longer top-level — **only after verifying no other first-level shadowing** depends on it. *(Citation corrected 2026-08-03: the guard is a `graph_t` member — `graph.hpp:1121` / `graph.cpp:727` — and was never in `transport_vertex.cpp`, which the original text named. It is **still in tree**, so this Consequence remains an open obligation, not a done one.)*
