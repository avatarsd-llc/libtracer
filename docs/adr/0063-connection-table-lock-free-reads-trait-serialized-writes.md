...
already recorded at `vertex.hpp:467`), i.e. ~188 B one-time for both locks — not per connection.
...
- **`transport_vertex_t` gains its first synchronization.** Anything called under the control-plane trait must not re-enter the graph's mutation APIs or block — the same discipline `graph.hpp:479` already documents for resolvers, and sharper here if the trait resolves to an interrupt-disable critical section.
