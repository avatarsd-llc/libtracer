...
- ~~**The multi-writer win is unmeasured.** No bench in this repo exercises concurrent publishers on one stripe, so the contention benefit is argued, not shown. A bench that does would also make §2's case measurable, and is the natural prerequisite to attempting it.~~
  ...
  **Erratum, 2026-08-15 ([#1285](https://github.com/avatarsd-llc/libtracer/issues/1285)): the alternative this control rules out does not exist on this libstdc++.** There is no address-hashed lock-bit table: `_Sp_atomic` keeps the lock **in the object**, as the LSB of the atomic's own pointer word, so two `atomic<shared_ptr>` instances cannot alias on a lock at *any* pair of addresses. The control's conclusion stands unchanged — the collapse is caused by sharing one slot — but what it "rules out" was never a candidate mechanism. The correction matters beyond wording: the hashed-table story was load-bearing in [#841](https://github.com/avatarsd-llc/libtracer/pull/841)'s reasoning about why a vertex's *address* could move the hot-vertex arm. #1285 found the real address sensitivity, and it is geometric, not associative: `lkv_`'s two 8-byte words straddle a 64-byte cache line at one of glibc's four 16-aligned placements (`mod64==32` before the [#1330](https://github.com/avatarsd-llc/libtracer/pull/1330) reorder), doubling the coherence footprint of every publish under contention.
