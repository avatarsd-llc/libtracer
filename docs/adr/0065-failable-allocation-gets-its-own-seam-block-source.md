...
`-Os` is what the reference ESP-IDF node ships (`CONFIG_COMPILER_OPTIMIZATION_SIZE`; note the IDF *default* is `-Og`, so this is a deployment choice, not a toolchain given). No *host unit-test* job compiles at it — core-ci, conformance, ws-interop and can-vcan build `-O0`/`-O1`/`-O2`/`-O3` — and the two footprint sentinels that do compile at `-Os` run nothing. One `-Os` job **does** execute: `esp-idf.yml`'s `full-node-host` builds the `full_node` example (its `sdkconfig.defaults` sets `CONFIG_COMPILER_OPTIMIZATION_SIZE=y`) for the linux target and runs the binary, which exits non-zero on failure. But it drives read / subscribe / latch / fan-out / await — it never drives **allocation exhaustion**, so the soft-fail branch this decision is about is still executed by nothing. So reusing `pmr` would have traded a diagnosed reboot for an undiagnosed null-deref plus a false SUCCESS on the wire, on a code path no job exercises at the level that breaks it.
...
[ADR-0060](0060-lkv-copy-store-injected-value-backend.md) §1 already chose `mem_backend_t` over `std::pmr` for durable *byte buffers*, so the natural question is why this is not a third use of it. Because `mem_backend_t` vends a **refcounted `view::segment_t`**, and the objects here have exactly one owner and no header: a `segment_t` measures 20 B on rv32 / 40 B on x86-64 against a `vertex_t` of 80 B (rv32) / 112 B (x86-64), so a refcount on them is pure overhead on the axis (RAM) that motivated the seam. The two L0 seams are complementary: `mem_backend_t` for shared payload bytes, `block_source_t` for single-owner failable blocks.

### 4. Latency, RAM and throughput do not discriminate — the decision is made below them

Stated explicitly because the priority order (latency > RAM > throughput > minimalism > unification) invites the assumption that it did. The call site is **33 instructions** through `block_source_t` and **33 instructions** through a `memory_resource`-derived class; the two bodies differ by one line, the vtable-slot immediate. The seam pointer is 4 B rv32 / 8 B x86-64 either way. The implementation object differs by one word — `block_source_t` carries a `name_` literal that `memory_resource` has no equivalent of — which is per-source, not per-allocation. The vtable-size delta is `.rodata`.
...
The rule turns on two conditions together: identity per-target-fixed **and** the path hot or size-critical. The backend set meets both. This seam meets the first and not the second in the way that matters. Its `try_alloc` is called **twice for a typical frame** — to size the node array and the open-node stack — and O(log n) beyond that, since `block_array_t` doubles; never once per node, and never on the span forward hop, which is offset-dispatch and allocates nothing (ADR-0038). The measurement is on the record: the migrated terminus decode came out at 236 ns against `main`'s 241–251 ns, i.e. one indirect call per allocation is below the noise of the operation it serves.
...
**No test exercises an allocation-failure path at the shipping optimization level.** §1 turned on a narrower version of this than first written: CI *does* execute one `-Os` binary (`full-node-host`'s `full_node` self-proof), but it drives the happy path. The general form still stands and is not specific to `pmr` — any construct whose correctness depends on the optimizer's assumptions can differ between what CI runs and what deploys, and nothing here forces an exhaustion branch to be compiled the way it ships. Recorded as a standing gap rather than fixed by this ADR.
...
**The shipped source set cannot yet bound a long-lived seam.** `heap_source()` recycles but is unbounded; `bump_source_t` is bounded but never reclaims a bump block, which makes it correct for a **scope-lifetime** consumer (the branch-write decode constructs one per call) and wrong for a long-lived one — wired as a router's `rx`, an 8 KiB bump source decoded 6 frames and rejected the next 194. So "one slab, whole stack" is not yet reachable for the failable seam: a bounded node must supply its own recycling `block_source_t` (a free-list over its slab) or leave `ctl`/`rx` on the heap. A pool-backed recycling source is the missing piece and is tracked separately. **[Closed 2026-07-28 by [ADR-0067](0067-bounded-recycling-source-and-per-owner-topology.md) — `tr::mem::pool_source_t` ships the recycling source, and `fwd_router_t` takes a per-child one so it can be used on the RX path without contending. This paragraph is retained as the record of what was outstanding.]**

**Migration is ordered, and incomplete.** Landed: the terminus arena (node array, sink stack, walk spill), the branch-write decode, and the **rope forward hop's egress iov** ([#596](https://github.com/avatarsd-llc/libtracer/issues/596) — the last peer-count-driven `std::pmr` growth on a path behind no ACL; it now draws from `fwd_router_t::rx_` and drops the hop on exhaustion rather than emitting a truncated frame). Outstanding: **vertex registration** — the allocation cluster that opened [#551](https://github.com/avatarsd-llc/libtracer/issues/551) (the `reg_escape` probe measures 4 blocks per registration; an earlier count of seven came from enumerating call sites, not blocks) — plus the `route_handle` label tables (drawn from **`fwd_router_t::mr_`**, not the graph's) and the `can_reassembly` maps. Registration is the hard one and is not merely mechanical: its allocations are `unique_ptr`-owned, a stateful deleter costs 8 B on rv32, and `vertex_t` has exactly **one** 4-byte hole (`+mr_` alone already puts x86-64 at 120 = `kMax64`, and both members put rv32 at 88 > `kMax32` = 80). It needs an ownership scheme that spends no per-vertex bytes, and the size gate will judge it.
