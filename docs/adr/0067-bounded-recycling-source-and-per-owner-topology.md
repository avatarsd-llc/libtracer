...
- **`fwd_router_t` conforms to §3.** It previously held a single `rx_` while its own class comment stated that `on_frame` fires on transports' receive threads *"possibly several concurrently"* with *"no per-request locking required"* — making `rx_` the one shared mutable object on a deliberately lock-free path. `child_rx_ctx_t` now carries an optional `mem::block_source_t* rx`, supplied through `add_child(name, link, rx)`; the two draw sites resolve it through `rx_for(ctx)`, a branch on a pointer already in a register. A child with no source of its own falls back to the router's, so every existing call site is unchanged and the default remains `heap_source()`. There is deliberately **no lookup by name** on the frame path — resolving the child per frame would trade a pointer for a string scan, which is backwards under latency-first.
  - This also makes the bound **per-peer**: one noisy link cannot starve another link's decode, which a single shared slab could not express.
  - `resolve_terminus_rope` takes no source and was left alone: it resolves off the rope through `tlv_view_t::over` with no arena, so threading a context into it would have been dead API surface.
