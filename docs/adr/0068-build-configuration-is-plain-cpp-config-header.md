...
- **Landed in two steps, deliberately.** This erratum's core half is #1142; the ESP-IDF half is #1244, which converted the component to a fragment and **deleted `config.hpp.in`**. `core/include/libtracer/config.hpp` is now the tree's only spelling of a default, and no build renders anything.
...
- The ESP-IDF component writes a four-knob fragment from its Kconfig values and chip facts instead of rendering the whole header, so it no longer carries a full copy of core's defaults (#1244). The Kconfig option names are unchanged, and no configured value changed with the conversion.
