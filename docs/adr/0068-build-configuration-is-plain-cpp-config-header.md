...
- **Templating the public types directly (`graph_t<Slot>` as the user spelling).** Rejected: API cleanliness was a stated constraint; the `basic_string`/`string` idiom delivers per-target selection with zero churn in the 67 files that spell `graph_t`.
...
- The LKV slot work (#604) lands as `basic_graph_t<Slot>` + explicit instantiation under this ADR's §2 binding; its own ADR still owes the reclamation-scheme argument (hazard vs epoch), not the mechanism.
