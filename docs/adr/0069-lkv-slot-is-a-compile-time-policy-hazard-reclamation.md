...
- **The hazard slot makes publish fallible.** `sp_atomic_slot_t` never allocates on publish; this one does when its free list is empty, and a failure leaves the previous value published while `vertex_t::store` still reports success. Warm-up-only after the free list lands, but it is a new failure mode in the policy's surface, not a shared one.
