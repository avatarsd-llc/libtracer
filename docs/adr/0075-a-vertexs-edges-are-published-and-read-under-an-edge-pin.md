...
The distinct-stripe control (`spread-fan1`) stays at unity across every T, which is what attributes the gain to the stripe. The `hot1` arms are excluded from acceptance and rejection alike, but not for the reason first recorded here. `hot1-fan1` is genuinely bistable ([#464](https://github.com/avatarsd-llc/libtracer/issues/464)) — the same two binaries, twenty minutes apart on one host, gave ×0.922 (disjoint worse) and ×0.949 (overlapping) on aggregate throughput. **`hot1-fan0` is not bistable; it moves deterministically with `sizeof(vertex_t)`.** An independent verify measured the mechanism: holding the code identical and padding the candidate back up, the arm reads 2.73 M/s at 96 B, 2.71 M/s at 104 B and 7.95 M/s at 112 B (T=8, 13 interleaved rounds). glibc rounds both a 96 B and a 104 B request into the same 112 B chunk and a 112 B request into the 128 B chunk, so the measurement groups on the allocator's **chunk class**, not on the byte count. At equal `sizeof` the movement disappears (×0.96, overlapping) — and *that* is what clears the arm. It is true that `fan_out` returns on the #708 gate (`graph.cpp:938`) before reaching one instruction of this change, but that fact alone never cleared it: this change moves every vertex's size and therefore its allocation class, which is a channel no instruction-level argument covers. Two limits on the clearing, stated rather than buried: the ablation was run at T=8 only, so the same result at T=16/24 is inferred, not measured; and the ×1.26 recorded for this arm in the implementing session, against ×0.33 in two later 13-sample runs, is unexplained.
