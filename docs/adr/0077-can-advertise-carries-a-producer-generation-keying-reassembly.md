...
**Why the generation could not have closed the residue either.** A generation rides the *advertise*; a **data frame carries no generation** — the 29-bit ID is fully consumed by `version|node|endpoint`, which is the whole point of the header-elided design. So for the one case neither instrument reaches — a slice parked *before* its advertise, re-driven into whichever group later claims its slot — a generation on the manifest is silent, exactly as the overlap-erase is. That residue is bounded by [#912](https://github.com/avatarsd-llc/libtracer/issues/912)'s `rx_ttl` age-out and is timing-dependent by nature, not by choice of key.
