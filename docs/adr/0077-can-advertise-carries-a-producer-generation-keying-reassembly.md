...
**Residue 2 is closed, and the wire change stayed declined** ([#1011](https://github.com/avatarsd-llc/libtracer/issues/1011), 2026-08-14). The weld above was reproduced at `main` and is now **refused rather than delivered**. Three instruments were priced; the choice was made explicitly rather than by default:

1. *Bind group identity into the data frame.* This is decisions 1+2 taken all the way — and it does not work, because a generation on the manifest is silent for a group whose manifest was lost. Making it reach the data frame means spending endpoint bits on a 29-bit ID that `version|node|endpoint` already fills, a `kAdvertiseFormatVersion` bump, and the mixed-version decode matrix this ADR flags as making the decision hard to reverse. **Declined**, which retires decisions 1 and 2 from *held* to *rejected*.
2. *Refuse rather than weld.* **Chosen.**
3. *Accept, document, and count.* Declined as a decision to ship a known silent-corruption window; the counter it asks for was worth having on its own and shipped as part of (2).

**How "no advertise in this lap" was made decidable without a wire change.** The producer's `alloc_base` is monotone within a lap and resets to `kCanFirstDataEndpoint` at the wrap, so an advertise whose base does **not exceed** the last base that node placed *is* the wrap, observed by the receiver from frames it already parses. Every binding of that node is then flagged as belonging to a prior lap, and a data slice resolving to a prior-lap binding is dropped — the group discarded on `dropped_groups()`, the slice counted on the new, distinctly named `transport_can::dropped_stale_binding()`. State added: one `std::uint16_t` per remote node (folded into the per-node control-stream entry that already existed, so no second map) and one `bool` per binding, which lands in padding `binding_t` already carried — **zero added bytes per binding**. The common path costs one already-loaded `bool` test per data slice: no lookup, no clock, no allocation.

This is deliberately **not** the general epoch framework decision 1 would have started. There is no generation on the wire, no per-group sequence, and nothing a sender must emit or a peer must understand. The lap is inferred, receiver-locally, from the allocator's own observable behaviour — which is why a mixed-version bus needs no rule at all.

**What it costs a conforming sender.** A group can now be refused for a reason the sender cannot observe: if its advertise is lost on the bus *and* its slots were previously held by a binding from an earlier lap, its data is dropped instead of welded. That is a delivery traded for a counted drop, the same disposition already chosen for the fabricated-empty-slice and DLC-0 cases — and the delivery it replaces was byte-corrupt.

**Residue 1 (the parked slice) remains open** and was deliberately not conflated: the refusal keeps the stale binding in place precisely so its slices resolve, are refused, and are counted, rather than being pushed into the pending park where a later advertise could re-drive them.
