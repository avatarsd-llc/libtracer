...
- **[#375](https://github.com/avatarsd-llc/libtracer/issues/375)'s deliverable 4 is discharged.** Its deliverable 3 (compiling out the `bus_link_t` machinery on minimal builds per [ADR-0047](0047-build-time-closed-module-sets-compile-time-seams.md), and pairing `true` with an injected per-peer bound per [ADR-0079](0079-allocation-store-composition-defaults-to-per-plane-mid.md)) is unaffected and still open — this ADR strengthens its case, because a default-`false` flag on a NARROW target is precisely a module set that can be closed out at build time.
