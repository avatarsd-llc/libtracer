...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:1849-1850` | the delivery is **dropped** |
| Per-delivery frame build | `core/src/fwd_router.cpp:1854` | the delivery is **dropped** |
...
drops that one delivery (`core/src/fwd_router.cpp:1849-1850`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:1832`).
