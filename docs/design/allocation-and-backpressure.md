...
"does". **One** site still reports exhaustion by throwing, so "nothing on the delivery path can
abort" is not yet unconditionally true. (There were three. The CAN egress window table went away
in #1110, which deleted the table rather than bounding it — the windows are derivable from the
payload length and the mode, so `split` allocates nothing at all. The peer-driven label-table
binds of #603 defect 1 went away when `route_handle_t` moved onto the `block_source_t` substrate:
every byte of label state is drawn from an injected source and every door answers exhaustion by
value, degrading the flow to the full-route `FWD{WRITE}` form.) The one that remains is NOT
peer-provoked — read the "who provokes it" column rather than assuming it is, because the provoker
is what decides whether a bound is an admission-control problem or a sizing one. It is
named here so a bounded deployment prices it rather than rediscovers it; this list is the
sites found by the sweeps behind #603, #850 and the CAN egress review, and is not claimed to be
exhaustive:

| Site | Code | Who provokes it |
| --- | --- | --- |
| `try_reserve` on `-fno-exceptions` (#923, #850) | `core/include/libtracer/mem_heap.hpp:169-183` — `try_grow` catches the container's own allocation failure where it can; where it cannot (`-fno-exceptions`, where `reserve` `abort()`s with nothing to catch) it falls back to probe-then-commit | on the MCU profile only, anything concurrent — a FreeRTOS context switch between the probe's free and the `reserve` is enough. The hosted profile no longer has the window; the exception-free one closes it by migrating the site to the ADR-0065 failable seam, not by a better `try_reserve` |
...
egress gather) does not close it either.

## The one injected source, and the four channels it feeds
...
platform heap and byte-identical behaviour (`core/include/libtracer/graph.hpp:608`; the
reference spelling is beside it at `:617`, and the parameter's whole contract is documented at
`:599`).
...
sizes and censuses ONE store.

| Channel | Reaches the source through | What it allocates | Exhaustion |
| --- | --- | --- | --- |
| `mr_` (`graph.hpp:2860`) | `tr::mem::source_resource_t` (`src_mr_`, `graph.hpp:2837`) | the small control *objects* of a stored write: the `shared_ptr` control block and the `rope_t` wrapping the value's links | throws — `std::pmr` structurally cannot report by value, so the adapter translates `nullptr` to `bad_alloc` at its own boundary |
| `value_backend_` (`graph.hpp:2876`) | `tr::mem::source_backend_t` (`src_backend_`, `graph.hpp:2826`) | the graph's **payload** byte `segment`s: the durable buffer holding a vertex's last-known value when the write path must own its bytes, and (since #831) **both** folded READs' POINT headers — the composed root's per-node header and the `":children"` listing's per-member + outer header | `nullptr` → the operation answers `BACKPRESSURE` |
| `ctl_` (`graph.hpp:2995`) | directly — it IS the injected source | every allocation a peer can provoke | `nullptr` → the operation answers a status |
| `ring_` (`graph.hpp:3010`) | directly | the graph-level DEFAULT for a receiving STREAM vertex's ring ADMISSIONS — the reservation each queued entry holds until it retires ([RFC-0025](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0025-stream-class-values.md) §4.6.1). A vertex that declares its own through `graph_t::set_ring_source` never touches this one. It bounds admission, NOT placement | `nullptr` → best-effort sheds the oldest with a gap; reliable answers `BACKPRESSURE` |
...
seam existed, which keeps the forward-hop bench measuring the same layout (`graph.hpp:2988`).
...
#873 **phase 2** built the migration and **reverted** it on its own gate (+22.7 % on hazard-node
acquisition, +3.5 % on the free-list-hit steady arm, ranges disjoint against a −0.12 % A/A null),
which the ruling named an acceptable terminal outcome. And the plain `std::vector<std::byte>`
sites — the KEY containers and the read-back encoders' staging buffers — are an allocator swap
only in appearance, because their container type is fixed by the signatures they cross; the
`std::vector` growth behind `tr::detail::try_reserve` / `try_push_back` migrates only where the
element type allows it, and the sites that cannot are annotated in place.

**Phase 3 widened `value_backend_`'s row above.** Every segment the graph's READ-BACK encoders
mint — `:point`, `:settings`, `:settings.app`, `:acl`, `:children`, the identity record, the
stats block, an app field's stored bytes and the subscriber / mount-route records `subscribe`
composes — was reaching the global heap through `view::over_bytes`'s one-argument overload, so a
peer's READ escaped the injection. They now draw from the graph's own backend (published as
`graph_t::value_backend()`), and a refusal is the `BACKPRESSURE` every one of those sites already
...
contract: `egress` (`fwd_router.hpp`, documented on the constructor), the `mem_backend_t` the
terminus **reply head** and its RFC-0024 mint draw from. It is kept separate from `flat` because a
reply head is egress construction sized against *route* bytes, not the *payload* bytes a flatten
seam is sized against — folding it into `flat` would silently re-scope a slab already sized for
...
same underlying slab; the composition guidance is in
...
the reply head that was the last such source is detailed in
...
in that table, not "the router's allocations".
...
the span tier's counterpart, which #766 had not counted at all; #795 closed the reply head — the
last one — with its own `egress` seam. None of what `flat` does not cover is an oversight, and the
reason each is out is different — so they are named here rather than left to be rediscovered:

| Site | Code | Verdict |
| --- | --- | --- |
| `own_wire`'s SINGLE-link ownership copy (rope tier) | `core/src/op_resolve_view.cpp:151` | **Closed by #793.** Same function, same ADR-0041 §2 obligation and same peer-drivability as the multi-link flatten beside it — one function must not draw from two allocators. |
...
(`core/src/op_resolve_walk.hpp:565`) turns into an addressed `kind=ERROR BACKPRESSURE` rather than
...
rather than the payload. Pointing it at `flat` would silently re-scope an injection somebody has
already sized, so a node that budgeted a small pool for flattens could begin refusing replies it
used to send. So the terminus egress gets its own `mem_backend_t`, defaulting to the global heap
so an un-injected node is byte-unchanged; a bounded node points it at its slab and this last
terminus allocation joins the bound. The seam decision is
[ADR-0074](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0074-terminus-reply-egress-is-its-own-injected-backend.md).
The honest statement is now: **`flat` covers every rope flatten and every rope-tier ownership copy
on the forward and terminus paths; the reply head and its mint draw from the injected `egress`
backend, answer exhaustion by value, and — with `egress` pointed at the node's slab — are bounded
too, leaving no terminus byte source on the global heap.**
...
(`core/include/libtracer/mem_source.hpp:333`). The `noexcept` is the whole point: the override
...
| --- | --- | --- |
| `heap_source()` (`mem_source.hpp:279`) | free function, process-wide | wraps the platform allocator; the default for all three seams |
| `null_source()` (`mem_source.hpp:300`) | free function, process-wide | serves nothing; makes a `bump_source_t`'s buffer a hard bound |
| `bump_source_t` (`mem_source.hpp:325`) | `bump_source_t(std::span<std::byte> buffer, block_source_t& upstream = heap_source())` | carves from `buffer`, falls back to `upstream` once it cannot fit |
| `pool_source_t` (`mem_source.hpp:507`) | caller-supplied slab plus a caller-supplied span of size classes | segregated exact-size free lists; recycles, so it suits a long-lived seam |
...
exists and fails as a value where it does not (`mem_source.hpp:311-314`).
...
(`core/src/graph.cpp:2501-2502`.) Three properties follow, and each closes a different failure mode:
...
- **A bounded node that injected `ctl` gets its own store here too.** The overflow leg draws from
  ...
  arena** (`graph.cpp:2498-2500`). Read that literally: it is a statement about the decode arena, not
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:2483-2488` | the refusal's cause (#917): `flatten_err_t::NO_MEMORY` → `BACKPRESSURE`, `NOT_HOST` → `TYPE_MISMATCH` |
| Field-write flatten into the value backend | `core/src/graph.cpp:2830-2863` | same two-verdict split as the branch-write row above |
| Branch-write root key render (`try_build_key`) | `core/src/graph.cpp:2520-2521` | `false` → `BACKPRESSURE` |
| Branch-write parse-key copy (`detail::try_assign`) | `core/src/graph.cpp:2526` | `false` → `BACKPRESSURE` |
| Branch-write decode arena | `core/src/graph.cpp:2501-2504` | decode error → `TYPE_MISMATCH` |
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:3392-3393` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:3395`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:2816` (the make-contiguous seam the ADVERTISE arm asks at `:2764`), answered at `:2771` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:2816` (the same seam, asked at `:2783`), answered at `:2790` | the delivery is **dropped**; the subscriber keeps its last-known value |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:2771`, still an `empty()` test — that arm reads a
span through the make-contiguous seam) and bus-name (`core/src/fwd_router.cpp:2152-2154`, since
...
nobody can fail is a guard nobody can prove.
...
drops that one delivery (`core/src/fwd_router.cpp:3392-3393`). Dropping *invisibly* is the part that
...
subscriber set and still returns success (`core/src/graph.cpp:2757`), so it moves the counter by
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:3371`). Since #885 the router
...
an `abort()` under `-fno-exceptions` (`core/include/libtracer/rope.hpp:301-305`). The terminus reply
...
(`rope.hpp:329-334`): it clears `out`, sizes it to `link_count()` through `tr::detail::try_reserve`,
...
(as the branch-write decode does) or `reset()` it between operations (`mem_source.hpp:367`).
A **long-lived** bounded seam — a router's `rx`, a graph's `ctl` — wants `pool_source_t`, which
recycles (`mem_source.hpp:316-321`).

`bump_source_t` is also single-threaded by contract: a bump cursor is not synchronized, and its
intended use is a function-scoped buffer on the calling thread's stack (`mem_source.hpp:322-323`).
...
corroborated at `core/include/libtracer/mem_source.hpp:320` and
