...
| --- | --- | --- | --- |
| `mr_` (`graph.hpp:1239`) | `std::pmr::memory_resource*` | the small control *objects* of a stored write: the `shared_ptr` control block and the `rope_t` wrapping the value's links | throws — structurally cannot report by value |
| `value_backend_` (`graph.hpp:1248`) | `mem::mem_backend_t*` | the durable byte `segment` holding a vertex's last-known value when the write path must own its bytes | `nullptr` → write rejects `BACKPRESSURE` |
| `ctl_` (`graph.hpp:1308`) | `mem::block_source_t*` | every allocation a peer can provoke | `nullptr` → the operation answers a status |
...
restated at `graph.hpp:1299-1300`). `ctl_` is declared last in the object on purpose: no hot path
reads it, so placing it there leaves every other member at the byte offset it had before the seam
existed, which keeps the forward-hop bench measuring the same layout (`graph.hpp:1302-1306`).

`fwd_router_t` carries the same failable seam separately as its `rx` parameter
(`core/include/libtracer/fwd_router.hpp:155`), because the terminus arena decode belongs to the
router's receive thread rather than to the graph. It carries a **fourth** injection beside it, and
for a different contract: `flat` (`fwd_router.hpp:156`, documented at `:96`), the `mem_backend_t` **every rope flatten on
...
(`core/src/graph.cpp:1109-1044`.) Three properties follow, and each closes a different failure mode:
...
- **A bounded node that injected `ctl` gets its own store here too.** The overflow leg draws from
  ...
  arena** (`graph.cpp:1106-1108`). Read that literally: it is a statement about the decode arena, not
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:1096-1097` | empty head with a non-zero rope length → `BACKPRESSURE` |
| Field-write flatten into the value backend | `core/src/graph.cpp:1333-1334` | empty head with a non-zero rope length → `BACKPRESSURE` |
...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:1652-1653` | the delivery is **dropped** |
| Per-delivery frame build | `core/src/fwd_router.cpp:1657` | the delivery is **dropped** |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:1296-1297`, answered at `:1304` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:1317`, answered at `:1324` | the delivery is **dropped**; the subscriber keeps its last-known value |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:1304`) and bus-name (`core/src/fwd_router.cpp:918`) flattens are redundant with the `wire::decode`
...
drops that one delivery (`core/src/fwd_router.cpp:1652-1653`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:1638`).
...
(`fwd_router.hpp:212`, resolved at `fwd_router.hpp:765-767`) precisely so each transport's receive
