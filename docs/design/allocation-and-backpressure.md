...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:3354-3355` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:3357`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:2778` (the make-contiguous seam the ADVERTISE arm asks at `:2726`), answered at `:2733` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:2778` (the same seam, asked at `:2745`), answered at `:2752` | the delivery is **dropped**; the subscriber keeps its last-known value |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:2733`, still an `empty()` test — that arm reads a
...
drops that one delivery (`core/src/fwd_router.cpp:3354-3355`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:3333`). Since #885 the router
...
(`fwd_router.hpp:479`, resolved at `fwd_router.hpp:1968-1968`) precisely so each transport's receive
