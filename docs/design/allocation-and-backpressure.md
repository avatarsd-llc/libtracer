...
(`core/src/graph.cpp:2063-2064`.) Three properties follow, and each closes a different failure mode:
...
- **A bounded node that injected `ctl` gets its own store here too.** The overflow leg draws from
  ...
  arena** (`graph.cpp:2060-2062`). Read that literally: it is a statement about the decode arena, not
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:2045-2050` | the refusal's cause (#917): `flatten_err_t::NO_MEMORY` → `BACKPRESSURE`, `NOT_HOST` → `TYPE_MISMATCH` |
| Field-write flatten into the value backend | `core/src/graph.cpp:2363-2369` | same two-verdict split as the branch-write row above |
| Branch-write root key render (`try_build_key`) | `core/src/graph.cpp:2082-2083` | `false` → `BACKPRESSURE` |
| Branch-write parse-key copy (`detail::try_assign`) | `core/src/graph.cpp:2088` | `false` → `BACKPRESSURE` |
| Branch-write decode arena | `core/src/graph.cpp:2063-2066` | decode error → `TYPE_MISMATCH` |
...
still returns success (`core/src/graph.cpp:1980`), so it moves the counter by the fan-out width
