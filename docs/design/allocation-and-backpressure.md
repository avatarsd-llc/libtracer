...
heap and byte-identical behaviour (`core/include/libtracer/graph.hpp:324-326`).

| Seam | Type | What it allocates | Exhaustion |
| --- | --- | --- | --- |
| `mr_` (`graph.hpp:1801`) | `std::pmr::memory_resource*` | the small control *objects* of a stored write: the `shared_ptr` control block and the `rope_t` wrapping the value's links | throws — structurally cannot report by value |
| `value_backend_` (`graph.hpp:1810`) | `mem::mem_backend_t*` | the graph's **payload** byte `segment`s: the durable buffer holding a vertex's last-known value when the write path must own its bytes, and (since #831) **both** folded READs' POINT headers — the composed root's per-node header and the `":children"` listing's per-member + outer header | `nullptr` → the operation answers `BACKPRESSURE` |
| `ctl_` (`graph.hpp:1899`) | `mem::block_source_t*` | every allocation a peer can provoke | `nullptr` → the operation answers a status |
...
and so retiring `mr_` later is a compile error rather than a silent rebind (`graph.hpp:311-323`,
restated at `graph.hpp:1890-1891`). `ctl_` is declared last in the object on purpose: no hot path
reads it, so placing it there leaves every other member at the byte offset it had before the seam
existed, which keeps the forward-hop bench measuring the same layout (`graph.hpp:1893-1897`).
...
(`core/src/op_resolve_walk.hpp:544`) turns into an addressed `kind=ERROR BACKPRESSURE` rather than
...
(`core/src/graph.cpp:1484-1485`.) Three properties follow, and each closes a different failure mode:
...
- **A bounded node that injected `ctl` gets its own store here too.** The overflow leg draws from
  ...
  arena** (`graph.cpp:1481-1483`). Read that literally: it is a statement about the decode arena, not
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:1466-1471` | the refusal's cause (#917): `flatten_err_t::NO_MEMORY` → `BACKPRESSURE`, `NOT_HOST` → `TYPE_MISMATCH` |
| Field-write flatten into the value backend | `core/src/graph.cpp:1792-1798` | same two-verdict split as the branch-write row above |
| Branch-write root key render (`try_build_key`) | `core/src/graph.cpp:1503-1504` | `false` → `BACKPRESSURE` |
| Branch-write parse-key copy (`detail::try_assign`) | `core/src/graph.cpp:1506` | `false` → `BACKPRESSURE` |
| Branch-write decode arena | `core/src/graph.cpp:1484-1487` | decode error → `TYPE_MISMATCH` |
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:2243-2244` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:2246`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:1834` (the make-contiguous seam the ADVERTISE arm asks at `:1784`), answered at `:1791` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:1834` (the same seam, asked at `:1803`), answered at `:1810` | the delivery is **dropped**; the subscriber keeps its last-known value |
| Bus-name rejection reply flatten (cold) | flatten `core/src/fwd_router.cpp:1184`, answered by the `wire::decode` opening `reject_bus_name_hop` | the frame is **dropped** by value — no reply |
| Terminus per-node span materialize (rope tier) | flatten `core/src/op_resolve_view.cpp:258`, answered at `core/src/op_resolve_walk.hpp:871` / `core/src/op_resolve_walk.hpp:982` | a refusal on the reply's own route bytes ⇒ `BACKPRESSURE` on the error side ⇒ the frame is **dropped**; anywhere else before dispatch ⇒ an **addressed** `kind=ERROR STATUS{BACKPRESSURE}` reply |
| Terminus ownership flatten (rope tier, ADR-0053 ⑤, MULTI-link) | flatten `core/src/op_resolve_view.cpp:140`, answered by the empty-value guards in `resolve_node` (`core/src/op_resolve_walk.hpp:788-789`) | the write is **not stored** — the vertex keeps its previous value — and the reply is `BACKPRESSURE` |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:1791`, still an `empty()` test — that arm reads a
span through the make-contiguous seam) and bus-name (`core/src/fwd_router.cpp:1463-1465`, since
...
drops that one delivery (`core/src/fwd_router.cpp:2243-2244`). Dropping *invisibly* is the part that
needs an answer, which is why `graph_t::delivery_drops()` exists
(`core/include/libtracer/graph.hpp:1499`): four relaxed monotonic counters — `no_target`, `denied`,
`out_of_memory`, `fan_out_truncated` (`graph.hpp:1467-1489`) — incremented only on a drop, so the
...
still returns success (`core/src/graph.cpp:1401`), so it moves the counter by the fan-out width
rather than by one (#896).

A dropped fresh ADVERTISE on the COMPACT leg self-heals: the peer answers the unknown label with
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:2222`). Since #885 the router
...
(`fwd_router.hpp:267`, resolved at `fwd_router.hpp:1118-1118`) precisely so each transport's receive
