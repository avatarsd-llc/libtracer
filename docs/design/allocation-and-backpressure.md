...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:2002-2003` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:2005`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:1593` (the make-contiguous seam the ADVERTISE arm asks at `:1543`), answered at `:1550` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:1593` (the same seam, asked at `:1562`), answered at `:1569` | the delivery is **dropped**; the subscriber keeps its last-known value |
| Bus-name rejection reply flatten (cold) | flatten `core/src/fwd_router.cpp:1022`, answered by the `wire::decode` opening `reject_bus_name_hop` | the frame is **dropped** by value — no reply |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:1550`, still an `empty()` test — that arm reads a
span through the make-contiguous seam) and bus-name (`core/src/fwd_router.cpp:1242-1244`, since
...
drops that one delivery (`core/src/fwd_router.cpp:2002-2003`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:1981`). Since #885 the router
