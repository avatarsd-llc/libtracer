...
(`core/include/libtracer/fwd_router.hpp:181`), because the terminus arena decode belongs to the
router's receive thread rather than to the graph. It carries a **fourth** injection beside it, and
for a different contract: `flat` (`fwd_router.hpp:182`, documented at `:100`), the `mem_backend_t` **every rope flatten on
...
(`core/src/op_resolve_walk.hpp:549`) turns into an addressed `kind=ERROR BACKPRESSURE` rather than
...
(`core/src/graph.cpp:1714-1715`.) Three properties follow, and each closes a different failure mode:
...
- **A bounded node that injected `ctl` gets its own store here too.** The overflow leg draws from
  ...
  arena** (`graph.cpp:1711-1713`). Read that literally: it is a statement about the decode arena, not
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:1696-1701` | the refusal's cause (#917): `flatten_err_t::NO_MEMORY` → `BACKPRESSURE`, `NOT_HOST` → `TYPE_MISMATCH` |
| Field-write flatten into the value backend | `core/src/graph.cpp:2014-2020` | same two-verdict split as the branch-write row above |
| Branch-write root key render (`try_build_key`) | `core/src/graph.cpp:1733-1734` | `false` → `BACKPRESSURE` |
| Branch-write parse-key copy (`detail::try_assign`) | `core/src/graph.cpp:1739` | `false` → `BACKPRESSURE` |
| Branch-write decode arena | `core/src/graph.cpp:1714-1717` | decode error → `TYPE_MISMATCH` |
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:2679-2680` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:2682`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:2178` (the make-contiguous seam the ADVERTISE arm asks at `:2128`), answered at `:2135` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:2178` (the same seam, asked at `:2147`), answered at `:2154` | the delivery is **dropped**; the subscriber keeps its last-known value |
| Bus-name rejection reply flatten (cold) | flatten `core/src/fwd_router.cpp:1263`, answered by the `wire::decode` opening `reject_bus_name_hop` | the frame is **dropped** by value — no reply |
| Terminus per-node span materialize (rope tier) | flatten `core/src/op_resolve_view.cpp:258`, answered at `core/src/op_resolve_walk.hpp:881` / `core/src/op_resolve_walk.hpp:993` | a refusal on the reply's own route bytes ⇒ `BACKPRESSURE` on the error side ⇒ the frame is **dropped**; anywhere else before dispatch ⇒ an **addressed** `kind=ERROR STATUS{BACKPRESSURE}` reply |
| Terminus ownership flatten (rope tier, ADR-0053 ⑤, MULTI-link) | flatten `core/src/op_resolve_view.cpp:140`, answered by the empty-value guards in `resolve_node` (`core/src/op_resolve_walk.hpp:798-799`) | the write is **not stored** — the vertex keeps its previous value — and the reply is `BACKPRESSURE` |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:2135`, still an `empty()` test — that arm reads a
span through the make-contiguous seam) and bus-name (`core/src/fwd_router.cpp:1687-1689`, since
...
drops that one delivery (`core/src/fwd_router.cpp:2679-2680`). Dropping *invisibly* is the part that
...
still returns success (`core/src/graph.cpp:1631`), so it moves the counter by the fan-out width
rather than by one (#896).

A dropped fresh ADVERTISE on the COMPACT leg self-heals: the peer answers the unknown label with
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:2658`). Since #885 the router
...
(`fwd_router.hpp:332`, resolved at `fwd_router.hpp:1387-1387`) precisely so each transport's receive
