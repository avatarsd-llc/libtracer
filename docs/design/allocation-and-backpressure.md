...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:1955-1956` | the delivery is **dropped** |
| Per-delivery frame build | *deleted* (#885) — the COMPACT leg gathers off a stack head (`core/src/fwd_router.cpp:1958`) instead of building a frame | n/a: there is nothing left to refuse |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:1578` (the make-contiguous seam the ADVERTISE arm asks at `:1528`), answered at `:1535` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:1578` (the same seam, asked at `:1547`), answered at `:1554` | the delivery is **dropped**; the subscriber keeps its last-known value |
| Bus-name rejection reply flatten (cold) | flatten `core/src/fwd_router.cpp:1007`, answered by the `wire::decode` opening `reject_bus_name_hop` | the frame is **dropped** by value — no reply |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:1535`, still an `empty()` test — that arm reads a
span through the make-contiguous seam) and bus-name (`core/src/fwd_router.cpp:1227-1229`, since
...
drops that one delivery (`core/src/fwd_router.cpp:1955-1956`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:1934`). Since #885 the router
