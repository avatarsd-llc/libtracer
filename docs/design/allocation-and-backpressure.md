...
| Per-delivery COMPACT flatten (egress) | `core/src/fwd_router.cpp:1838-1839` | the delivery is **dropped** |
| Per-delivery frame build | `core/src/fwd_router.cpp:1843` | the delivery is **dropped** |
| Ingress `ADVERTISE` route flatten | flatten `core/src/fwd_router.cpp:1481` (the make-contiguous seam the ADVERTISE arm asks at `:1435`), answered at `:1442` | the empty flatten **fails the `wire::decode`** ⇒ the frame is **dropped**; the label stays **unbound** (the peer's COMPACTs draw a `HANDLE_NACK`) |
| Ingress `COMPACT` payload flatten | flatten `core/src/fwd_router.cpp:1481` (the same seam, asked at `:1454`), answered at `:1461` | the delivery is **dropped**; the subscriber keeps its last-known value |
...
beside the `ADVERTISE` (`core/src/fwd_router.cpp:1442`) and bus-name (`core/src/fwd_router.cpp:1141`) flattens are redundant with the `wire::decode`
...
drops that one delivery (`core/src/fwd_router.cpp:1838-1839`). Dropping *invisibly* is the part that
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:1821`).
