...
(`core/include/libtracer/fwd_router.hpp:93`), because the terminus arena decode belongs to the
router's receive thread rather than to the graph.
...
the router's `rx` and the transport-receive backend at the same underlying slab; the composition
guidance is in [`../reference/09-memory-substrate.md`](../reference/09-memory-substrate.md).
...
(`core/src/graph.cpp:1063-1064`.) Three properties follow, and each closes a different failure mode:
...
are enumerated in the changelog (`core/CHANGELOG.md:387`), which is the citation for that leg being
closed.
...
| --- | --- | --- |
| Branch-write flatten into the value backend | `core/src/graph.cpp:1050-1051` | empty head with a non-zero rope length → `BACKPRESSURE` |
| Field-write flatten into the value backend | `core/src/graph.cpp:1287-1288` | empty head with a non-zero rope length → `BACKPRESSURE` |
| Branch-write root key render (`try_build_key`) | `core/src/graph.cpp:1082-1083` | `false` → `BACKPRESSURE` |
| Branch-write parse-key copy (`detail::try_assign`) | `core/src/graph.cpp:1085` | `false` → `BACKPRESSURE` |
| Branch-write decode arena | `core/src/graph.cpp:1065-1067` | decode error → `TYPE_MISMATCH` |
...
(`core/include/libtracer/graph.hpp:775`): three relaxed monotonic counters — `no_target`, `denied`,
...
`HANDLE_NACK` and the next delivery re-advertises (`fwd_router.cpp:960-969`).
...
(`fwd_router.hpp:137`, resolved at `fwd_router.hpp:505-506`) precisely so each transport's receive
