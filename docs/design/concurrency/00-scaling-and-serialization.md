...
`core/include/libtracer/vertex.hpp:1191`), called from `core/src/graph.cpp:821`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1034`), which is the entire "writes
