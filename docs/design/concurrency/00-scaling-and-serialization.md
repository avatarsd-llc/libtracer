...
`core/include/libtracer/vertex.hpp:769`), called from `core/src/graph.cpp:1650`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`vertex.hpp:1545-1546`) no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1945`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1545-1546`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
