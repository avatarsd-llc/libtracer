...
element takes the lock (`op_resolve_walk.hpp:635` → `vertex_slot`) and honouring one takes it
again (`op_resolve_walk.hpp:963` and `fwd_router.cpp:1184-1190` → `deref_vertex_slot`), so a bound-path hop pays
