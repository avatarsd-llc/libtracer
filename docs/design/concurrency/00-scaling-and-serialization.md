...
`core/include/libtracer/vertex.hpp:1594`), called from `core/src/graph.cpp:1228`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:1175-1179`). The stripe is selected by
`vertex_stripe_of` (`:1257`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:1253`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2344-2345`) no
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:1235`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:1243`) | one predicted branch per control-plane verb |
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2344-2345`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
