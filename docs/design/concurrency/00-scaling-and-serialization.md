...
again (`op_resolve_walk.hpp:1073` and `fwd_router.cpp:1246-1252` → `deref_vertex_slot`), so a bound-path hop pays
...
`graph.hpp:696-703` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:729`), called from `core/src/graph.cpp:1360`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`vertex.hpp:1504-1505`) no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1632`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1504-1505`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
