...
element takes the lock (`op_resolve_walk.hpp:652` → `vertex_slot`) and honouring one takes it
again (`op_resolve_walk.hpp:1147` and `fwd_router.cpp:1377-1383` → `deref_vertex_slot`), so a bound-path hop pays
...
`graph.hpp:850-857` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:916`), called from `core/src/graph.cpp:1857`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`vertex.hpp:1813-1814`) no
...
The write path takes no map lock (`write_impl`, `graph.cpp:2200`), which is the entire "writes
...
(`.github/workflows/core-ci.yml:454-461`) — rebuilt with `find_ptr`'s `shared_lock` removed,
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1813-1814`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
