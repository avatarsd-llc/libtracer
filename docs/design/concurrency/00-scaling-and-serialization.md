...
`graph.hpp:877-884` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:982`), called from `core/src/graph.cpp:1941`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:2300`), which is the entire "writes
