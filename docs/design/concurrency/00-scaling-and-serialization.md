...
`core/include/libtracer/vertex.hpp:1083`), called from `core/src/graph.cpp:766`, and takes no
...
`vertex_stripe_of` (`:818`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:814`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `snapshot_edges` (`:1501`) takes one on **every delivery**; so do `add_edge`,
...
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:804`) | one predicted branch per control-plane verb |
...
`lkv_slot_t` is a compile-time policy (`core/include/libtracer/config.hpp:198`,
