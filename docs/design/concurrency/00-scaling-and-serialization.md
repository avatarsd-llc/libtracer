...
again (`op_resolve_walk.hpp:965` and `fwd_router.cpp:1166-1172` → `deref_vertex_slot`), so a bound-path hop pays
...
`core/include/libtracer/vertex.hpp:715`), called from `core/src/graph.cpp:1267`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`vertex.hpp:1474-1475`) no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1539`), which is the entire "writes
...
(`.github/workflows/core-ci.yml:153-164`) — rebuilt with `find_ptr`'s `shared_lock` removed,
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1474-1475`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
