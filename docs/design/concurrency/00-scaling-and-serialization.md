...
element takes the lock (`op_resolve_walk.hpp:639` → `vertex_slot`) and honouring one takes it
again (`op_resolve_walk.hpp:1070` and `fwd_router.cpp:1241-1247` → `deref_vertex_slot`), so a bound-path hop pays
...
`core/include/libtracer/vertex.hpp:721`), called from `core/src/graph.cpp:1312`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`vertex.hpp:1480-1481`) no
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1480-1481`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
