...
element takes the lock (`op_resolve_walk.hpp:633` → `vertex_slot`) and honouring one takes it
again (`op_resolve_walk.hpp:954` and `fwd_router.cpp:1162-1168` → `deref_vertex_slot`), so a bound-path hop pays
...
`graph.hpp:554-561` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:1481`), called from `core/src/graph.cpp:1176`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:1062-1066`). The stripe is selected by
`vertex_stripe_of` (`:1144`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:1140`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2231-2232`) no
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:1122`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:1130`) | one predicted branch per control-plane verb |
...
The write path takes no map lock (`write_impl`, `graph.cpp:1425`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2231-2232`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
