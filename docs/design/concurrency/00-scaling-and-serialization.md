...
`core/include/libtracer/vertex.hpp:715`), called from `core/src/graph.cpp:1243`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1515`), which is the entire "writes
