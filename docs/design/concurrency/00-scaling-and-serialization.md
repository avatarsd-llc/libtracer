...
element takes the lock (`op_resolve_walk.hpp:608` → `vertex_slot`) and honouring one takes it
again (`op_resolve_walk.hpp:887` and `fwd_router.cpp:1058` → `deref_vertex_slot`), so a bound-path hop pays
...
`graph.hpp:546-553` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:1460`), called from `core/src/graph.cpp:1078`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:1041-1045`). The stripe is selected by
`vertex_stripe_of` (`:1123`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:1119`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2182`) no
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:1101`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:1109`) | one predicted branch per control-plane verb |
...
The write path takes no map lock (`write_impl`, `graph.cpp:1321`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2182`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
