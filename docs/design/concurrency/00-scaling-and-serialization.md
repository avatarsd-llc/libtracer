...
`core/include/libtracer/vertex.hpp:1524`), called from `core/src/graph.cpp:1176`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:1105-1109`). The stripe is selected by
`vertex_stripe_of` (`:1187`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:1183`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2274-2275`) no
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:1165`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:1173`) | one predicted branch per control-plane verb |
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2274-2275`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
