...
`graph.hpp:582-589` states in the header. The hold is not incidental in either: one shared
...
`core/include/libtracer/vertex.hpp:1547`), called from `core/src/graph.cpp:1228`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:1128-1132`). The stripe is selected by
`vertex_stripe_of` (`:1210`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:1206`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2297-2298`) no
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:1188`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:1196`) | one predicted branch per control-plane verb |
...
The write path takes no map lock (`write_impl`, `graph.cpp:1486`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2297-2298`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
