...
again (`op_resolve_walk.hpp:954` and `fwd_router.cpp:1066` → `deref_vertex_slot`), so a bound-path hop pays
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2232`) no
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2232`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
