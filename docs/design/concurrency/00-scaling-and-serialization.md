...
`core/include/libtracer/vertex.hpp:916`), called from `core/src/graph.cpp:1724`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:2067`), which is the entire "writes
