...
`core/include/libtracer/vertex.hpp:1460`), called from `core/src/graph.cpp:979`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1214`), which is the entire "writes
