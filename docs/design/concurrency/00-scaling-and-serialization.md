...
again (`op_resolve_walk.hpp:887` and `fwd_router.cpp:929` → `deref_vertex_slot`), so a bound-path hop pays
...
`core/include/libtracer/vertex.hpp:1460`), called from `core/src/graph.cpp:993`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1236`), which is the entire "writes
