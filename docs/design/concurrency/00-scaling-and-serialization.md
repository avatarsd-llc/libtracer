...
`core/include/libtracer/vertex.hpp:1524`), called from `core/src/graph.cpp:1185`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:1443`), which is the entire "writes
