...
again (`op_resolve_walk.hpp:887` and `fwd_router.cpp:909` → `deref_vertex_slot`), so a bound-path hop pays
...
`core/include/libtracer/vertex.hpp:1452`), called from `core/src/graph.cpp:964`, and takes no
...
the ACL state. `add_edge`, `clear_edge` and `set_acl` take one; **`snapshot_edges` (`:2079`) no
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:2079`) is on the **delivery** path; `read` never calls it — and since ADR-0075 it takes no stripe lock at all | reading the call graph |
