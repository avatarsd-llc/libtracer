...
`core/include/libtracer/vertex.hpp:1055`), called from `core/src/graph.cpp:781`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:898`), which is the entire "writes
