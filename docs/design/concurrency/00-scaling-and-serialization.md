...
`core/include/libtracer/vertex.hpp:1159`), called from `core/src/graph.cpp:786`, and takes no
...
default 16, the sharing rationale at `vertex.hpp:770-774`). The stripe is selected by
`vertex_stripe_of` (`:852`) from the vertex address, hashed `(h >> 6) % kVertexLockStripes`
(`:848`). The stripes guard the fan-out edge list, the STREAM ring, the write-sequence bump and
the ACL state. `snapshot_edges` (`:1636`) takes one on **every delivery**; so do `add_edge`,
...
| platform | table | cost |
| --- | --- | --- |
| host libstdc++ / libc++ | `inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes` (`vertex.hpp:830`) | none — constant-initialized |
| a target without a constexpr `std::mutex` | guarded function-local `static` (`:838`) | one predicted branch per control-plane verb |
...
The write path takes no map lock (`write_impl`, `graph.cpp:999`), which is the entire "writes
...
| --- | --- | --- |
| The read-path residual is `snapshot_edges`' stripe lock | `snapshot_edges` (`vertex.hpp:1636`) is on the **delivery** path; `read` never calls it | reading the call graph |
