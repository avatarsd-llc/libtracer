...
`core/include/libtracer/vertex.hpp:982`), called from `core/src/graph.cpp:1959`, and takes no
...
The write path takes no map lock (`write_impl`, `graph.cpp:2318`), which is the entire "writes
