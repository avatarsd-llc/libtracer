...
`write_impl` (`core/src/graph.cpp:2233`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2367`) |
| store | `store_value` (`graph.cpp:2107`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2294-2316`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2724-2726`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1622-1623`), so the path overload
(`graph.cpp:4570`) pays one shared map hold that the handle overload does not.
...
(`graph.cpp:2306-2319`). A `HANDLER` has no such pointer — the user handler consumes the value
...
handed to `deliver_vertex` directly (`graph.cpp:2259-2292`). `try_clone_rope` (`:1890`) survives
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:2024`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1851`), which takes **no
...
and swaps it in (`vertex.hpp:2581`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:2081`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:2099`) | an empty local vector that never allocates unless the snapshot spills |
...
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:738`, `subscriber.hpp:667`). A warm wide publish reuses the
...
array** (`graph.cpp:2064-2065`, `vertex.hpp:2663`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2670-2685`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2478` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2437` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2443` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2449` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2450` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2474` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:2013-2022`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:2170`, entered only when `listeners_above() > 0`, `:2512`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1936-1944`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:2316
    std::uint64_t denied = 0;            // graph.hpp:2326
    std::uint64_t out_of_memory = 0;     // graph.hpp:2329
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2333
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1915-1918` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1932-1935` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2233-2278` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1944-1947` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2675` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1805`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:2084-2085`, `:2099-2100`) through
`count_snapshot_drops` (`:1567`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1532`), so a path here that abandons an
...
(`graph.hpp:2371`, `graph.cpp:1552`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2344`, `graph.cpp:1525`) is the only record that any of this
