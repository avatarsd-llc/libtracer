...
`write_impl` (`core/src/graph.cpp:2251`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2394`) |
| store | `store_value` (`graph.cpp:2142`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2321-2343`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2751-2753`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1657-1658`), so the path overload
(`graph.cpp:4597`) pays one shared map hold that the handle overload does not.
...
(`graph.cpp:2333-2346`). A `HANDLER` has no such pointer — the user handler consumes the value
...
handed to `deliver_vertex` directly (`graph.cpp:2286-2319`). `try_clone_rope` (`:1925`) survives
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:2059`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1863`), which takes **no
...
and swaps it in (`vertex.hpp:2603`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:2116`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:2134`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:2099-2100`, `vertex.hpp:2685`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2692-2707`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2477` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2436` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2442` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2448` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2449` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2473` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:2048-2057`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:2205`, entered only when `listeners_above() > 0`, `:2539`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1971-1979`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:2315
    std::uint64_t denied = 0;            // graph.hpp:2325
    std::uint64_t out_of_memory = 0;     // graph.hpp:2328
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2332
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1950-1953` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1967-1970` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2251-2305` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1979-1982` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2697` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1817`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:2119-2120`, `:2134-2135`) through
`count_snapshot_drops` (`:1602`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1567`), so a path here that abandons an
...
(`graph.hpp:2370`, `graph.cpp:1587`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2343`, `graph.cpp:1560`) is the only record that any of this
