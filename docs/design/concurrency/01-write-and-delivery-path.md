...
(`graph.cpp:2355`) pays one shared map hold that the handle overload does not.
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1096` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1076` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1082` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1088` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1089` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1092` | vertical fan-out to every registered ancestor's subscribers |
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;      // graph.hpp:1029
    std::uint64_t denied = 0;         // graph.hpp:1031
    std::uint64_t out_of_memory = 0;  // graph.hpp:1033
};
```
...
`delivery_drops()` (`graph.hpp:1044`, `graph.cpp:634`) is the only record that any of this
