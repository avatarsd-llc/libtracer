...
(`graph.cpp:2495`) pays one shared map hold that the handle overload does not.
...
array** (`graph.cpp:995-996`, `vertex.hpp:2732`), so a subscriber added between the count and
