...
`write_impl` (`core/src/graph.cpp:1484`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1584`) |
| store | `store_value` (`graph.cpp:1422`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1535-1555`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1907-1909`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1036-1037`), so the path overload
(`graph.cpp:3256`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1503-1533`, `try_clone_rope` at `:1245`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1545-1555`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1346`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1459`), which takes **no
...
and swaps it in (`vertex.hpp:2141`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1396`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1414`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1379-1380`, `vertex.hpp:2205`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2211-2226`).
...
`dispatch_edge` is `always_inline` (`graph.cpp:1335-1344`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1451`, entered only when `listeners_above() > 0`, `:1723`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1284-1292`).
...
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1273-1276` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1280-1283` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:1484-1522` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1283-1286` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1531` |
| `out_of_memory` | an edge's owning copies (link NAME / stored caller) could not be allocated, so the snapshot skipped it | `vertex.hpp:2222` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2217` |
...
snapshot sheds are tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1409`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1399-1400`, `:1414-1415`) through
...
(`graph.hpp:1603`, `graph.cpp:979`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:1576`, `graph.cpp:952`) is the only record that any of this
