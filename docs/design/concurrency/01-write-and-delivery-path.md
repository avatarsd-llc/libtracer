...
`write_impl` (`core/src/graph.cpp:2393`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2546`) |
| store | `store_value` (`graph.cpp:2250`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2471-2493`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2962-2964`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1755-1756`), so the path overload
(`graph.cpp:4845`) pays one shared map hold that the handle overload does not.
...
(`graph.cpp:2483-2496`). A `HANDLER` has no such pointer — the user handler consumes the value
...
handed to `deliver_vertex` directly (`graph.cpp:2436-2469`). `try_clone_rope` (`:2023`) survives
...
(#1477) — the doctrine is stated at the handle overload (`graph.cpp:2938`). A map lock on the
...
(`core/include/libtracer/vertex.hpp:3047`). `write_impl` snapshots the role **once** per frame
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:2167`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1971`), which takes **no
...
and swaps it in (`vertex.hpp:2724`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:2224`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:2242`) | an empty local vector that never allocates unless the snapshot spills |
...
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:807`, `subscriber.hpp:667`). A warm wide publish reuses the
...
array** (`graph.cpp:2207-2208`, `vertex.hpp:2806`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2813-2828`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2568` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2527` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2533` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2539` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2540` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2564` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:2156-2165`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:2347`, entered only when `listeners_above() > 0`, `:2723`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:2069-2077`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:3355`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:3465-3472`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:3462-3466`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:3486-3488`). Its entry
...
where the growth **throws** (`core/include/libtracer/mem_heap.hpp:169-183`): under
...
(`fwd_router.cpp:3393`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:3394`) — since #917 that is a
...
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:2048-2051` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:2065-2068` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2393-2447` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:2077-2080` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2818` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1925`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:2227-2228`, `:2242-2243`) through
`count_snapshot_drops` (`:1700`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1665`), so a path here that abandons an
...
(`graph.hpp:2459`, `graph.cpp:1685`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2432`, `graph.cpp:1658`) is the only record that any of this
...
(`core/tests/registry_teardown_test.cpp:351`), because a disagreement would throw nothing and
