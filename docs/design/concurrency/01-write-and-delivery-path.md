...
`write_impl` (`core/src/graph.cpp:1199`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1271`) |
| store | `store_value` (`graph.cpp:1145`) — LKV or history per role, sequence bump, `await` wake |
...
runs and only the subscriber delivery drops (`graph.cpp:1208-1234`, `try_clone_rope` at `:989`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1244-1248`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1069`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:2070`), which takes **no
...
and swaps it in (`vertex.hpp:2766`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1119`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1137`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1102-1103`, `vertex.hpp:2829`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2835-2850`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1430` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1396` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1402` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1408` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1409` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1426` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is defined inline (`graph.cpp:1059-1066`) precisely so its body stays in the
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1028-1036`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:1908`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:1967-1974`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:1964-1968`). The route bytes were copied once at subscribe
...
(`fwd_router.cpp:1980-1981`). That helper runs the *throwing* `std::vector::reserve` through
...
(`fwd_router.cpp:1946`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:1947`) — since #917 that is a
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1334
    std::uint64_t denied = 0;            // graph.hpp:1336
    std::uint64_t out_of_memory = 0;     // graph.hpp:1339
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1343
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1017-1020` |
| `denied` | the target's `:acl` denied WRITE to the **edge's stored caller**, not the writer's | `graph.cpp:1024-1027` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1036-1039` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1232` |
| `out_of_memory` | an edge's owning copies (link NAME / stored caller) could not be allocated, so the snapshot skipped it | `vertex.hpp:2846` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2841` |
...
snapshot sheds are tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:2020`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1123`, `:1138`) through
`count_snapshot_drops` (`:749`) rather than counted as one at the caller. Every site **on
...
`delivery_drops()` (`graph.hpp:1354`, `graph.cpp:722`) is the only record that any of this
