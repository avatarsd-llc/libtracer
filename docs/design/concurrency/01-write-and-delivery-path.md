...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2929`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:3030-3037`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:3027-3031`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:3051-3052`). Its entry
...
(`fwd_router.cpp:2967`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:2968`) — since #917 that is a
...
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:2836`), or its binding vanished under a concurrent unbind (`:2731`) | `fwd_router.cpp:2731`, `:2836` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:2682-2684`, `:2693`, `:2843` |
