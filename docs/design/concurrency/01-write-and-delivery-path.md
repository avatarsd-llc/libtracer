...
`write_impl` (`core/src/graph.cpp:1354`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1454`) |
| store | `store_value` (`graph.cpp:1298`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1405-1425`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1777-1779`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:918-919`), so the path overload
(`graph.cpp:3052`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1373-1403`, `try_clone_rope` at `:1130`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1415-1425`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1222`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:2223`), which takes **no
lock at all** (#635): it copies the vertex's PUBLISHED, immutable-after-publish edge array
(`vertex.hpp:943`) into one of two buffers under a bounded per-participant **edge pin**
...
and swaps it in (`vertex.hpp:2901`); the displaced array is scanned and freed on the mutator's
own thread, outside the lock, once no participant announces it (`vertex.hpp:1041`).
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1272`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1290`) | an empty local vector that never allocates unless the snapshot spills |
...
(`vertex.hpp:846-884`). Its width is `kInlineFanout` — the no-heap small-fan-out snapshot width,
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:1297`, `:849`). A warm wide publish reuses the
...
array** (`graph.cpp:1255-1256`, `vertex.hpp:2965`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2971-2986`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1616` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1575` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1581` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1587` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1588` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1612` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:1211-1220`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1321`, entered only when `listeners_above() > 0`, `:1593`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1169-1177`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1471
    std::uint64_t denied = 0;            // graph.hpp:1481
    std::uint64_t out_of_memory = 0;     // graph.hpp:1484
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1488
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1158-1161` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1165-1168` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:1354-1392` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1177-1180` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1401` |
...
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2977` |
...
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1276`, `:1291`) through
`count_snapshot_drops` (`:876`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:841`), so a path here that abandons an
...
(`graph.hpp:1526`, `graph.cpp:861`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:1499`, `graph.cpp:834`) is the only record that any of this
