...
`write_impl` (`core/src/graph.cpp:1425`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1525`) |
| store | `store_value` (`graph.cpp:1369`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1476-1496`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1848-1850`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:989-990`), so the path overload
(`graph.cpp:3135`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1444-1474`, `try_clone_rope` at `:1201`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1486-1496`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1293`) does that
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1343`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1361`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1326-1327`, `vertex.hpp:3012`), so a subscriber added between the count and
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1637` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1596` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1602` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1608` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1609` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1633` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:1282-1291`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1392`, entered only when `listeners_above() > 0`, `:1664`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1240-1248`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1492
    std::uint64_t denied = 0;            // graph.hpp:1502
    std::uint64_t out_of_memory = 0;     // graph.hpp:1505
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1509
};
```
...
(`graph.hpp:1547`, `graph.cpp:932`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:1520`, `graph.cpp:905`) is the only record that any of this
