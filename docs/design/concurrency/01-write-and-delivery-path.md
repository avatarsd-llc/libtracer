...
`write_impl` (`core/src/graph.cpp:1945`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2045`) |
| store | `store_value` (`graph.cpp:1883`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1996-2016`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2378-2380`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1460-1461`), so the path overload
(`graph.cpp:3946`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1964-1994`, `try_clone_rope` at `:1683`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:2006-2016`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1800`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1538`), which takes **no
...
and swaps it in (`vertex.hpp:2221`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1857`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1875`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1840-1841`, `vertex.hpp:2303`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2310-2325`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2179` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2138` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2144` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2150` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2151` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2175` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:1789-1798`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1912`, entered only when `listeners_above() > 0`, `:2190`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1729-1737`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:2021
    std::uint64_t denied = 0;            // graph.hpp:2031
    std::uint64_t out_of_memory = 0;     // graph.hpp:2034
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2038
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1708-1711` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1725-1728` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:1945-1983` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1737-1740` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1992` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2315` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1492`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1860-1861`, `:1875-1876`) through
`count_snapshot_drops` (`:1415`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1380`), so a path here that abandons an
...
(`graph.hpp:2076`, `graph.cpp:1400`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2049`, `graph.cpp:1373`) is the only record that any of this
