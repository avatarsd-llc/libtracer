...
`write_impl` (`core/src/graph.cpp:2029`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2131`) |
| store | `store_value` (`graph.cpp:1933`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2080-2102`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2464-2466`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1471-1472`), so the path overload
(`graph.cpp:4033`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:2048-2078`, `try_clone_rope` at `:1719`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:2089-2102`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1850`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1764`), which takes **no
...
and swaps it in (`vertex.hpp:2490`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1907`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1925`) | an empty local vector that never allocates unless the snapshot spills |
...
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:652`, `subscriber.hpp:619`). A warm wide publish reuses the
...
array** (`graph.cpp:1890-1891`, `vertex.hpp:2572`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2579-2594`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2256` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2215` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2221` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2227` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2228` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2252` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:1839-1848`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1996`, entered only when `listeners_above() > 0`, `:2276`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1765-1773`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2997`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:3098-3105`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:3095-3099`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:3119-3120`). Its entry
...
(`fwd_router.cpp:3035`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:3036`) — since #917 that is a
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:2094
    std::uint64_t denied = 0;            // graph.hpp:2104
    std::uint64_t out_of_memory = 0;     // graph.hpp:2107
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2111
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1744-1747` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1761-1764` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2029-2067` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1773-1776` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:2076` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2584` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1718`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1910-1911`, `:1925-1926`) through
`count_snapshot_drops` (`:1416`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1381`), so a path here that abandons an
...
(`graph.hpp:2149`, `graph.cpp:1401`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2122`, `graph.cpp:1374`) is the only record that any of this
