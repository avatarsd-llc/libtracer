...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:1876`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:1931-1938`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:1928-1932`). The route bytes were copied once at subscribe
...
(`fwd_router.cpp:1944-1945`). That helper runs the *throwing* `std::vector::reserve` through
...
(`fwd_router.cpp:1910`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A failed flatten drops the delivery (`fwd_router.cpp:1911`).
