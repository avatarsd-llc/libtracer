...
`write_impl` (`core/src/graph.cpp:2067`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2191`) |
| store | `store_value` (`graph.cpp:1971`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2118-2140`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2548-2550`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1489-1490`), so the path overload
(`graph.cpp:4343`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:2086-2116`, `try_clone_rope` at `:1757`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:2127-2140`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1888`) does that
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1945`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1963`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1928-1929`, `vertex.hpp:2618`), so a subscriber added between the count and
...
`dispatch_edge` is `always_inline` (`graph.cpp:1877-1886`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:2034`, entered only when `listeners_above() > 0`, `:2336`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1803-1811`).
...
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1782-1785` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1799-1802` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2067-2105` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1811-1814` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:2114` |
...
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1948-1949`, `:1963-1964`) through
...
(`graph.hpp:2264`, `graph.cpp:1419`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2237`, `graph.cpp:1392`) is the only record that any of this
