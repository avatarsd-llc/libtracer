...
`write_impl` (`core/src/graph.cpp:2047`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2149`) |
| store | `store_value` (`graph.cpp:1951`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2098-2120`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2495-2497`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1489-1490`), so the path overload
(`graph.cpp:4278`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:2066-2096`, `try_clone_rope` at `:1737`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:2107-2120`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1868`) does that
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1925`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1943`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1908-1909`, `vertex.hpp:2618`), so a subscriber added between the count and
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2344` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2303` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2309` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2315` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:2316` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:2340` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is `always_inline` (`graph.cpp:1857-1866`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:2014`, entered only when `listeners_above() > 0`, `:2294`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1783-1791`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:3047`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:3148-3155`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:3145-3149`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:3169-3170`). Its entry
...
(`fwd_router.cpp:3085`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:3086`) — since #917 that is a
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:2182
    std::uint64_t denied = 0;            // graph.hpp:2192
    std::uint64_t out_of_memory = 0;     // graph.hpp:2195
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2199
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1762-1765` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1779-1782` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2047-2085` |
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:2947`), or its binding vanished under a concurrent unbind (`:2842`) | `fwd_router.cpp:2842`, `:2947` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:2793-2795`, `:2804`, `:2954` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1791-1794` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:2094` |
...
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1928-1929`, `:1943-1944`) through
`count_snapshot_drops` (`:1434`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:1399`), so a path here that abandons an
...
(`graph.hpp:2237`, `graph.cpp:1419`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2210`, `graph.cpp:1392`) is the only record that any of this
