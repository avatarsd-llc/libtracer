...
`write_impl` (`core/src/graph.cpp:1198`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1264`) |
| store | `store_value` (`graph.cpp:1144`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1235-1247`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1508-1510`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:776-777`), so the path overload
(`graph.cpp:2772`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1207-1233`, `try_clone_rope` at `:988`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1243-1247`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1068`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:2065`), which takes **no
lock at all** (#635): it copies the vertex's PUBLISHED, immutable-after-publish edge array
(`vertex.hpp:914`) into one of two buffers under a bounded per-participant **edge pin**
...
and swaps it in (`vertex.hpp:2761`); the displaced array is scanned and freed on the mutator's
own thread, outside the lock, once no participant announces it (`vertex.hpp:1012`).
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1118`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1136`) | an empty local vector that never allocates unless the snapshot spills |
...
(`vertex.hpp:819-857`). Its width is `kInlineFanout` — the no-heap small-fan-out snapshot width,
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:1268`, `:822`). A warm wide publish reuses the
...
array** (`graph.cpp:1101-1102`, `vertex.hpp:2817`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2823-2838`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1416` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1382` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1388` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1394` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1395` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1412` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is defined inline (`graph.cpp:1058-1065`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1165`, entered only when `listeners_above() > 0`, `:1381`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1027-1035`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:1895`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:1950-1957`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:1947-1951`). The route bytes were copied once at subscribe
...
(`fwd_router.cpp:1963-1964`). That helper runs the *throwing* `std::vector::reserve` through
...
(`fwd_router.cpp:1929`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A failed flatten drops the delivery (`fwd_router.cpp:1930`).
...
more sheds happen further up, before an edge is ever dispatched. What is counted is the drop:

```cpp
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1320
    std::uint64_t denied = 0;            // graph.hpp:1322
    std::uint64_t out_of_memory = 0;     // graph.hpp:1325
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1329
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1016-1019` |
| `denied` | the target's `:acl` denied WRITE to the **edge's stored caller**, not the writer's | `graph.cpp:1023-1026` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1035-1038` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1230` |
| `out_of_memory` | an edge's owning copies (link NAME / stored caller) could not be allocated, so the snapshot skipped it | `vertex.hpp:2834` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2829` |
...
snapshot sheds are tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:2015`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1122`, `:1137`) through
`count_snapshot_drops` (`:748`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:728`), so a path here that abandons an
...
`delivery_drops()` (`graph.hpp:1340`, `graph.cpp:721`) is the only record that any of this
