...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2641`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:2742-2749`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:2739-2743`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:2763-2764`). Its entry
...
(`fwd_router.cpp:2679`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:2680`) — since #917 that is a
...
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:2548`), or its binding vanished under a concurrent unbind (`:2458`) | `fwd_router.cpp:2458`, `:2548` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:2428-2430`, `:2439`, `:2555` |
