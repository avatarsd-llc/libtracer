...
`write_impl` (`core/src/graph.cpp:1297`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1397`) |
| store | `store_value` (`graph.cpp:1241`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1348-1368`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1720-1722`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:867-868`), so the path overload
(`graph.cpp:2991`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1316-1346`, `try_clone_rope` at `:1079`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1358-1368`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1165`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:2119`), which takes **no
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1215`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1233`) | an empty local vector that never allocates unless the snapshot spills |
...
array** (`graph.cpp:1198-1199`, `vertex.hpp:2859`), so a subscriber added between the count and
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1550` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1509` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1515` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1521` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1522` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1546` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is defined inline (`graph.cpp:1155-1162`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1264`, entered only when `listeners_above() > 0`, `:1536`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1118-1126`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2010`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:2069-2076`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:2066-2070`). The route bytes were copied once at subscribe
...
(`fwd_router.cpp:2082-2083`). That helper runs the *throwing* `std::vector::reserve` through
...
(`fwd_router.cpp:2048`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:2049`) — since #917 that is a
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1405
    std::uint64_t denied = 0;            // graph.hpp:1415
    std::uint64_t out_of_memory = 0;     // graph.hpp:1418
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1422
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1107-1110` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1114-1117` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:1297-1335` |
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:1935`), or its binding vanished under a concurrent unbind (`:1846`) | `fwd_router.cpp:1846`, `:1935` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:1820`, `:1825`, `:1942` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1126-1129` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1344` |
...
snapshot sheds are tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:2069`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1219`, `:1234`) through
`count_snapshot_drops` (`:825`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:790`), so a path here that abandons an
...
(`graph.hpp:1460`, `graph.cpp:810`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:1433`, `graph.cpp:783`) is the only record that any of this
