...
(`subscriber.hpp:702`) into one of two buffers under a bounded per-participant **edge pin**
...
own thread, outside the lock, once no participant announces it (`subscriber.hpp:805`).
...
(`subscriber.hpp:613-651`). Its width is `kInlineFanout` — the no-heap small-fan-out snapshot width,
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:546`, `subscriber.hpp:616`). A warm wide publish reuses the
...
The snapshot reaches an allocator in exactly ONE place, and it is nothrow: an unreservable
overflow vector degrades the snapshot to the first `kInlineFanout` views and drops the rest of
that delivery (`vertex.hpp:2269-2277`). It cannot abort. The **per-edge** copy has no allocation
at all since #1448 — it is two pointer copies and two refcounts on every edge shape, the cold
half being shared rather than reproduced — so the second shed this paragraph used to describe,
an edge whose owning link / caller copies could not be cloned, no longer exists. A thread that cannot claim a pin cell — more concurrent publishers than
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:2147` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:2106` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:2112` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:2118` | the local re-dispatch leg — a delivery into another vertex |
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1989
    std::uint64_t denied = 0;            // graph.hpp:1999
    std::uint64_t out_of_memory = 0;     // graph.hpp:2002
    std::uint64_t fan_out_truncated = 0; // graph.hpp:2006
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1699-1700` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1713-1714` |
...
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1725-1726` |
...
The last two here, plus a third that no longer exists, were **uncounted until #896**, and the
handler one is why that mattered: it sheds every subscriber of the vertex and still returns
success, so `delivery_drops()` — the only thing that could say so — read zero while a whole
fan-out evaporated. The unit of every counter is therefore a **delivery**, not an event: a shed
fan-out of N counts N, which is why the snapshot's shed is tallied inside
`vertex_t::snapshot_edges` (`vertex.hpp:1458`, its `snapshot_drops_t`) and folded by `fan_out`
(`graph.cpp:1848-1849`, `:1863-1864`) through `count_snapshot_drops` (`:1403`) rather than
counted as one at the caller.

**The third snapshot shed is gone, and its removal is a fix rather than a gap** (#1448). A
per-edge `out_of_memory` used to count the edges whose owning link / caller copies the snapshot
could not allocate. `edge_view_t` no longer copies them — it takes a refcount share of the
immutable cold half — so the per-edge copy reaches no allocator on any edge shape and cannot
fail; a delivery that used to be shed under memory pressure now lands. `snapshot_drops_t`
carries only `truncated`. Nothing else changed: `out_of_memory` is still a live DELIVERY cause,
counted from every leg above that can still hit it. Every site **on
...
(`graph.hpp:2044`, `graph.cpp:1388`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2017`, `graph.cpp:1361`) is the only record that any of this
