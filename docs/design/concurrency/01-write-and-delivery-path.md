...
`write_impl` (`core/src/graph.cpp:1321`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1421`) |
| store | `store_value` (`graph.cpp:1265`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1372-1392`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1744-1746`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:891-892`), so the path overload
(`graph.cpp:3015`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1340-1370`, `try_clone_rope` at `:1103`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1382-1392`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:1189`) does that
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:2173`), which takes **no
lock at all** (#635): it copies the vertex's PUBLISHED, immutable-after-publish edge array
(`vertex.hpp:922`) into one of two buffers under a bounded per-participant **edge pin**
...
and swaps it in (`vertex.hpp:2850`); the displaced array is scanned and freed on the mutator's
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf, drops)` (`graph.cpp:1239`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf, drops)` (`graph.cpp:1257`) | an empty local vector that never allocates unless the snapshot spills |
...
(`vertex.hpp:827-865`). Its width is `kInlineFanout` — the no-heap small-fan-out snapshot width,
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:1276`, `:830`). A warm wide publish reuses the
...
array** (`graph.cpp:1222-1223`, `vertex.hpp:2913`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2919-2934`).
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1576` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1535` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1541` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1547` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1548` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1572` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is defined inline (`graph.cpp:1179-1186`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:1288`, entered only when `listeners_above() > 0`, `:1560`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:1142-1150`).
...
`kMaxDispatchDepth` is deleted with nothing replacing it (`graph.hpp:84-89`). An application that
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2107`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:2166-2173`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:2163-2167`). The route bytes were copied once at subscribe
...
(`fwd_router.cpp:2179-2180`). That helper runs the *throwing* `std::vector::reserve` through
...
(`fwd_router.cpp:2145`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:2146`) — since #917 that is a
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;         // graph.hpp:1431
    std::uint64_t denied = 0;            // graph.hpp:1441
    std::uint64_t out_of_memory = 0;     // graph.hpp:1444
    std::uint64_t fan_out_truncated = 0; // graph.hpp:1448
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:1131-1134` |
| `denied` | a subscription edge's delivery was refused by the target's `:acl`, gated on the **edge's stored caller**, not the writer's | `graph.cpp:1138-1141` |
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:1321-1359` |
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:2032`), or its binding vanished under a concurrent unbind (`:1943`) | `fwd_router.cpp:1943`, `:2032` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:1917`, `:1922`, `:2039` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:1150-1153` |
| `out_of_memory` | a HANDLER write's notify clone failed — the WHOLE fan-out is shed, one count per subscriber | `graph.cpp:1368` |
| `out_of_memory` | an edge's owning copies (link NAME / stored caller) could not be allocated, so the snapshot skipped it | `vertex.hpp:2930` |
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2925` |
...
snapshot sheds are tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:2123`, its
`snapshot_drops_t`) and folded by `fan_out` (`graph.cpp:1243`, `:1258`) through
`count_snapshot_drops` (`:849`) rather than counted as one at the caller. Every site **on
this plane** goes through one door, `count_drop` (`:814`), so a path here that abandons an
...
(`graph.hpp:1486`, `graph.cpp:834`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:1459`, `graph.cpp:807`) is the only record that any of this
