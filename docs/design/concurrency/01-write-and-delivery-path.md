...
`write_impl` (`core/src/graph.cpp:1044`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1095`) |
| store | `store_value` (`graph.cpp:990`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1066-1078`) |
...
(`graph.cpp:2312`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1053-1064`, `try_clone_rope` at `:856`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1074-1078`).
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf)` (`graph.cpp:970`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf)` (`graph.cpp:983`) | an empty local vector that never allocates unless the snapshot spills |
...
(`graph.cpp:954-955`, `vertex.hpp:1641-1672`), so a subscriber added between the count and the lock costs
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1086` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1066` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1072` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1078` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1079` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1082` | vertical fan-out to every registered ancestor's subscribers |
...
`bubble_up` (`graph.cpp:1011`, entered only when `listeners_above() > 0`, `:1212`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:880-888`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:1660`) is the sink the remote leg calls.
...
runtime construction (`fwd_router.cpp:1710-1714`). The route bytes were copied once at subscribe
...
rather than emitting a truncated frame (`fwd_router.cpp:1726-1727`).

**The COMPACT leg is the one that flattens.** `const view_t flat = value.materialize(*flat_);`
(`fwd_router.cpp:1689`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A failed flatten drops the delivery (`fwd_router.cpp:1690`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;      // graph.hpp:1019
    std::uint64_t denied = 0;         // graph.hpp:1021
    std::uint64_t out_of_memory = 0;  // graph.hpp:1023
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:869-872` |
...
`delivery_drops()` (`graph.hpp:1034`, `graph.cpp:634`) is the only record that any of this
