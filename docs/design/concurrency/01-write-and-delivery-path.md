...
`write_impl` (`core/src/graph.cpp:1019`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:1070`) |
| store | `store_value` (`graph.cpp:965`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:1041-1053`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:1305-1307`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:588`), so the path overload
(`graph.cpp:2287`) pays one shared map hold that the handle overload does not.
...
runs and only the subscriber delivery drops (`graph.cpp:1028-1039`, `try_clone_rope` at `:831`).
Every other role delivers **the exact pointer `store_value` handed back**, so the hot write path
reclones nothing (`graph.cpp:1049-1053`).
...
graph. The edge list therefore has to be copied out first. `fan_out` (`graph.cpp:896`) does that
...
| --- | --- | --- |
| wide, `own_subs() > kInlineFanout` | `v->snapshot_edges(inline_buf, tls_buf)` (`graph.cpp:945`) | a persistent `thread_local` vector, cleared but keeping capacity |
| small, or a nested wide fan-out | `v->snapshot_edges(inline_buf, heap_buf)` (`graph.cpp:958`) | an empty local vector that never allocates unless the snapshot spills |
...
(`graph.cpp:929-930`, `vertex.hpp:1641-1672`), so a subscriber added between the count and the lock costs
...
| --- | --- | --- |
| `deliver_vertex` | `graph.hpp:1046` | the per-vertex delivery unit both `write` and `propagate` build on: `fan_out`, then `bubble_up` if anyone listens above |
| `fan_out` | `graph.hpp:1026` | return at once if nothing subscribes here; else snapshot under the stripe lock, then `dispatch_edge` per view, outside it |
| `dispatch_edge` | `graph.hpp:1032` | the one dispatch of an edge's three legs, shared by `fan_out` and the admission durability latch so the legs cannot diverge |
| `dispatch_edge_target` | `graph.hpp:1038` | the local re-dispatch leg — a delivery into another vertex |
| `dispatch_edge_remote` | `graph.hpp:1039` | the remote leg — a `FWD{WRITE}` through the injected sink |
| `bubble_up` | `graph.hpp:1042` | vertical fan-out to every registered ancestor's subscribers |

`dispatch_edge` is defined inline (`graph.cpp:886-893`) precisely so its body stays in the
...
`bubble_up` (`graph.cpp:986`, entered only when `listeners_above() > 0`, `:1187`) walks parent pointers, which
...
re-dispatches to the target's own `:subscribers[]` and never bubbles (`graph.cpp:855-863`).
...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:1444`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:1509-1516`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:1494-1498`). The route bytes were copied once at subscribe
...
rather than emitting a truncated frame (`fwd_router.cpp:1510-1511`).

**The COMPACT leg is the one that flattens.** `const view_t flat = value.materialize(*flat_);`
(`fwd_router.cpp:1473`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A failed flatten drops the delivery (`fwd_router.cpp:1474`).
...
struct delivery_drops_t {
    std::uint64_t no_target = 0;      // graph.hpp:979
    std::uint64_t denied = 0;         // graph.hpp:981
    std::uint64_t out_of_memory = 0;  // graph.hpp:983
};
```

| counter | condition | site |
| --- | --- | --- |
| `no_target` | the target PATH resolved to no live vertex — retired, or never created | `graph.cpp:844-847` |
| `denied` | the target's `:acl` denied WRITE to the **edge's stored caller**, not the writer's | `graph.cpp:851-854` |
| `out_of_memory` | the nothrow delivery clone could not be allocated | `graph.cpp:863-866` |

`delivery_drops()` (`graph.hpp:994`, `graph.cpp:609`) is the only record that any of this
