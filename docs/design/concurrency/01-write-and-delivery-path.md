...
`write_impl` (`core/src/graph.cpp:2203`) is the one body behind every write overload. It takes
**no map lock**. The stages, in order:

| stage | what it does |
| --- | --- |
| ACL gate | `acl_allows(v, caller, WRITE)`; a denial returns `PERMISSION_DENIED` and stores nothing |
| branch fork | a POINT payload with the branch bit set decomposes through `write_branch` (`graph.cpp:2330`) |
| store | `store_value` (`graph.cpp:2107`) — LKV or history per role, sequence bump, `await` wake |
| deliver | `deliver_vertex` for a leaf value, `deliver_current` for a `STREAM` (`graph.cpp:2257-2279`) |

The handle overload is the whole of `write(vertex_handle_t, rope_t, caller)`
(`graph.cpp:2687-2689`) — a call straight into `write_impl`. A **path**-addressed write resolves
first, and `find_ptr` takes `map_mutex_` in shared mode (`graph.cpp:1622-1623`), so the path overload
(`graph.cpp:4533`) pays one shared map hold that the handle overload does not.
...
(`graph.cpp:2269-2282`). A `HANDLER` has no such pointer — the user handler consumes the value
...
handed to `deliver_vertex` directly (`graph.cpp:2222-2255`). `try_clone_rope` (`:1890`) survives
...
in one call to `snapshot_edges` (`core/include/libtracer/vertex.hpp:1809`), which takes **no
...
and swaps it in (`vertex.hpp:2539`); the displaced array is scanned and freed on the mutator's
...
`edge_snapshot_t::kCapacity`, 8 (`vertex.hpp:696`, `subscriber.hpp:667`). A warm wide publish reuses the
...
array** (`graph.cpp:2064-2065`, `vertex.hpp:2621`), so a subscriber added between the count and
...
owning copies cannot be cloned is skipped, dropping that one delivery (`vertex.hpp:2628-2643`).
...
`bubble_up` (`graph.cpp:2170`, entered only when `listeners_above() > 0`, `:2475`) walks parent pointers, which
...
| `denied` | a WRITE was refused at the graph's own gate — the API write, the `FWD{WRITE}` terminus and both `COMPACT` terminus arms enter through it | `graph.cpp:2203-2241` |
...
| `fan_out_truncated` | the wide-fan-out overflow buffer could not be reserved, so every edge past the inline prefix was abandoned | `vertex.hpp:2633` |
...
remaining snapshot shed is tallied inside `vertex_t::snapshot_edges` (`vertex.hpp:1763`, its
...
(`graph.hpp:2370`, `graph.cpp:1552`), which maps its two causes onto `count_drop` and adds no
...
`delivery_drops()` (`graph.hpp:2343`, `graph.cpp:1525`) is the only record that any of this
