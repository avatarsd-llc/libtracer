...
`fwd_router_t::deliver_remote` (`core/src/fwd_router.cpp:2700`) is the sink the remote leg calls.
...
rope link (`fwd_router.cpp:2801-2808`). The header is a `stack_writer<16>` — the FWD header of at
most 6 bytes plus the 5-byte op TLV — and both constant TLVs are `constexpr` arrays with no
runtime construction (`fwd_router.cpp:2798-2802`). The route bytes were copied once at subscribe
...
drops that delivery rather than emitting a truncated frame (`fwd_router.cpp:2822-2823`). Its entry
...
(`fwd_router.cpp:2738`) precedes the compact encode, because a COMPACT wraps a contiguous
...
global heap. A REFUSED flatten drops the delivery (`fwd_router.cpp:2739`) — since #917 that is a
...
| `no_target` | a net-plane route resolved to no vertex (`fwd_router.cpp:2607`), or its binding vanished under a concurrent unbind (`:2517`) | `fwd_router.cpp:2517`, `:2607` |
| `out_of_memory` | a `COMPACT` terminus could not take the payload view or reserve its rope | `fwd_router.cpp:2487-2489`, `:2498`, `:2614` |
