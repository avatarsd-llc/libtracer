...
- **`sizeof(vertex_t)` is gated in the header, not in a test.** The ceilings are `config_t`
  ...
  (`core/include/libtracer/vertex.hpp:3411,3416`), so every build on every target checks its
  ...
  assertion of a different kind: `alignof(vertex_stripe_t) == kStripeAlign` (`vertex.hpp:1219`),
