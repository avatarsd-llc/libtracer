...
(`core/include/libtracer/graph.hpp:1539`) stashes one LKV per node into a vector that outlives
...
- **`sizeof(vertex_t)` is gated in the header, not in a test.** The ceilings are `config_t`
  ...
  (`core/include/libtracer/vertex.hpp:2995,3000`), so every build on every target checks its
