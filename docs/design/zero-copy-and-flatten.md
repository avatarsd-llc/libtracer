...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1921`, materialize at `:1959`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1561`, sub-rope materialize at `:1576` and `:1545`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1561`, `:1512`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1566`, `:1545`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:1197`, the check at `:1201-1206`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1250-1254`). The contract at
