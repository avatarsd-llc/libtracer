...
The 4096-byte decode arena (`core/src/graph.cpp:2483`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:2465-2470`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:2853`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:2483-2484`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:2465   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:2466       value.try_materialize(*value_backend_);
core/src/graph.cpp:2468   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:2483   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:2484   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:2486   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:2465`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:2468` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:2483`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:2483-2484` reads
...
`bump_source_t` (`core/include/libtracer/mem_source.hpp:298`) carves from that stack buffer and,
past it, falls back to the graph's injected control seam `ctl_`
(`core/include/libtracer/graph.hpp:599`, `control_source()`), whose default is the NOTHROW heap
...
(`core/src/graph.cpp:2487`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:1522-1530`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:1581-1589`,
[#596]).
...
everywhere smaller (`core/include/libtracer/fwd_router.hpp:1495-1498`, recorded as an erratum to
...
`core/include/libtracer/fwd_router.hpp:758-762` states it — the router performs no decode and no
