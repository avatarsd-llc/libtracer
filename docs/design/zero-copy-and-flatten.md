...
The 4096-byte decode arena (`core/src/graph.cpp:1451`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1433-1438`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1760`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1451-1452`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2107`, materialize at `:2145`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1721`, sub-rope materialize at `:1736` and `:1705`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1433   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1434       value.try_materialize(*value_backend_);
core/src/graph.cpp:1436   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1451   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1452   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1454   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1433`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1436` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1451`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1451-1452` reads
...
(`core/src/graph.cpp:1455`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1721`, `:1672`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1726`, `:1705`) — out of the router's injected `flat` backend, and a refused flatten drops the
  frame rather than delivering an empty value (#730).
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:834-842`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:886-894`,
[#596]).
...
single-link view path (`core/src/fwd_router.cpp:1339`, the check at `:1343-1348`). Second, the tier earns its place only on large, lightly
fragmented frames: at 64 KB across 2 links it is ~12% ahead of flatten-then-arena, and behind it
everywhere smaller (`core/include/libtracer/fwd_router.hpp:815-818`, recorded as an erratum to
...
(`core/src/fwd_router.cpp:1401-1405`). The contract at
`core/include/libtracer/fwd_router.hpp:430-434` states it — the router performs no decode and no
