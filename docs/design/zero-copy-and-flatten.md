...
The 4096-byte decode arena (`core/src/graph.cpp:2317`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:2299-2304`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:2660`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:2317-2318`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:2299   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:2300       value.try_materialize(*value_backend_);
core/src/graph.cpp:2302   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:2317   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:2318   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:2320   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:2299`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:2302` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:2317`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
array that `parse_branch_node` (`core/src/graph.cpp:571`) walks via `end` / `first_child`. That
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:2317-2318` reads
...
(`core/src/graph.cpp:2321`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
