...
The 4096-byte decode arena (`core/src/graph.cpp:1035`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.materialize(*value_backend_)` (`core/src/graph.cpp:1022`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1185`, materialize at `:1213`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:870`, sub-rope materialize at `:897` and `:917`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1022   const view_t head = value.materialize(*value_backend_);   // A: the flatten
core/src/graph.cpp:1023   if (head.empty() && value.total_length() != 0) return std::unexpected(status_t::BACKPRESSURE);
core/src/graph.cpp:1035   std::array<std::byte, 4096> stack;                        // B: the arena
core/src/graph.cpp:1036   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1038   wire::decode_into(head.bytes(), src);
```
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1035-1036` reads
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:870`, `:883`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:897`, `:917`) — out of the router's injected `flat` backend, and a refused flatten drops the
  frame rather than delivering an empty value (#730).
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:516-532`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:534-550`,
[#596]).
...
everywhere smaller (`core/include/libtracer/fwd_router.hpp:521-524`, recorded as an erratum to
...
`core/include/libtracer/fwd_router.hpp:273-277` states it — the router performs no decode and no
