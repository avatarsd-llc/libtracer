...
The 4096-byte decode arena (`core/src/graph.cpp:2545`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:2527-2532`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:2947`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:2545-2546`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:2527   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:2528       value.try_materialize(*value_backend_);
core/src/graph.cpp:2530   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:2545   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:2546   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:2548   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:2527`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:2530` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:2545`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
array that `parse_branch_node` (`core/src/graph.cpp:609`) walks via `end` / `first_child`. That
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:2545-2546` reads
...
(`core/src/graph.cpp:2549`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:1586-1594`,
[#596]).
...
everywhere smaller (`core/include/libtracer/fwd_router.hpp:1500-1503`, recorded as an erratum to
...
`core/include/libtracer/fwd_router.hpp:763-767` states it — the router performs no decode and no
...
| **2** | WS receive into a pooled segment, deliver owning — **LANDED for the ESP `esp_http_server` link, opt-in** (#1565: `httpd_ws_link_t`'s `rx_backend`, unfragmented frames; borrowed stays the default) | ⑧'s per-frame `new[]`; the borrowed-then-store copy | One allocation per WS frame per session removed, and the rope tier gets an owned segment | yes — every WS frame | Changes the WS delivery contract, which is why it is opt-in and per link rather than a flip; the pool is the INTEGRATOR's injection, not a library buffer |
