...
The 4096-byte decode arena (`core/src/graph.cpp:1616`) is **structure storage, not a payload copy**:
...
```
core/src/graph.cpp:1598   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1599       value.try_materialize(*value_backend_);
core/src/graph.cpp:1601   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1616   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1617   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1619   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1598`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1601` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1616`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1616-1617` reads
...
(`core/src/graph.cpp:1620`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
