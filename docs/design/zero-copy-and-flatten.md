...
The 4096-byte decode arena (`core/src/graph.cpp:1301`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1283-1288`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1561`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1301-1302`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1917`, materialize at `:1955`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1563`, sub-rope materialize at `:1578` and `:1547`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1283   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1284       value.try_materialize(*value_backend_);
core/src/graph.cpp:1286   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1301   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1302   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1304   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1283`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1286` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1301`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
(`integrations/esp-idf/libtracer/include/libtracer_esp/httpd_ws_link.hpp:170` — a PUBLIC constant
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1301-1302` reads
...
(`core/src/graph.cpp:1305`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1563`, `:1514`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1568`, `:1547`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:1195`, the check at `:1199-1204`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1248-1252`). The contract at
...
- **WS reassembly (⑧)** regrows exact-size per fragment
  (`integrations/esp-idf/libtracer/httpd_ws_link.cpp:547-557`), which is O(n²) in total bytes
