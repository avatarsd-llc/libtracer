...
The 4096-byte decode arena (`core/src/graph.cpp:1762`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1744-1749`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:2081`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1762-1763`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:1744   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1745       value.try_materialize(*value_backend_);
core/src/graph.cpp:1747   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1762   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1763   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1765   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1744`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1747` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1762`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1762-1763` reads
...
(`core/include/libtracer/graph.hpp:426`, `control_source()`), whose default is the NOTHROW heap
...
(`core/src/graph.cpp:1766`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
