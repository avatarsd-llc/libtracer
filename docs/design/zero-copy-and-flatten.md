...
returns the sole link (`core/include/libtracer/rope.hpp:204`), a refcount bump, so the branch
...
The 4096-byte decode arena (`core/src/graph.cpp:1484`) is **structure storage, not a payload copy**:
...
- **Composition shares segments.** `rope_t::subrope(off, len)`
  (`core/include/libtracer/rope.hpp:257`) trims the covering links with `view_t::subview` and
...
- **Egress scatter-gathers.** `rope_t::to_iovec` (`core/include/libtracer/rope.hpp:286`) emits one
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1466-1471`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1793`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1484-1485`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2205`, materialize at `:2243`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1819`, sub-rope materialize at `:1834` and `:1803`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
| ⑫ | Reply-route synthesis — `tlv_sliced` (`core/src/fwd_reply.hpp:109`, called at `core/src/fwd_reply.cpp:132-133`) | yes | yes | Bounded frame synthesis: the route wires are rewritten | No — these are emitted bytes, not a copy of payload |
...
```
core/src/graph.cpp:1466   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1467       value.try_materialize(*value_backend_);
core/src/graph.cpp:1469   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1484   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1485   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1487   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1466`)** is zero-copy for a single-link rope — `try_materialize` returns
`links()[0]`, a refcount bump (`core/include/libtracer/rope.hpp:204`) — and memcpys only a
multi-link rope, drawing from the injected `value_backend_`. An exhausted pool is refused by name:
`graph.cpp:1469` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1484`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
`core/src/transport_ws.cpp:651` (the WS client's receive loop), and
`core/src/posix_endpoint.cpp:483` — the ONE per-chunk scratch both multi-peer servers now
...
(`integrations/esp-idf/libtracer/include/libtracer_esp/httpd_ws_link.hpp:171` — a PUBLIC constant
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1484-1485` reads
...
(`core/include/libtracer/graph.hpp:337`, `control_source()`), whose default is the NOTHROW heap
...
(`core/src/graph.cpp:1488`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1819`, `:1770`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1824`, `:1803`) — out of the router's injected `flat` backend, and a refused flatten drops the
  frame rather than delivering an empty value (#730).
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:844-852`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:896-904`,
[#596]).
...
single-link view path (`core/src/fwd_router.cpp:1431`, the check at `:1435-1440`). Second, the tier earns its place only on large, lightly
fragmented frames: at 64 KB across 2 links it is ~12% ahead of flatten-then-arena, and behind it
everywhere smaller (`core/include/libtracer/fwd_router.hpp:825-828`, recorded as an erratum to
...
(`core/src/fwd_router.cpp:1493-1497`). The contract at
`core/include/libtracer/fwd_router.hpp:440-444` states it — the router performs no decode and no
...
3. **Rope-native node accessors for the walk** — `ensure_cache`
   (`core/src/op_resolve_view.cpp:252-258`) flattens each *accessed* node whose own subrope
...
None of these reaches ⑤. `own_wire` (`core/src/op_resolve_view.cpp:140`) is structural: a *mutated*
...
accepted segment: `read_exact(fd, seg->bytes.data(), len)` (`core/src/transport_tcp.cpp:282`,
`read_exact` defined at `:219`) fills a segment freshly allocated from the injected backend by
...
chunk-fed transports rather than their state machine (`core/src/transport_tcp.cpp:256-261`). The
only stack scratch left on this path is `drain()`'s 4096-byte backpressure discard buffer
(`core/src/transport_tcp.cpp:239`), which runs when a frame is dropped, not when one is delivered.
...
- **WS reassembly (⑧)** regrows exact-size per fragment
  (`integrations/esp-idf/libtracer/httpd_ws_link.cpp:574-584`), which is O(n²) in total bytes
