...
The 4096-byte decode arena (`core/src/graph.cpp:1366`) is **structure storage, not a payload copy**:
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1964`, materialize at `:2002`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1578`, sub-rope materialize at `:1593` and `:1562`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1348   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1349       value.try_materialize(*value_backend_);
core/src/graph.cpp:1351   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1366   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1367   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1369   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1348`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1351` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1366`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1366-1367` reads
...
(`core/src/graph.cpp:1370`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1578`, `:1529`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1583`, `:1562`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:1210`, the check at `:1214-1219`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1263-1267`). The contract at
