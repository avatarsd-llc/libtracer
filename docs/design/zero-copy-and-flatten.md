...
The 4096-byte decode arena (`core/src/graph.cpp:1050`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.materialize(*value_backend_)` (`core/src/graph.cpp:1037`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1274`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1050-1051`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:1037   const view_t head = value.materialize(*value_backend_);   // A: the flatten
core/src/graph.cpp:1038   if (head.empty() && value.total_length() != 0) return std::unexpected(status_t::BACKPRESSURE);
core/src/graph.cpp:1050   std::array<std::byte, 4096> stack;                        // B: the arena
core/src/graph.cpp:1051   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1053   wire::decode_into(head.bytes(), src);
```

**Cost A, the flatten (`core/src/graph.cpp:1037`)** is zero-copy for a single-link rope —
...
yields an empty head, which `core/src/graph.cpp:1038` surfaces as `BACKPRESSURE` rather than
...
**Cost B, the arena (`core/src/graph.cpp:1050`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1050-1051` reads
