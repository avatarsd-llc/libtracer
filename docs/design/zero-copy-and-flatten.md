...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2285`, materialize at `:2323`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
...
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:878-886`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:930-938`,
[#596]).
...
everywhere smaller (`core/include/libtracer/fwd_router.hpp:859-862`, recorded as an erratum to
