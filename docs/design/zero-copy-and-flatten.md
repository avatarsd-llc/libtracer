...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1672`, materialize at `:1704`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1307`, `:1320`) read a control frame's
