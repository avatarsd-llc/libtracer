...
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1385`, sub-rope materialize at `:1412` and `:1432`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1385`, `:1398`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1412`, `:1432`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:980`, the check at `:986-991`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1049-1080`). The contract at
