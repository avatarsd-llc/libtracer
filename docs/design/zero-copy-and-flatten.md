...
The 4096-byte decode arena (`core/src/graph.cpp:1851`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1833-1838`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:2170`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1851-1852`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2925`, materialize at `:2963`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:2377`, sub-rope materialize at `:2394` and `:2361`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1833   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1834       value.try_materialize(*value_backend_);
core/src/graph.cpp:1836   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1851   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1852   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1854   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1833`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1836` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1851`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
array that `parse_branch_node` (`core/src/graph.cpp:396`) walks via `end` / `first_child`. That
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1851-1852` reads
...
(`core/include/libtracer/graph.hpp:444`, `control_source()`), whose default is the NOTHROW heap
...
(`core/src/graph.cpp:1855`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:2377`, `:2328`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:2383`, `:2361`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:1790`, the check at `:1795-1800`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1854-1858`). The contract at
