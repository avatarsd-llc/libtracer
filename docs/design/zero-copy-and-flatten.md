...
The 4096-byte decode arena (`core/src/graph.cpp:1669`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1651-1656`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1988`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1669-1670`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2213`, materialize at `:2251`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1823`, sub-rope materialize at `:1838` and `:1807`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
...
```
core/src/graph.cpp:1651   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1652       value.try_materialize(*value_backend_);
core/src/graph.cpp:1654   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1669   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1670   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1672   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1651`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1654` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1669`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1669-1670` reads
...
(`core/src/graph.cpp:1673`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1823`, `:1774`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1828`, `:1807`) — out of the router's injected `flat` backend, and a refused flatten drops the
...
single-link view path (`core/src/fwd_router.cpp:1435`, the check at `:1439-1444`). Second, the tier earns its place only on large, lightly
...
(`core/src/fwd_router.cpp:1497-1501`). The contract at
