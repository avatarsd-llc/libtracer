...
The 4096-byte decode arena (`core/src/graph.cpp:1044`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.materialize(*value_backend_)` (`core/src/graph.cpp:1031`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1268`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1044-1045`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
```
core/src/graph.cpp:1031   const view_t head = value.materialize(*value_backend_);   // A: the flatten
core/src/graph.cpp:1032   if (head.empty() && value.total_length() != 0) return std::unexpected(status_t::BACKPRESSURE);
core/src/graph.cpp:1044   std::array<std::byte, 4096> stack;                        // B: the arena
core/src/graph.cpp:1045   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1047   wire::decode_into(head.bytes(), src);
```

**Cost A, the flatten (`:961`)** is zero-copy for a single-link rope — `materialize` returns
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1044-1045` reads
