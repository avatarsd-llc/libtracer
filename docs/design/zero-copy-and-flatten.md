...
returns the sole link (`core/include/libtracer/rope.hpp:182`), a refcount bump, so the branch
...
|---|---|---|
| Ingress ownership | `rope_t::flatten` (`core/src/rope.cpp:22`), `read_exact` into the accepted segment (`core/src/transport_tcp.cpp:243`) | A transient recv buffer cannot be borrowed by a rope that outlives the receive call |
...
`core/src/op_resolve_walk.hpp:542`) emits rewritten route wires — tens of bytes, never
payload-scaled.

The 4096-byte decode arena (`core/src/graph.cpp:1179`) is **structure storage, not a payload copy**:
...
- **Composition shares segments.** `rope_t::subrope(off, len)`
  (`core/include/libtracer/rope.hpp:218`) trims the covering links with `view_t::subview` and
...
- **Egress scatter-gathers.** `rope_t::to_iovec` (`core/include/libtracer/rope.hpp:247`) emits one
  ...
  (`core/src/transport_ws.cpp:272`); TCP prepends a u32-LE length via `prefixed_iov_t`
  (`core/src/transport_tcp.cpp:59`). With `kMaxServerIov = 16` (`core/src/transport_ws.cpp:150`),
  the common reply (≤ ~6 spans) fits the stack `std::array<::iovec, kMaxServerIov + 1>`
  (`core/src/transport_ws.cpp:280`) — zero heap, zero payload copy. The only host TX copy is the
...
| ② | Branch write — `value.materialize(*value_backend_)` (`core/src/graph.cpp:1166`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1403`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1179-1180`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:1752`, materialize at `:1784`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
| ⑪ | Control-child strip — `on_control_rope` (`core/src/fwd_router.cpp:1387`, sub-rope materialize at `:1414` and `:1434`, from the router's injected backend) | no | only a multi-link ADVERTISE / COMPACT sub-rope | Fallback, and fused rather than eliminated — the next consumer re-encodes anyway | Yes, with a near-zero saving |
| ⑫ | Reply-route synthesis — `tlv_sliced` (`core/src/op_resolve_walk.hpp:542`, called at `:609-610`) | yes | yes | Bounded frame synthesis: the route wires are rewritten | No — these are emitted bytes, not a copy of payload |
...
```
core/src/graph.cpp:1166   const view_t head = value.materialize(*value_backend_);   // A: the flatten
core/src/graph.cpp:1167   if (head.empty() && value.total_length() != 0) return std::unexpected(status_t::BACKPRESSURE);
core/src/graph.cpp:1179   std::array<std::byte, 4096> stack;                        // B: the arena
core/src/graph.cpp:1180   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1182   wire::decode_into(head.bytes(), src);
```

**Cost A, the flatten (`:1166`)** is zero-copy for a single-link rope — `materialize` returns
`links()[0]`, a refcount bump (`core/include/libtracer/rope.hpp:182`) — and memcpys only a
multi-link rope, drawing from the injected `value_backend_`. An exhausted pool yields an empty
head, which `graph.cpp:1167` surfaces as `BACKPRESSURE` rather than letting the decoder read it back as a
...
**Cost B, the arena (`graph.cpp:1179`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
`transport_ws.cpp:406` and `:793`. They are not decode arenas and carry no structure,
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1179-1180` reads
...
(`core/src/graph.cpp:1183`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
...
- `on_control_rope` / `peek_control` (`core/src/fwd_router.cpp:1387`, `:1400`) read a control frame's
  label off the rope and materialize only the sub-rope a re-encoding consumer needs contiguous
  (`:1414`, `:1434`) — out of the router's injected `flat` backend, and a refused flatten drops the
  frame rather than delivering an empty value (#730).
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:779-787`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:797-805`,
[#596]).
...
single-link view path (`core/src/fwd_router.cpp:983`, the check at `:989-994`). Second, the tier earns its place only on large, lightly
fragmented frames: at 64 KB across 2 links it is ~12% ahead of flatten-then-arena, and behind it
everywhere smaller (`core/include/libtracer/fwd_router.hpp:760-763`, recorded as an erratum to
...
(`core/src/fwd_router.cpp:1052-1083`). The contract at
`core/include/libtracer/fwd_router.hpp:411-415` states it — the router performs no decode and no
...
accepted segment: `read_exact(fd, seg->bytes.data(), len)` (`core/src/transport_tcp.cpp:243`,
`read_exact` defined at `:181`) fills a segment freshly allocated from the injected backend by
...
- **WS reassembly (⑧)** regrows exact-size per fragment
  (`integrations/esp-idf/libtracer/httpd_ws_link.cpp:272-282`), which is O(n²) in total bytes
...
- **⑨ the WS TX gather** — `httpd_ws_send_frame_async` takes one contiguous `{payload, len}` and
  ...
  not a decoder.
- **⑫ reply-route synthesis** — `tlv_sliced` (`core/src/op_resolve_walk.hpp:542`) emits rewritten
