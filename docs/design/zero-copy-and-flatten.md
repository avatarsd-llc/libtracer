...
The 4096-byte decode arena (`core/src/graph.cpp:1119`) is **structure storage, not a payload copy**:
...
```
core/src/graph.cpp:1106   const view_t head = value.materialize(*value_backend_);   // A: the flatten
core/src/graph.cpp:1107   if (head.empty() && value.total_length() != 0) return std::unexpected(status_t::BACKPRESSURE);
core/src/graph.cpp:1119   std::array<std::byte, 4096> stack;                        // B: the arena
core/src/graph.cpp:1120   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1122   wire::decode_into(head.bytes(), src);
```
...
- The FWD request terminus: `resolve_terminus_rope`
  (`core/include/libtracer/fwd_router.hpp:674-684`) adopts a fragmented request as
...
exhaustion drops the frame rather than throwing (`core/include/libtracer/fwd_router.hpp:692-702`,
[#596]).
...
everywhere smaller (`core/include/libtracer/fwd_router.hpp:679-682`, recorded as an erratum to
