...
The 4096-byte decode arena (`core/src/graph.cpp:1645`) is **structure storage, not a payload copy**:
...
| ② | Branch write — `value.try_materialize(*value_backend_)` (`core/src/graph.cpp:1627-1632`) | no — refcount bump | yes (one flatten to feed the span cursor) | Fallback | Multi-link leg: yes, via a rope-native branch decode |
| ③ | Field write — the twin of ② (`core/src/graph.cpp:1964`) | no — refcount bump | yes | Fallback | Same as ② |
| ④ | 4096-byte decode arena (`core/src/graph.cpp:1645-1646`) | yes — paid on every branch write | yes | Structure scratch, not a payload copy | **No** — see §3; the rope cursor is a byte source, not a structure store |
...
| ⑩ | COMPACT remote delivery — `deliver_remote` (`core/src/fwd_router.cpp:2211`, materialize at `:2249`, from the router's injected backend) | no — adopt | yes — auto-promotion leg only | Fallback, narrow | Yes — a scatter-gather compact encoder |
...
```
core/src/graph.cpp:1627   const std::expected<view_t, tr::view::flatten_err_t> head =    // A: the flatten
core/src/graph.cpp:1628       value.try_materialize(*value_backend_);
core/src/graph.cpp:1630   head.error() == NO_MEMORY ? BACKPRESSURE : TYPE_MISMATCH       //    (paraphrased)
core/src/graph.cpp:1645   std::array<std::byte, 4096> stack;                             // B: the arena
core/src/graph.cpp:1646   mem::bump_source_t src(stack, *ctl_);
core/src/graph.cpp:1648   wire::decode_into(head->bytes(), src);
```

**Cost A, the flatten (`:1627`)** is zero-copy for a single-link rope — `try_materialize` returns
...
`graph.cpp:1630` reads `flatten_err_t::NO_MEMORY` and surfaces `BACKPRESSURE` rather than letting
...
**Cost B, the arena (`graph.cpp:1645`)** is the `std::array<std::byte, 4096>` backing `decode_into`'s node
...
The overflow leg does not draw from a throwing upstream. `core/src/graph.cpp:1645-1646` reads
...
(`core/src/graph.cpp:1649`), which is *not* `BACKPRESSURE` — this decode cannot distinguish "the
