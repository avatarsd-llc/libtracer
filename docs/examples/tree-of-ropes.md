...
- **A transport is an identity, not memory** — mounting a transport (ADR-0027) via
  ...
  `core/src/transport_vertex.cpp:164`). The transport's real bytes live *outside*
...
- **Mount = identity** — a fresh `/net/can/link0` resolves as an address but holds no
  ...
  `core/src/transport_vertex.cpp:68`) — categorically not a chained payload. The live
