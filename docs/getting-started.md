...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1134,1231`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1432-1435`), so a temporary lambda written inline at
