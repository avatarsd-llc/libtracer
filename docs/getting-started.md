...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:512,599`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:742-745`), so a temporary lambda written inline at
