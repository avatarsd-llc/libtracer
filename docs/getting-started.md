...
| --- | --- | --- |
| `BUILD_TESTING` | unset (off) | compiles `core/tests` and registers the example programs as `example_*` smoke tests (`core/CMakeLists.txt:373`) |
| `LIBTRACER_BUILD_EXAMPLES` | on when libtracer is the top-level project | compiles the seven programs under `core/examples/` (`core/CMakeLists.txt:380`) |
...
| `LIBTRACER_WITH_QUIC` | `OFF` | configures the separate `libtracer_quic` target (QUIC and WebTransport); needs msquic installed (`core/CMakeLists.txt:274`) |
...
(`core/CMakeLists.txt:362-361`): pre-1.0, a minor bump may break the C++ API, so a
...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:936,1023`), so `(*got)` is a
...
value it names (`core/include/libtracer/vertex.hpp:166-169`), so a long-lived reference
holds the graph's memory; take the bytes and drop it.

```{note}
`rope_t::only()` has a precondition — `link_count() == 1`, debug-asserted
(`core/include/libtracer/rope.hpp:180-189`). It is the consumer's explicit "this value
...
(`core/include/libtracer/vertex.hpp:598`). The sugar takes the callable as `F&`
(`core/include/libtracer/graph.hpp:1166-1169`), so a temporary lambda written inline at
