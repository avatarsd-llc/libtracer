...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:883,970`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1113-1116`), so a temporary lambda written inline at
