...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:628,715`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:858-861`), so a temporary lambda written inline at
