...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:607,694`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:837-840`), so a temporary lambda written inline at
