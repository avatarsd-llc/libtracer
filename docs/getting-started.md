...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1197,1294`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1495-1498`), so a temporary lambda written inline at
