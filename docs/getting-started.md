...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:552,639`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:782-785`), so a temporary lambda written inline at
