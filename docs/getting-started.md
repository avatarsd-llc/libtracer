...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1103,1190`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1344-1347`), so a temporary lambda written inline at
