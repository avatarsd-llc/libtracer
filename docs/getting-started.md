...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:392,444`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:569-574`), so a temporary lambda written inline at
