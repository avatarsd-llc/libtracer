...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1217,1396`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1604-1607`), so a temporary lambda written inline at
