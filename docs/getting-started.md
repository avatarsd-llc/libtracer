...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:915,1002`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1145-1148`), so a temporary lambda written inline at
