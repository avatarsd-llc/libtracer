...
`tr::view::over_bytes` (`core/include/libtracer/mem_heap.hpp:229`) is the one audited
...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:343,395`), so `(*got)` is a
...
value it names (`core/include/libtracer/vertex.hpp:78-81`), so a long-lived reference
...
(`core/include/libtracer/vertex.hpp:494`). The sugar takes the callable as `F&`
(`core/include/libtracer/graph.hpp:520-525`), so a temporary lambda written inline at
...
field-writes a QoS setting and reads the result back through `:schema`.
