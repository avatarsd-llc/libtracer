...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:620,707`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:850-853`), so a temporary lambda written inline at
