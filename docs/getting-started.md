...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1357,1563`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1771-1774`), so a temporary lambda written inline at
