...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1340,1546`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1754-1757`), so a temporary lambda written inline at
