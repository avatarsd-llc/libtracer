...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:857,944`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1087-1090`), so a temporary lambda written inline at
