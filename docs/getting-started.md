...
| --- | --- | --- |
| `BUILD_TESTING` | unset (off) | compiles `core/tests` and registers the example programs as `example_*` smoke tests (`core/CMakeLists.txt:374`) |
| `LIBTRACER_BUILD_EXAMPLES` | on when libtracer is the top-level project | compiles the seven programs under `core/examples/` (`core/CMakeLists.txt:381`) |
...
| `LIBTRACER_WITH_QUIC` | `OFF` | configures the separate `libtracer_quic` target (QUIC and WebTransport); needs msquic installed (`core/CMakeLists.txt:275`) |
...
(`core/CMakeLists.txt:363-362`): pre-1.0, a minor bump may break the C++ API, so a
...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1077,1164`), so `(*got)` is a
...
value it names (`core/include/libtracer/vertex.hpp:231-234`), so a long-lived reference
...
(`core/include/libtracer/graph.hpp:1309-1312`), so a temporary lambda written inline at
