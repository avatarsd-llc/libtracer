...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:626,713`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:856-859`), so a temporary lambda written inline at
