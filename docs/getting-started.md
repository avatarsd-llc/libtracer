...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:777,864`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1007-1010`), so a temporary lambda written inline at
