...
| --- | --- | --- |
| `BUILD_TESTING` | unset (off) | compiles `core/tests` and registers the example programs as `example_*` smoke tests (`core/CMakeLists.txt:409`) |
| `LIBTRACER_BUILD_EXAMPLES` | on when libtracer is the top-level project | compiles the seven programs under `core/examples/` (`core/CMakeLists.txt:416`) |
...
| `LIBTRACER_WITH_QUIC` | `OFF` | configures the separate `libtracer_quic` target (QUIC and WebTransport); needs msquic installed (`core/CMakeLists.txt:312`) |
...
(`core/CMakeLists.txt:398-401`): pre-1.0, a minor bump may break the C++ API, so a
...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:806,893`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1036-1039`), so a temporary lambda written inline at
