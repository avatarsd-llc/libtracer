...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1023,1110`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1255-1258`), so a temporary lambda written inline at
