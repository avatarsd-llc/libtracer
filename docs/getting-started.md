...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1127,1214`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1363-1366`), so a temporary lambda written inline at
