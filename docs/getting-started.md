...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:560,647`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:790-793`), so a temporary lambda written inline at
