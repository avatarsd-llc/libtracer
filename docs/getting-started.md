...
`tr::view::over_bytes` (`core/include/libtracer/mem_heap.hpp:405`) is the one audited
...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:1397,1603`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:1811-1814`), so a temporary lambda written inline at
