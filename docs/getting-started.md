...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:762,849`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:992-995`), so a temporary lambda written inline at
