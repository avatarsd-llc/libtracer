...
`result_t<value_ref_t>` (`core/include/libtracer/graph.hpp:576,663`), so `(*got)` is a
...
(`core/include/libtracer/graph.hpp:806-809`), so a temporary lambda written inline at
