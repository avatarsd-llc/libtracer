...
(`core/src/transport_vertex.cpp:93-100`); the `config` member `kind` selects which
transport factory builds the link (`core/src/transport_vertex.cpp:58`, factories
registered through `register_transport_type` at `:122`). The created connection is
...
(`core/src/transport_vertex.cpp:143-161,197-215`).
