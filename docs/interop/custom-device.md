...
(`graph_t::create_child`, `core/src/graph.cpp:2756-2783`). Reading `:children[]`
...
registered through `register_transport_type` at `:133`). The created connection is
...
(`core/src/transport_vertex.cpp:158-176,435-461`).
...
behind it (`core/include/libtracer/graph.hpp:481-485`). Retirement empties the
