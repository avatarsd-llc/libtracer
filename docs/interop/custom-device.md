...
(`graph_t::create_child`, `core/src/graph.cpp:2693-2720`). Reading `:children[]`
returns the parent's **members**, never SPECs.

On the `/net` plane the registered child types are `client` and `listener`
(`core/src/transport_vertex.cpp:102-109`); the `config` member `kind` selects which
transport factory builds the link (`core/src/transport_vertex.cpp:60`, factories
registered through `register_transport_type` at `:136`). The created connection is
...
(`core/src/transport_vertex.cpp:161-179,438-464`).
...
behind it (`core/include/libtracer/graph.hpp:473-477`). Retirement empties the
