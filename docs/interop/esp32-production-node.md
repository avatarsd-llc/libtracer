...
(`core/include/libtracer/graph.hpp:324-326`) — and the **four** of
...
undeclared kind fails creation with `SCHEMA_NOT_FOUND` (`:152`). The accepted direction is
...
(`core/include/libtracer/graph.hpp:1499`), which snapshots four per-cause totals —
`no_target`, `denied`, `out_of_memory`, `fan_out_truncated` (`graph.hpp:1467-1489`). Each
