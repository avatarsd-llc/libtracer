...
its rope flattens draw from (`core/include/libtracer/fwd_router.hpp:154-156`). The full set of
...
(`core/include/libtracer/graph.hpp:1031`), which snapshots three per-cause totals —
`no_target`, `denied`, `out_of_memory` (`graph.hpp:1014-1021`). They are counted and
