...
its rope flattens draw from (`core/include/libtracer/fwd_router.hpp:152-154`). The full set of
...
(`core/include/libtracer/graph.hpp:903`), which snapshots three per-cause totals —
`no_target`, `denied`, `out_of_memory` (`graph.hpp:886-893`). They are counted and
