...
(`core/include/libtracer/graph.hpp:413-415`) — and the **four** of
...
application** via `register_module` (`:138`) — declared-only per ADR-0073 §4, so an
undeclared kind fails creation with `SCHEMA_NOT_FOUND` (`:176`). The accepted direction is
...
(`core/include/libtracer/graph.hpp:1820`), which snapshots four per-cause totals —
`no_target`, `denied`, `out_of_memory`, `fan_out_truncated` (`graph.hpp:1788-1810`). Each
