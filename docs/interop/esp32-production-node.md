...
(`view_node::ensure_cache`, `view_node::own_wire` — both of its branches since #793, so a
peer cannot escape the bound by sending a payload that happens to land contiguously), which
the router reaches by handing `flat` to its `op_resolver_t` (#766). Two caveats remain, so
