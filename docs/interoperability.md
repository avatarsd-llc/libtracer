...
| | Surface | Catalog |
| ---- | ---- | ---- |
| **What the implementation accepts** | a CREATE-gated `SPEC{ NAME "type", NAME "name", SETTINGS "config" }` write to `/net:children[]` (`graph_t::create_child`, `core/src/graph.cpp:3149`) | one global set of registered child types — `client` and `listener` (`core/src/transport_vertex.cpp:129-137`); the concrete transport is selected by a `kind` key inside the `config` SETTINGS (`core/src/transport_vertex.cpp:50,69`), extended per transport module through `register_transport_type` (`core/src/transport_vertex.cpp:168`) |
...
(`core/src/transport_vertex.cpp:168-173,188-390,439-465`). `/net` itself is the recommended root
