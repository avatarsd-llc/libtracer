...
| | Surface | Catalog |
| ---- | ---- | ---- |
| **What the implementation accepts** | a CREATE-gated `SPEC{ NAME "type", NAME "name", SETTINGS "config" }` write to `/net:children[]` (`graph_t::create_child`, `core/src/graph.cpp:1914`) | one global set of registered child types — `client` and `listener` (`core/src/transport_vertex.cpp:93-101`); the concrete transport is selected by a `kind` key inside the `config` SETTINGS (`core/src/transport_vertex.cpp:47,58`), extended per transport module through `register_transport_type` (`core/src/transport_vertex.cpp:122`) |
...
(`core/src/transport_vertex.cpp:127,143-161,197-215`). `/net` itself is the recommended root
