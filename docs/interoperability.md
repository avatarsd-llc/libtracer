...
| | Surface | Catalog |
| ---- | ---- | ---- |
| **What the implementation accepts** | a CREATE-gated `SPEC{ NAME "type", NAME "name", SETTINGS "config" }` write to `/net:children[]` (`graph_t::create_child`, `core/src/graph.cpp:3149`) | one global set of registered child types — `client` and `listener` (`core/src/transport_vertex.cpp:235-243`); the concrete transport is selected by a `kind` key inside the `config` SETTINGS (`core/src/transport_vertex.cpp:52,71`), extended per transport module through `register_transport_type` (`core/src/transport_vertex.cpp:274`) |
...
(`core/src/transport_vertex.cpp:274-279,294-508,557-583`). `/net` itself is the recommended root
