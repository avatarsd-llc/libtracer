...
| | Surface | Catalog |
| ---- | ---- | ---- |
| **What the implementation accepts** | a CREATE-gated `SPEC{ NAME "type", NAME "name", SETTINGS "config" }` write to `/net:children[]` (`graph_t::create_child`, `core/src/graph.cpp:2554`) | one global set of registered child types — `client` and `listener` (`core/src/transport_vertex.cpp:99-107`); the concrete transport is selected by a `kind` key inside the `config` SETTINGS (`core/src/transport_vertex.cpp:48,64`), extended per transport module through `register_transport_type` (`core/src/transport_vertex.cpp:128`) |
...
(`core/src/transport_vertex.cpp:128-133,144-167,213-239`). `/net` itself is the recommended root
