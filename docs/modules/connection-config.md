...
| `backoff` | `VALUE` u32 | DIAL | `0` | Self-heal retry interval in ms (RFC-0014 §4), consumed by the S5 liveness engine (`self_heal_link_t`) on a kind registered `self_heal_dial`. `0` = the engine's default (`kDefaultBackoffMs`, 1000 ms). On a kind not opted in (today: all the built-ins) it is parsed but has no consumer. |
