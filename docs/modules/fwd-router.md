...
Signature source: `core/include/libtracer/fwd_router.hpp:177` (constructor), `:267`
(`add_child`), `:324` (`subscribe_toward`), `:426-438` (the sink function-pointer types);
...
- **A reply is delivered as a rope, never flattened by the router**
  (`core/include/libtracer/fwd_router.hpp:440-449`). A sink that wants contiguous bytes holds
...
`kind` selector naming a registered transport factory (`core/src/transport_vertex.cpp:48` documents
the config shape; `kind` is read at `:59`), plus the universal keys `addr`, `port`, `role`,
`keepalive`, `max_frame`, `backoff` and `connect_timeout`. Two catalog child types are registered
against the graph, `client` and `listener` (`core/src/transport_vertex.cpp:94,98`), which supply
the role default. Extra transport kinds join the catalog through `register_transport_type`
(`core/src/transport_vertex.cpp:123`) — that is how the QUIC module extends a node without this
file ever learning about it.

**The write is ACL-gated.** The `:children[]` append is gated on the parent vertex's `CREATE`
right and denied with `PERMISSION_DENIED` otherwise (`core/src/graph.cpp:2421-2422`). Under
...
hop (`core/src/transport_vertex.cpp:288-295,305-312`;
...
dedupe rather than a second source of truth (`core/src/transport_vertex.cpp:314-322`). Because a
...
`SCHEMA_NOT_FOUND` (`core/src/transport_vertex.cpp:162`). The application declares each module
under a name it chooses through `register_module` (`core/src/transport_vertex.cpp:128`,
...
only place that can say so (`core/include/libtracer/fwd_router.hpp:267`,
...
no liveness, and answers `BACKPRESSURE` (`core/src/transport_vertex.cpp:414-417`). Discarding that
...
(`core/src/transport_vertex.cpp:423`), so a retry after the pressure clears still finds its link.
...
reports `UP` or `LISTENING` at creation (`core/src/transport_vertex.cpp:437-439`).
