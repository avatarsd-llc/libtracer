...
- **The default delivery leg copies nothing.** A full-route `FWD{WRITE}` fan-out scatter-gathers a
  ...
  stored value (`core/src/fwd_router.cpp:2387`). The `COMPACT` leg is the one that flattens,
  because a `COMPACT` wraps a contiguous payload (`core/src/fwd_router.cpp:2188`) — single-link, that
...
- **Delivery drops rather than aborts on the value path — with one named residual.** The flatten,
  ...
  **compact-flagged** flow's first delivery on a link resolves its label *before* those three steps
  (`fwd_router.cpp:2195`), and that allocates its `link_tables_t` and its egress entry from the
...
against the graph, `client` and `listener` (`core/src/transport_vertex.cpp:98,102`), which supply
the role default. Extra transport kinds join the catalog through `register_transport_type`
(`core/src/transport_vertex.cpp:132`) — that is how the QUIC module extends a node without this
...
hop (`core/src/transport_vertex.cpp:480-487,497-503`;
...
dedupe rather than a second source of truth (`core/src/transport_vertex.cpp:505-513`). Because a
...
no liveness, and answers `BACKPRESSURE` (`core/src/transport_vertex.cpp:606-609`). Discarding that
...
(`core/src/transport_vertex.cpp:615`), so a retry after the pressure clears still finds its link.
...
reports `UP` or `LISTENING` at creation (`core/src/transport_vertex.cpp:634-637`).
