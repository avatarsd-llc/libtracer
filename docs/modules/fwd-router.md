...
Signature source: `core/include/libtracer/fwd_router.hpp:179` (constructor), `:351`
(`add_child`), `:408` (`subscribe_toward`), `:528-540` (the sink function-pointer types);
...
- **A reply is delivered as a rope, never flattened by the router**
  (`core/include/libtracer/fwd_router.hpp:542-551`). A sink that wants contiguous bytes holds
...
- **The default delivery leg copies nothing.** A full-route `FWD{WRITE}` fan-out scatter-gathers a
  ...
  stored value (`core/src/fwd_router.cpp:2924`). The `COMPACT` leg is the one that flattens,
  because a `COMPACT` wraps a contiguous payload (`core/src/fwd_router.cpp:2725`) — single-link, that
...
- **Delivery drops rather than aborts on the value path — with one named residual.** The flatten,
  ...
  **compact-flagged** flow's first delivery on a link resolves its label *before* those three steps
  (`fwd_router.cpp:2732`), and that allocates its `link_tables_t` and its egress entry from the
...
against the graph, `client` and `listener` (`core/src/transport_vertex.cpp:102,106`), which supply
the role default. Extra transport kinds join the catalog through `register_transport_type`
(`core/src/transport_vertex.cpp:136`) — that is how the QUIC module extends a node without this
file ever learning about it.

**The write is ACL-gated.** The `:children[]` append is gated on the parent vertex's `CREATE`
right and denied with `PERMISSION_DENIED` otherwise (`core/src/graph.cpp:2827-2860`). Under
...
hop (`core/src/transport_vertex.cpp:487-494,504-510`;
...
dedupe rather than a second source of truth (`core/src/transport_vertex.cpp:512-520`). Because a
...
`SCHEMA_NOT_FOUND` (`core/src/transport_vertex.cpp:179`). The application declares each module
under a name it chooses through `register_module` (`core/src/transport_vertex.cpp:141`,
`core/include/libtracer/transport_vertex.hpp:357`), a minting boundary gated by the shared
...
only place that can say so (`core/include/libtracer/fwd_router.hpp:351`,
...
no liveness, and answers `BACKPRESSURE` (`core/src/transport_vertex.cpp:613-616`). Discarding that
...
(`core/src/transport_vertex.cpp:622`), so a retry after the pressure clears still finds its link.
...
reports `UP` or `LISTENING` at creation (`core/src/transport_vertex.cpp:641-644`).
...
— see [ADR-0082 — the auth subject and `peer_named` are decoupled claims](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0082-auth-subject-and-peer-named-are-decoupled-claims-default-stays-false.md).

```{doxygenstruct} tr::graph::inbound_ref_t
:project: libtracer
:members:
```
...
```{doxygenfunction} tr::net::rebuild_request_reverse_mint
:project: libtracer
```
