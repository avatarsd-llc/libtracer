...
depth cap to tune (`core/include/libtracer/graph.hpp:88-93`;
...
:class: important
`unsubscribe` **deactivates** the slot (`core/include/libtracer/graph.hpp:1784`); a
...
quiescent on return (`core/include/libtracer/graph.hpp:1742` states the bound on `ctx`).
```
...
(`core/include/libtracer/graph.hpp:1357,1563` by handle, `:2195,2201` by path;
...
```{doxygenstruct} tr::graph::stats_counter_t
:project: libtracer
:members:
```

```{doxygenstruct} tr::graph::stats_block_t
:project: libtracer
:members:
```

```{doxygentypedef} tr::graph::stats_sampler_fn_t
:project: libtracer
```
