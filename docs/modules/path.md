...
`result_t` the caller would only `*`-deref unchecked (`path.hpp:196`, defined
`:337-341`). Neither uses
...
- **The limits are a receiver's buffer budget.** ≤64 B per segment, ≤1024 B total,
  ...
  and ≤8 field steps (`core/include/libtracer/path.hpp:34,36,38,40`) let a
...
- **Ordinary names cost no heap block.** `path_key_t` holds records up to 16 bytes inline
  (`path_key_t::kInlineBytes`, `core/include/libtracer/path.hpp:358`) — a packed segment
