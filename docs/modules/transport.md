...
refcounts would lie about lifetime. `fwd_router_t::add_child` (`core/src/fwd_router.cpp:627`)
therefore branches on the link's declared capability and installs exactly one sink —
the rope form for an owning link, the span form otherwise (`fwd_router.cpp:734,671`, and
`fwd_router.cpp:688,695` for the peer-named bus equivalent).
