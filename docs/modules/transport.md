...
WebSocket server and client (`transport_ws.hpp:181,339`), CAN
(`transport_can.hpp:429`), QUIC (`transport_quic.hpp:153`) and WebTransport
...
(`transport_can.hpp:410`); the TCP and WebSocket **servers** return it when
configured peer-named (`transport_tcp.hpp:284`, `transport_ws.hpp:197`); every other
kind keeps the `nullptr` default.

## QUIC and WebTransport

Both live in the separate `libtracer_quic` target, configured by
`LIBTRACER_WITH_QUIC` (`core/CMakeLists.txt:291`, default `OFF` because msquic must
