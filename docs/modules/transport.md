...
refcounts would lie about lifetime. `fwd_router_t::add_child` (`core/src/fwd_router.cpp:747`)
therefore branches on the link's declared capability and installs exactly one sink —
the rope form for an owning link, the span form otherwise (`fwd_router.cpp:893,791`, and
`fwd_router.cpp:843,850` for the peer-named bus equivalent).
...
WebSocket server and client (`transport_ws.hpp:240,436`), CAN
...
`LIBTRACER_WITH_QUIC` (`core/CMakeLists.txt:275`, default `OFF` because msquic must
