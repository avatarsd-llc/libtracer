...
`core/include/libtracer/rope.hpp:287`); the third link spills the whole chain to the
...
and it is *debug-asserted* (`rope_t::only`, `core/include/libtracer/rope.hpp:165-171`).
...
promise contiguity calls `materialize()` (`rope.hpp:182`) instead — zero copy when the
...
returns `false` instead, leaving `out` empty (`rope.hpp:247-269`). ⚠️ The probe is not a hard
