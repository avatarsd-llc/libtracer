...
6. Forward `FWD` frames hop-by-hop per [07-host-embedding.md](07-host-embedding.md): strip the whole leading `dst` mount run — `net/<module>/<name>[/<peer>]` (RFC-0014 S2a) — and prepend the inbound link's own mount run to `src` when acting as a forwarder. Loop-freedom is by construction — `dst` shrinks monotonically, so a route is finite; there is no revisit check.
...
4. **Forwarding is core.** Any host with two transport modules loaded is a forwarder — a stateless `FWD` hop between links. The "one address space across CAN + IP + RDMA" claim is structural, not opt-in. From a subscriber's API view, transport choice is invisible. ([07-host-embedding.md](07-host-embedding.md))
   ...
   > **Statelessness, qualified.** "Stateless" describes the *bare* forward hop, which is the default and the only thing a cold or one-shot flow ever touches. It is already narrower than it reads: the route-handle plane of RFC-0004 §E.1 puts per-link label state on the hops a compact delivery flow crosses ([05-protocol-tlvs.md](05-protocol-tlvs.md) §Route-handle frames). [RFC-0027](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0027-label-switched-path-compression.md) (**accepted 2026-08-15, implemented**) narrows it further — a hop that mints a path label learns per-flow state for the flows it minted for — and both mechanisms degrade to the bare stateless hop when the state is absent. Path-label minting is **opt-in and off by default**: a node with no injected label table mints nothing and stays the bare hop, which is the shape every deployment ships until it chooses otherwise ([05-protocol-tlvs.md](05-protocol-tlvs.md) §`0x06` §path label element).
...
The wire format (L2) carries the `type` byte and `opt.PL` at fixed positions so routers can decide whether to recurse into nested children without parsing payload. The `type` byte's meaning is L3; it sits in the L2 header for routing convenience. Priority is **not** an L2 concern — it is cached at L4 from the SUBSCRIPTION's delivery policy (bits 2-4, [RFC-0022](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md) §3.A; it was a per-vertex `:settings.priority` knob until then, and nothing consumed it). The L2 `opt` byte's other bits select wire-format variants (`LL` length width, `CW` CRC width, `TF` TS form) — these are framing choices, not semantic information.
...
| Fixed-width little-endian integer load/store (`u16` and `u32`) | TLV length encoding, selected by the header's `LL` bit | trivially portable |
