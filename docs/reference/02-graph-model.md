...
**Open — child removal.** Appearance and value-change are covered above, but how a child *removal* is delivered to a composite subscriber (a tombstone delta? a `STATUS=ERROR(NOT_FOUND)` at the child's path? snapshot-diff only?) is **not yet specified**. Because it is wire-observable behavior it is an RFC/ADR-level decision, now tracked under **RFC-0009** ([#424](https://github.com/avatarsd-llc/libtracer/issues/424); the earlier [#66](https://github.com/avatarsd-llc/libtracer/issues/66) is closed). The maintainer's **position into** that comment window is that removal **collapses into absence** — a removed path simply reads `NOT_FOUND`, with no distinct removal delivery ([ADR-0059](../adr/0059-creator-endpoint-creation-and-removal-are-writes-to-a-vertex.md) §Decision 4) — but that is a **position, not a settled outcome**, so the notification semantics stay unspecified until the window closes. The `DELETE` access-mask bit ([05 §ACL](05-protocol-tlvs.md)) gates the *right* to remove but does not define the *notification*.
