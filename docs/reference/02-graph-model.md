...
**The same park serves unsubscribe** ([#894](https://github.com/avatarsd-llc/libtracer/issues/894)). A fan-out snapshots the producer's edge set under the stripe lock and dispatches **outside** it, and the one leg of the snapshot that is *not* owned by it — the subscriber's own `{fn, ctx}` pair, whose `void*` is the subscriber's object — is therefore still invoked after `unsubscribe()` returned success. The reclamation posture is identical to the seam's, and so is the mechanism: `unsubscribe()` **swap-parks** the retired pair and `graph_t::collect()` releases it, with `graph_t::parked_callback_count()` as the observability half. The contract that follows is stated on the API: *the callback may still be invoked until the first `collect()` that completes after `unsubscribe()` returns; free the `ctx` after that.* An optional per-unsubscribe release hook has `collect()` free the context itself, so the application is **signalled** rather than left to track in-flight deliveries it cannot observe. The rejected alternative — refcounting the pair so an in-flight dispatch pins it — was priced and refused: two atomic read-modify-writes **per callback per delivery**, landing worst exactly on WIDE fan-out, to buy what an application-declared quiescent point already provides for free. Reclamation safety (QSBR/hazard) is a **different axis** from capacity (WIDE/NARROW); this is the reclamation one, and on it the house pattern is *the application names the quiescent point*. The callback park carries its **own leaf lock**, never the map lock the delivery path holds shared on every write: the two parks share a doctrine, not a mutex, and guarding the unsubscribe-frequency one with the graph's widest lock starves the unsubscriber against a publishing loop — zero delivery-path cost means zero contention, not merely zero instructions.
