...
> **What the synthesized part emits** ([RFC-0022](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0022-delivery-policy-is-per-subscription-vertex-keeps-storage.md)). The synthesized part enumerates neither `:subscribers[]` nor `:settings.*`, `:liveness.*` or `:acl`. `graph_t::read_schema` emits exactly:
>
> ```
...
Rationale is expanded in [08-views-and-ownership.md](08-views-and-ownership.md) §refcount semantics. The reference implementation uses C++ `std::atomic`; implementations in C (`<stdatomic.h>`), Rust (`Arc<T>` / `AtomicUsize`), or any other language MUST implement equivalent semantics.
...
### Admission: a write is filtered before it becomes state

A write reaching a vertex passes the ACL gate, and then — if the vertex declared one — an **admission filter**, before anything is stored. The filter is the owner's, installed at registration (`handlers_t::on_admit`), and it returns one of three answers: accept the value as written, accept a **normalised** value to be stored in its place, or **refuse** with a status the writer receives.

The rule this closes is a retention/validation exclusivity. A **handler** vertex (role 3+) could always refuse a write from its `on_write` seam, but a handler retains nothing — no last-known-value, no `await` value, no composed read. A **stored value** (role 1) or **stream** (role 2) retained everything and could refuse nothing: the bytes landed and the fan-out went out before any application code saw them. An owner that wanted both had to keep a shadow handler beside the vertex that held the truth. Admission puts the refusal on the retaining roles, at the single point every store passes through, so the two properties compose.

What the placement buys, stated as observable facts:

- **A refused write is unobservable except as the writer's error.** Nothing is stored, the write sequence does not advance, no `await` wakes, no local, bubbled or remote subscriber is delivered, and the prior last-known-value stands. A delivery landing at the vertex from an inbound subscription edge is refused the same way, and counts as a *denied* delivery — the same counter the fan-in ACL denial uses, because it is the same kind of event.
- **A normalised write is observable only in its normalised form.** Storage and every delivery read what the filter returned, so the canonical form is what subscribers see, what a later read returns, and what an `await` wakes with. (The one exception is the eager notify of a **branch** write, which fans out slices cut from the written tree; a filter that normalises a branch slice changes what is retained and read there, not what that eager delivery carries.)
- **The filter is a property of the vertex, not of a door.** `write` and `assign`, a `FWD{WRITE}` terminus, an inbound edge's delivery and each landing site of a branch decomposition all reach storage through one call, and all of them are filtered. The owner's own writes included — they are distinguished by `write_ctx_t::is_local_owner`, not exempted, because an invariant that only held against remote writers would not hold.
- **A vertex that declares no filter pays nothing**: one bit of an already-hot flags word decides it. The filters themselves live on the graph, in the same insert-only declaration list the payload-right rows use, so no per-vertex structure grows for them — not the value-seam block, and not the app-field group.

The same seam exists on the **app-field plane** (`handlers_t::on_app_field_admit`, [RFC-0010](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0010-owner-app-fields-and-schema.md) §A.3): it runs below the ACL and writability gates and above the store, and may refuse or normalise the field's bytes. A refused field write leaves the field's prior bytes in place and does not fire the apply seam. This does not make the descriptor table a validation schema — the runtime still validates nothing against it, and a vertex with no filter still stores verbatim; it gives the *owner* somewhere to say no before the bytes land, instead of only after.
...
- **A LOCAL data write creates its target** (`mkdir -p`) when the vertex does not exist, gated by the existing `CREATE` access bit on the nearest existing ancestor's effective ACL. A **remote** fieldless `FWD{WRITE}` whose `dst` resolves to nothing does **not** create: it answers `tr::path::not_found`, the same as `:field` writes, `read` and `await` ([RFC-0005](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0005-subtree-subscriptions.md) §D amendment 1). A peer creates through the creator endpoint ([ADR-0059](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0059-creator-endpoint-creation-and-removal-are-writes-to-a-vertex.md)), where creation is typed, catalogued and ACL-gated; the in-process owner of a graph keeps building its own structure by writing to it.
...
**The FOLD emission mode** ([RFC-0025](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0025-stream-class-values.md) §4.1.2, Amendment 3 clause 5) is the producer's alternative to that one-per-vertex fan-out, and it is **opt-in per call**: `propagate(v, emission_mode_t::FOLD)` emits **one branch-write frame** for the swept subtree — the folded `POINT` tree of the selection, node shape byte-for-byte [RFC-0005](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0005-subtree-subscriptions.md) §B's, the root carrying its own leading `NAME` — where the default emits N. Selection is identical in both modes; only the framing differs, and `propagate(v)` is unchanged. The terminus is untouched: §B's decomposition already hands each covered subscription point the smallest subview covering every value at-or-below it, so a covered leaf sees **byte-identical** bytes either way. It is **one frame per subtree**, never a container spanning several — two disjoint subtrees are two calls and two frames.
...
- **Writes**: the owner always reads and writes its own declared fields (`ro`/`wo` constrain remote callers). A caller-attributed write is admitted iff the caller holds the ordinary WRITE right on the vertex (otherwise `tr::access::denied` — evaluated **first**, before any name under `settings.app.` is resolved, so a denied caller cannot probe which owner names exist; [RFC-0010](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0010-owner-app-fields-and-schema.md) §Erratum 2026-08-12) — and the field is declared `rw`/`wo` (otherwise `SCHEMA_NOT_FOUND`, the answer every admitted caller gets, identically). The value is stored **verbatim**: the runtime performs no dtype/range validation against the descriptor (self-description for consumers; semantic validation is the owner's). The owner may take that validation *before* the store rather than after it, by installing the admission filter `on_app_field_admit` (§Admission above), which refuses or normalises the write; absent one, the bytes land exactly as written and the apply step is the only place the owner sees them. Field writes to a nonexistent vertex do not create it.
...
Delivery of a removal is neither parked nor unspecified: **no removal-push surface exists, normatively** ([#937](https://github.com/avatarsd-llc/libtracer/issues/937), ruled 2026-08-12 — #66's option 3, striking the deferral [RFC-0005](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0005-subtree-subscriptions.md) §E carried). The silence stated above is the whole of the contract: a tombstone delta, a `NOT_FOUND` delivered at the child's path, and a pushed snapshot diff were each considered and none exists ([RFC-0009](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0009-vertex-removal-and-subscriber-eviction.md) §Alternatives; §B.5 forbids the mechanism outright). Anyone building a topology view over the graph model re-reads — enumerating `:children[]`, or diffing a composed branch read ([RFC-0016](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0016-composed-branch-read.md)) — and carries its own expectation of what should exist, per the paragraph above. The ruling is revisited only when a real consumer (the [#58](https://github.com/avatarsd-llc/libtracer/issues/58) reconciler, or the SPA) demonstrates the need; because RFC-0009 §B.5 forbids the mechanism, any future push surface is an amendment (a new RFC).
