...
A subtree subscriber receives TLVs produced at many concrete paths and may need to know which vertex produced each. Provenance travels **in the data** where the application needs it ([CONTEXT.md](../../CONTEXT.md) §SUBSCRIBER direction); for **local** delivery the concrete path is additionally available out-of-band (implementation-defined — typically a callback argument). Wire-level concrete-path tagging of **remote** deliveries was proposed as [RFC-0003](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0003-bridged-wildcard-delivery-path.md), which is **closed as superseded** (2026-08-24, [#303](https://github.com/avatarsd-llc/libtracer/issues/303)) — not a draft awaiting a comment window: its carrier was the `ROUTER` envelope, which [ADR-0040](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0040-net-plane-is-explicit-source-routed-only.md) left reserved and MUST-NOT-emit ([05-protocol-tlvs.md](05-protocol-tlvs.md) §`0x0D`); its trigger was a wildcard subscription, which has no grammar to spell (§the path grammar above); and its producer-pays cost shape was ruled the other way by receiver-pays. For a **remote operation** the provenance is inherent instead: a `FWD`'s accumulated `src` is the complete source route home ([RFC-0004](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0004-remote-operation-addressing.md) §B — *"exactly the provenance RFC-0003 wanted, now inherent"*). Should a concrete bridged-wildcard interop need appear, it takes a **new RFC with a new carrier**, not a revival of this one.
...
Each subsequent `write("/camera/frame/<i>", ...)` bubbles to the parent's subscription and produces a delivery to `/local/handler`, with the slice index recoverable from the producing path (out-of-band for local delivery; the [RFC-0003](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0003-bridged-wildcard-delivery-path.md) on-wire tagging proposal for remote delivery is **closed as superseded** — see [§subscriber identity across a subtree](#subscriber-identity-across-a-subtree) for what carries provenance instead).
