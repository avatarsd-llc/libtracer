...
- Maximum **segment depth**: 32 (an addressing limit on PATH construction; the TLV parser itself has no depth cap — nesting is receiver-resource-bounded per [RFC-0006](../spec/rfcs/0006-resource-bounded-nesting-depth.md)).
...
- `write("/x:subscribers[3]", …)` is resolved by its **payload** (RFC-0009 §D.1): an empty `STATUS` clears slot 3, a `SUBSCRIBER` replaces its edge, anything else is `TYPE_MISMATCH`. `:subscribers[*]` on a write is `INVALID_PATH`. See [02 §writing `:subscribers[N]`](02-graph-model.md#writing-subscribersn-is-payload-discriminating).
