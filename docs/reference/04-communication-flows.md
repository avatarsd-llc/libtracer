...
- **Forward hop** — the leading `dst` segments match one of this node's transport mount runs. The hop reads roughly three TLV headers **by offset**, with no decoded tree; a **single-link (contiguous)** frame gathers into a stack array of layout-derived size and allocates nothing on the hop (CI-gated at zero allocations for that arm; the gate covers the hop's own work against a stub link, not the shipping transport's `iovec` table, which spills to the heap above 16 spans), while a **multi-link rope** frame draws its scatter-gather table from the injected receive source, because the sub-span count is the sender's choice and is known only at run time. It strips that whole leading mount run from `dst`, prepends the inbound link's own mount run to `src`, and scatter-gather-sends the result: stack-built replacement heads plus untouched views over the original frame bytes.
...
- **Forwarders are stateless.** There is no per-request table: the forward route is the shrinking `dst` and the return route is the growing `src`, both carried in the frame. A hop may reboot mid-operation and the reply routes regardless. *Scope of the claim: it is about **per-request** state, and it holds for every bare `FWD` hop. A hop crossed by a compact delivery flow does hold per-link label state (RFC-0004 §E.1; [05-protocol-tlvs.md](05-protocol-tlvs.md) §Route-handle frames), and [RFC-0027](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0027-label-switched-path-compression.md) (**accepted 2026-08-15, implemented**) adds per-flow path-label state on hops that mint — **opt-in and off by default**, so a hop with no injected label table holds none of it ([05-protocol-tlvs.md](05-protocol-tlvs.md) §`0x06` §path label element). Both are per-flow, never per-request, and both fall back to the route in the frame when absent — so the reboot property above survives either. A labelled hop's fallback is not free-of-charge repair: the frame no longer carries that hop's names, so a stale label costs one refused operation and the origin re-sends against the string it still holds.*
...
- **Exhaustion is a value.** A rejected operation MUST return an error; it MUST NOT throw, and on a build without exceptions it MUST NOT abort. This is what makes a bounded node's memory ceiling the host's injected resource rather than a hidden global heap. ⚠️ *The reference implementation does not yet meet this everywhere*: the route-handle label tables ([#603](https://github.com/avatarsd-llc/libtracer/issues/603)) and the second step of the `try_reserve` probe ([#850](https://github.com/avatarsd-llc/libtracer/issues/850)) still allocate through a throwing path, so a `-fno-exceptions` node can still abort on those two. (The CAN egress window vector was a third until [#1110](https://github.com/avatarsd-llc/libtracer/issues/1110) deleted the table outright — `view_can.hpp` derives each window from the payload length, the mode and the index, so the framing allocates nothing and cannot fail.) The rule stands; the two residuals are conformance gaps, not amendments to it.
