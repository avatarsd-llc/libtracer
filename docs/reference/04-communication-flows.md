...
- **Exhaustion is a value.** A rejected operation MUST return an error; it MUST NOT throw, and on a build without exceptions it MUST NOT abort. This is what makes a bounded node's memory ceiling the host's injected resource rather than a hidden global heap. ⚠️ *The reference implementation does not yet meet this everywhere*: the CAN egress window vector (`view_can.hpp` `split`), the route-handle label tables ([#603](https://github.com/avatarsd-llc/libtracer/issues/603)) and the second step of the `try_reserve` probe ([#850](https://github.com/avatarsd-llc/libtracer/issues/850)) still allocate through a throwing path, so a `-fno-exceptions` node can still abort on those three. The rule stands; the three residuals are conformance gaps, not amendments to it.
