...
- the `src` **grows canonically**, by the full mount run for the link the frame arrived on, exactly as it grows on a canonical forward. A bound path changes how the *forward* address is spelled and nothing about the return route, so the reply routes home through the ordinary descent and every hop on the way back may be a peer that does not implement the bound form at all.
...
- **One direction is bound, and only one.** A mint binds the route the request took, from the origin outwards. There is no reverse-direction mint on the wire: a responder learns nothing about the route home beyond the canonical `src` it already receives, and a return route stays canonical. [RFC-0024](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0024-bound-paths-node-scoped-vertex-ref-source-routing.md) §7.1's reverse-direction option is ratified prose with **no spelling** and is deliberately **not incorporated** here — see its erratum 2, which records why every candidate spelling would contradict the "zero added request bytes" rule above or the `src`-accumulates-canonically hop rule, and is therefore a further amendment rather than an implementation.
