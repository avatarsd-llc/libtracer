...
- **Thread-safety.** Registration is safe to call concurrently with reads, writes and awaits on other addresses. A **handle**-addressed read or write resolves without taking the graph's lock; a **path**-addressed call resolves the path under it. A write bumps the write sequence without a lock and takes a short per-vertex lock only to wake parked waiters — and skips even that when no waiter is parked. How much serialization the value slot itself costs is an implementation choice: see [15-concurrency-and-scaling.md](15-concurrency-and-scaling.md) §4 and, for this implementation's figures with their host and spread, [../design/concurrency/](../design/concurrency/README.md).
