...
`flatten` is the lossy spelling — its empty view means "allocator refused", "DEVICE link", or "this rope really is zero bytes", and a caller cannot tell which. `try_flatten` (and its `materialize` twin, `try_materialize`) is the same copy with the cause kept: `std::expected<view_t, flatten_err_t>`, where `NO_MEMORY` is *transient* (this node is out of memory; a retry may succeed) and `NOT_HOST` is *permanent for this rope* (send the payload via its device path instead). A zero-length rope is a **success** carrying an empty view, and costs no allocation. Any caller whose next move differs between those verdicts — reply-vs-drop, warn-vs-reject, blame-this-node-vs-blame-the-peer — must use the `try_` form; collapsing them is how a local OOM gets reported to a peer as a malformed frame.
...
| `flatten` is failable. | Its failure is an empty view, not an exception. On a target built without exceptions this is the only signal there is, and an unchecked result is a dropped payload. And the empty view is ambiguous — allocator refusal, DEVICE link, or a genuinely empty rope — so a caller that reads it as one specific cause reports the wrong one: reading it as "malformed frame" turns this node's OOM into a permanent accusation against a peer that sent a valid frame. `try_flatten` / `try_materialize` name the cause. |
