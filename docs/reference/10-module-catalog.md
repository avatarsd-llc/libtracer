...
**The bus tier is its own module.** Peer-named links — per-peer addressing, in-band peer enumeration, the peer-named receiver and the two peer-lifecycle notifiers ([ADR-0044 — stateless transport peer enumeration](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0044-stateless-transport-peer-enumeration-separate-paths-client-side-identity.md)) — are selected independently of *which* transports a node loads, because whether a `tcp`/`ws` listener is peer-named is a wiring-time choice inside a translation unit a point-to-point node still compiles for its flat half. So the selector is a configuration member rather than a translation-unit list: `tr::graph::default_config_t::kBusLinks`, bound `false` by an override fragment, closes the tier out — measured **−2,078 B of flash** and **±0 B of `.bss`** on rv32 (`-Os`, GCC 15.2), and asking for a bus on such a build is refused at build time (`transport_can`) or at bring-up (`ok() == false`, `TYPE_MISMATCH` from the SPEC factory) rather than quietly served flat. This is [ADR-0047](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0047-build-time-closed-module-sets-compile-time-seams.md) §1's link-time module selection expressed through §Consequences' configuration surface; §4's ruling that peer wiring stays a *virtual* is untouched, and nothing here is templated over a transport list. See [modules/transport.md](https://github.com/avatarsd-llc/libtracer/blob/main/docs/modules/transport.md) §"Closing the bus module out at build time".
...
**The sinks are only "set before frames flow" if something gives the owner that window.** A transport that connects and starts its receive loop in one step does not: a DIAL link's peer is provoked into pushing by our own connect, so its first frame can be decoded while the owner is still installing sinks, and an empty sink drops it with no counter moving. So the bring-up is two-phase — create (connect, handshake), wire (sinks, down-notifier, and the forwarder's registry entry), then *start receiving* — and the start step is a no-op for any transport that was already receiving, so the owner never has to know which kinds defer. The reference implementation's `ws` DIAL connection is the case that needs it ([#1025](https://github.com/avatarsd-llc/libtracer/issues/1025)). Provocation is not what makes the window real, only what makes it easy to see: on a **bus** any bystander traffic already on the wire lands in it, and there is neither a dial to defer nor a per-peer flow-control window to hold bytes in. Where holding is impossible the ruling is drop-and-name — the frame is dropped and a distinctly named counter ticks, never parked inside the library ([ADR-0081](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0081-pre-sink-ingress-native-window-hold-or-named-drop-never-parked.md), [#1103](https://github.com/avatarsd-llc/libtracer/issues/1103)).
