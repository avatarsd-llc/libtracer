...
- **State size**: bounded in **bytes** by **this vertex's own** injected `mem::block_source_t` — never a magic number and never a pool shared with another vertex or another plane ([ADR-0079](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0079-allocation-store-composition-defaults-to-per-plane-mid.md) Amendment). The retention *intent* on top of that bound is set owner-side by the host call `graph_t::set_history_depth(v, keep)` and has **no wire surface** — it is not a `:settings` knob ([02-graph-model.md](02-graph-model.md) §storage is declared owner-side, and nothing is inherited); the protocol fixes no depth. A shortfall — an intent the source cannot fund — surfaces as a shed-with-gap or as backpressure, never as a silent shrink.
- **Who makes a vertex a STREAM**: **the party that wants a queue, on its own receiving vertex** ([RFC-0025](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0025-stream-class-values.md) §4.6.1, Amendment 2). A producer never queues — its writes are always the lock-free path, on every delivery class and at every depth. A subscriber wanting history subscribes as always and makes its **target** vertex a STREAM; nothing is requested from the producer and no depth crosses the wire. The producer's contribution is knowing the consumer's sampling rate and shaping its traffic to it — for which batch folding is the preferred carrier (32.0x per-sample amortization; with a uniform stream descriptor, per-sample time costs 0 bytes).
...
- **The surface an implementation meets.** A connection is created by a `SPEC{name, config}` write to the **module's creator endpoint** `/net/<module>/conn`. The created connection vertex is mounted — and routed — at **`/net/<module>/<name>`**, where `<module>` is the module name the **application** declared for that (kind, role) pair via `register_module` — never a library-derived name, and an undeclared pair is refused with `SCHEMA_NOT_FOUND` ([ADR-0061](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0061-per-transport-mount-routing-strip-k-l5-demux.md), [ADR-0073](https://github.com/avatarsd-llc/libtracer/blob/main/docs/adr/0073-naming-authority-the-application-mints-one-predicate-gates.md) §4). The `kind` config key still names the transport factory, but it no longer *selects* the module: the endpoint's path already did that, so `kind` is a cross-check on the module's declaration.
- **The accepted direction, and where it stands.** [RFC-0014](https://github.com/avatarsd-llc/libtracer/blob/main/docs/spec/rfcs/0014-creator-endpoint-connection-lifecycle-and-link-liveness.md) replaced the single global catalog with that per-module creator endpoint: `SPEC{name, config}` creates `/net/<module>/<name>` (gated `CREATE`), `NAME{<name>}` retires it (gated `WRITE`), and a read of `conn:schema` *is* that module's catalog. The role is positional — it is the module — rather than a config field. **The endpoint is implemented** (S2b): declaring a module mints it, and `SPEC`/`NAME` writes to it create and remove connections with the role fixed by the module. It is also **hidden** from `/net/<module>:children[]` (S4) — that listing returns the module's member connections, and the endpoint is the control that creates them, not one of them; it stays addressable, which is what §6's `read <module>/conn:schema` creatability probe depends on. And the global `/net:children[]` *creation* catalog is **retired** (S7): the `client` and `listener` child types are no longer registered and the `role` config key is no longer read, so the endpoint is the one door in. Still open: the `conn:schema` catalog read (S3) and the `CREATE`/`WRITE` gating split (S2c) — today the endpoint is gated by the ordinary vertex `WRITE` right.
- **What did not retire.** `:children[]` as an **enumeration** is a read and is untouched: `/net:children[]` lists the modules, `/net/<module>:children[]` lists that module's connections, and a bus connection's `:children[]` lists its live peers. `:children[]` *creation* also still exists generically — for `stored_value` and for any type an application registers itself with `graph_t::register_child_type`. Only the two connection types died.
