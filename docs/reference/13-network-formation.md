...
residual below it (`graph_t::subscribe_wire`, `core/src/graph.cpp:3470`), so
...
- **Re-`SPEC`ing to reconfigure.** A `SPEC` naming an existing connection is rejected
  ...
  caller-independently (`core/src/graph.cpp:3784`), leaving only the read container and
  its reserved `app` subkey (`core/src/graph.cpp:3983`). Moving a peer therefore means
