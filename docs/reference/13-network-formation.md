...
residual below it (`graph_t::subscribe_wire`, `core/src/graph.cpp:2959`), so
...
- **Re-`SPEC`ing to reconfigure.** A `SPEC` naming an existing connection is rejected
  ...
  caller-independently (`core/src/graph.cpp:3241`), leaving only the read container and
  its reserved `app` subkey (`core/src/graph.cpp:3439`). Moving a peer therefore means
