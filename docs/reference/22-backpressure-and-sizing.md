...
| --- | --- |
| Local + remote `READ` | a leaf read serves the stored pointer (`core/src/graph.cpp:1900`; the `FWD{READ}` terminus is the same call) — null ⇒ `NOT_FOUND`, unless the vertex composes an answer from its `on_read` seam (`core/src/graph.cpp:1866`) |
| `await`'s return value | the wake rides the write sequence and the stripe condvar (retention-free), but the value handed back is served through the **same role dispatch** `read` runs (`core/src/graph.cpp:2798`) |
| `assign` / `propagate` sweep | **the hard dependency** — RFC-0008 §C: `propagate` takes no value argument, "the last-known-value is the single source of truth" (`core/src/graph.cpp:2542`) |
| Composed subtree reads | RFC-0016 serves **landed** LKVs only, one atomic load per node (`core/src/graph.cpp:4072`); a non-retaining child contributes nothing |
...
storing role delivers the just-published pointer (`core/src/graph.cpp:2342`); a HANDLER delivers
from the incoming value (`core/src/graph.cpp:2286`). If subscribers are all a vertex has, it does
not need to retain.
...
- **`assign` and `propagate` refuse a non-retaining vertex with `SCHEMA_NOT_FOUND`**
  (`core/src/graph.cpp:2377`, `core/src/graph.cpp:2559-2565`) — the taxonomy's contract-mismatch
...
2. **HANDLER is the CHEAPER role, at every fan-out and every value shape.** This caveat used to
   ...
   delivered directly (`core/src/graph.cpp:2286`). Measured x86-64 `-O3`, p50 ns per write:
