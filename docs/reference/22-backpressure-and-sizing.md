...
| **graph write** | the ACL gate, plus the value backend the store draws its durable bytes from | `graph_t`'s four injected seams (see the design companion) | `PERMISSION_DENIED` by value; an exhausted value store answers `BACKPRESSURE` | `graph_t::delivery_drops()` — `denied`, `out_of_memory` (`core/include/libtracer/graph.hpp:2329`) |
| **ring admission** | a **byte** budget: `try_alloc(retained_bytes)` against the receiving vertex's own source | `graph_t::set_ring_source` (`core/include/libtracer/graph.hpp:1506`), per vertex, never a shared pool | **arm-dependent** — see §2 | `ring_reserved_bytes()` / `stream_gaps()` (`core/include/libtracer/graph.hpp:1510`) |
...
at wiring time and read at admission (`core/include/libtracer/vertex.hpp:1222`,
...
- Note the one cost a pooled RX backend adds: a **pinned** value borrows its whole inbound
  ...
  the number of values (`core/include/libtracer/graph.hpp:1528`). Declaring a non-sentinel ratio
...
| --- | --- |
| Local + remote `READ` | a leaf read serves the stored pointer (`core/src/graph.cpp:1929`; the `FWD{READ}` terminus is the same call) — null ⇒ `NOT_FOUND`, unless the vertex composes an answer from its `on_read` seam (`core/src/graph.cpp:1895`) |
| `await`'s return value | the wake rides the write sequence and the stripe condvar (retention-free), but the value handed back is served through the **same role dispatch** `read` runs (`core/src/graph.cpp:2827`) |
| `assign` / `propagate` sweep | **the hard dependency** — RFC-0008 §C: `propagate` takes no value argument, "the last-known-value is the single source of truth" (`core/src/graph.cpp:2571`) |
| Composed subtree reads | RFC-0016 serves **landed** LKVs only, one atomic load per node (`core/src/graph.cpp:4101`); a non-retaining child contributes nothing |
| Late-joiner replay | the durability latch snapshots the LKV at edge-add (RFC-0022 §3.A bit 5, `core/include/libtracer/vertex.hpp:1470`) |

**Not on the list: the whole callback / delivery plane.** Fan-out never reads the slot. A
storing role delivers the just-published pointer (`core/src/graph.cpp:2371`); a HANDLER delivers
from the incoming value (`core/src/graph.cpp:2315`). If subscribers are all a vertex has, it does
not need to retain.
...
- **`assign` and `propagate` refuse a non-retaining vertex with `SCHEMA_NOT_FOUND`**
  (`core/src/graph.cpp:2406`, `core/src/graph.cpp:2588-2594`) — the taxonomy's contract-mismatch
...
2. **HANDLER is the CHEAPER role, at every fan-out and every value shape.** This caveat used to
   ...
   delivered directly (`core/src/graph.cpp:2315`). Measured x86-64 `-O3`, p50 ns per write:
