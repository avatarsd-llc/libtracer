...
| **graph write** | the ACL gate, plus the value backend the store draws its durable bytes from | `graph_t`'s four injected seams (see the design companion) | `PERMISSION_DENIED` by value; an exhausted value store answers `BACKPRESSURE` | `graph_t::delivery_drops()` — `denied`, `out_of_memory` (`core/include/libtracer/graph.hpp:2312`) |
| **ring admission** | a **byte** budget: `try_alloc(retained_bytes)` against the receiving vertex's own source | `graph_t::set_ring_source` (`core/include/libtracer/graph.hpp:1489`), per vertex, never a shared pool | **arm-dependent** — see §2 | `ring_reserved_bytes()` / `stream_gaps()` (`core/include/libtracer/graph.hpp:1493`) |
...
at wiring time and read at admission (`core/include/libtracer/vertex.hpp:1210`,
...
- Note the one cost a pooled RX backend adds: a **pinned** value borrows its whole inbound
  ...
  the number of values (`core/include/libtracer/graph.hpp:1511`). Declaring a non-sentinel ratio
...
- **Naming the seams**: `label_source()`, `rx_source()`, `flatten_backend()` and
  `egress_backend()` (`core/include/libtracer/fwd_router.hpp:411`) reach the four injected
...
| --- | --- |
| Local + remote `READ` | a leaf read serves the stored pointer (`core/src/graph.cpp:1865`; the `FWD{READ}` terminus is the same call) — null ⇒ `NOT_FOUND`, unless the vertex composes an answer from its `on_read` seam (`core/src/graph.cpp:1831`) |
| `await`'s return value | the wake rides the write sequence and the stripe condvar (retention-free), but the value handed back is served through the **same role dispatch** `read` runs (`core/src/graph.cpp:2771`) |
| `assign` / `propagate` sweep | **the hard dependency** — RFC-0008 §C: `propagate` takes no value argument, "the last-known-value is the single source of truth" (`core/src/graph.cpp:2515`) |
| Composed subtree reads | RFC-0016 serves **landed** LKVs only, one atomic load per node (`core/src/graph.cpp:4045`); a non-retaining child contributes nothing |
| Late-joiner replay | the durability latch snapshots the LKV at edge-add (RFC-0022 §3.A bit 5, `core/include/libtracer/vertex.hpp:1458`) |

**Not on the list: the whole callback / delivery plane.** Fan-out never reads the slot. A
storing role delivers the just-published pointer (`core/src/graph.cpp:2315`); a HANDLER delivers
from the incoming value (`core/src/graph.cpp:2259`). If subscribers are all a vertex has, it does
not need to retain.
...
- **`assign` and `propagate` refuse a non-retaining vertex with `SCHEMA_NOT_FOUND`**
  (`core/src/graph.cpp:2350`, `core/src/graph.cpp:2532-2538`) — the taxonomy's contract-mismatch
...
2. **HANDLER is the CHEAPER role, at every fan-out and every value shape.** This caveat used to
   ...
   delivered directly (`core/src/graph.cpp:2259`). Measured x86-64 `-O3`, p50 ns per write:
...
5. **Re-run with the bounds armed and assert `refused == 0` on every stage except the designated
   ...
   (`core/include/libtracer/fwd_router.hpp:388`) is the one to assert zero against when sizing
