...
| **TX pool** | outstanding sends in flight, plus a reserve slots deep held back for replies | the link's ctor (`tx_slot_capacity()` + `tx_reply_reserve()` on the ESP httpd link, `integrations/esp-idf/libtracer/httpd_ws_link.cpp:3124`) | counted `dropped_tx`; a refused enqueue names the queue it could not enter | `transport_t::drop_stats()`; the link's own richer `stats()` where it has one |
...
- On the ESP httpd link the reply leg does not compete for the pool at all (#1494): a reply
  ...
  (`integrations/esp-idf/libtracer/httpd_ws_link.cpp:2967`). So a fan-out sweep of any width, a
