...
at wiring time and read at admission (`core/include/libtracer/vertex.hpp:1168`,
...
- Note the one cost a pooled RX backend adds: a **pinned** value borrows its whole inbound
  ...
  the number of values (`core/include/libtracer/graph.hpp:1510`). Declaring a non-sentinel ratio
...
| `await`'s return value | the wake rides the write sequence and the stripe condvar (retention-free), but the value handed back is served through the **same role dispatch** `read` runs (`core/src/graph.cpp:2734`) |
| `assign` / `propagate` sweep | **the hard dependency** — RFC-0008 §C: `propagate` takes no value argument, "the last-known-value is the single source of truth" (`core/src/graph.cpp:2478`) |
| Composed subtree reads | RFC-0016 serves **landed** LKVs only, one atomic load per node (`core/src/graph.cpp:4008`); a non-retaining child contributes nothing |
| Late-joiner replay | the durability latch snapshots the LKV at edge-add (RFC-0022 §3.A bit 5, `core/include/libtracer/vertex.hpp:1416`) |

**Not on the list: the whole callback / delivery plane.** Fan-out never reads the slot. A
storing role delivers the just-published pointer (`core/src/graph.cpp:2278`); a HANDLER delivers
from the incoming value (`core/src/graph.cpp:2222`). If subscribers are all a vertex has, it does
not need to retain.
...
- **`assign` and `propagate` refuse a non-retaining vertex with `SCHEMA_NOT_FOUND`**
  (`core/src/graph.cpp:2313`, `core/src/graph.cpp:2495-2501`) — the taxonomy's contract-mismatch
...
2. **HANDLER is the CHEAPER role, at every fan-out and every value shape.** This caveat used to
   ...
   delivered directly (`core/src/graph.cpp:2222`). Measured x86-64 `-O3`, p50 ns per write:
