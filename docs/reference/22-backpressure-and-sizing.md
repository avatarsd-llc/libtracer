...
at wiring time and read at admission (`core/include/libtracer/vertex.hpp:1231`,
...
- **Naming the seams**: `label_source()`, `rx_source()`, `flatten_backend()` and
  `egress_backend()` (`core/include/libtracer/fwd_router.hpp:412`) reach the four injected
...
| `await`'s return value | the wake rides the write sequence and the stripe condvar (retention-free), but the value handed back is served through the **same role dispatch** `read` runs (`core/src/graph.cpp:2864`) |
| `assign` / `propagate` sweep | **the hard dependency** — RFC-0008 §C: `propagate` takes no value argument, "the last-known-value is the single source of truth" (`core/src/graph.cpp:2581`) |
| Composed subtree reads | RFC-0016 serves **landed** LKVs only, one atomic load per node (`core/src/graph.cpp:4138`); a non-retaining child contributes nothing |
...
storing role delivers the just-published pointer (`core/src/graph.cpp:2379`); a HANDLER delivers
from the incoming value (`core/src/graph.cpp:2323`). If subscribers are all a vertex has, it does
not need to retain.
...
- **`assign` and `propagate` refuse a non-retaining vertex with `SCHEMA_NOT_FOUND`**
  (`core/src/graph.cpp:2416`, `core/src/graph.cpp:2598-2604`) — the taxonomy's contract-mismatch
...
2. **HANDLER is the CHEAPER role, at every fan-out and every value shape.** This caveat used to
   ...
   delivered directly (`core/src/graph.cpp:2323`). Measured x86-64 `-O3`, p50 ns per write:
...
5. **Re-run with the bounds armed and assert `refused == 0` on every stage except the designated
   ...
   (`core/include/libtracer/fwd_router.hpp:389`) is the one to assert zero against when sizing
