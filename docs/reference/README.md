...
| [22-backpressure-and-sizing.md](22-backpressure-and-sizing.md) | cross-cutting | How a deployment wires every bound so the bottleneck is *chosen*, never discovered: the pipeline map (every bounded stage from transport RX to TX pool, its capacity source, its refusal shape and the member that observes it); the two pressure arms (reliable = refuse and answer `BACKPRESSURE`; best-effort = shed oldest, count, gap-mark) picked per flow rather than per node; five deployment archetypes (bounded MCU node, hosted gateway, bulk-ingest producer, mixed control + stream, high-rate acquisition) each naming its designated pressure point, the last of them Recipe C's user-orchestrated **compose → swap → push** batching; the retention decision table (the five planes that consume the last-known-value, and when a vertex should be a HANDLER instead); the measure-then-size loop against the shipped introspection accessors; two monitoring recipes for the unacknowledged-ingest case; and the standing rulings a sizing fix must never violate. |
...
```{toctree}
:caption: Operating a deployment
:hidden:
:maxdepth: 1

Backpressure & sizing <22-backpressure-and-sizing>
```
