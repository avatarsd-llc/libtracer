...
| [07-host-embedding.md](07-host-embedding.md) | L4 | Per-host DAG (own vertices plus transport-vertex links); global topology (any shape, cycles allowed); loop safety by explicit source routes (`dst`-monotonicity, `INVALID_PATH` on revisit); node identity; embedding examples (RC car, robot, fleet, mesh, WAN). |
