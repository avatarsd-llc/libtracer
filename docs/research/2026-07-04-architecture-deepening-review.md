...
5. **Depth-cap silent truncation**: a fan-out chain beyond depth 32 is dropped while the originating `write()` reports success (`graph.cpp:441`) — interface honesty question (error? counter? documented-as-is?).
