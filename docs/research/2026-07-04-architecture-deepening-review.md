...
| 3 | ACL evaluation inlined in `graph.cpp` anon namespace; promised `security_acl` seam absent; runs per gated data op with ancestor mutex-walk + clock read; "control-plane frequency" comment wrong (`graph.hpp:277`) | **[ADR-0050](../adr/0050-acl-pure-policy-cached-effective-ace-merge.md)** — pure per-target policy (ALLOW-only MCU / full-DENY host) + graph-side cached effective-ACE merge with generation invalidation |
