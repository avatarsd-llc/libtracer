...
- **I3 — subscription's split identity.** The `SUBSCRIBER` edge is folded bytes on the producer, but its retained return-route segment + inbound link + listener counters are overlay state (05 §producer fan-out). *A subscribed vertex is therefore half-folded, half-overlay — which half is the source of truth on reconnect / unsubscribe / persistence replay (ADR-0080 TLV-replay)?*
