...
- **The current creation path is the superseded spelling.** `transport_vertex_t` registers ONE
  ...
  `/net` (`graph.cpp:1573`), and the `quic` module extends the *same* catalog via
...
> `:children[]` target, `:schema` is whole-vertex-only (`graph.cpp:1894`, `size()==1`), `set_link_state`
...
- **`SPEC` (create) gates on `CREATE` (0x08).** `CREATE` already gates `:children[]` creation and
  write-creates (`graph.cpp:1390`, `:475`, `:945`). This RFC **relocates** that gate onto the
