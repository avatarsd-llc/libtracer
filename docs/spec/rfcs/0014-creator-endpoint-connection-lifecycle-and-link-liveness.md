...
- **The current creation path is the superseded spelling.** `transport_vertex_t` registers ONE
  ...
  `/net` (`graph.cpp:1536`), and the `quic` module extends the *same* catalog via
...
> `:children[]` target, `:schema` is whole-vertex-only (`graph.cpp:1857`, `size()==1`), `set_link_state`
...
- **`SPEC` (create) gates on `CREATE` (0x08).** `CREATE` already gates `:children[]` creation and
  write-creates (`graph.cpp:1353`, `:438`, `:908`). This RFC **relocates** that gate onto the
