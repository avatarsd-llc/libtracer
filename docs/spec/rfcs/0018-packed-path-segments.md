...
(`core/include/libtracer/fwd_frame_view.hpp:95`, loop guard at `:174` and `:207`), and
`rebuild_fwd_forward` forwards the residual `dst` as **one untouched span**
(`fwd_frame_view.hpp:762-763`, emitted once by `gather` at `:601`). Segments 5..13 are never
parsed at any hop.
...
`COMPACT`, and the `COMPACT` terminus **never** replies (`core/src/fwd_router.cpp:1055-1089`).
...
- `path_lookup_key`'s re-emit fallback (`core/src/op_resolve_walk.hpp:660-669`), whose fast path
...
`resolve_route_vertex` (`core/src/fwd_router.cpp:1151-1153`) resolves an ADVERTISE route, so a peer
...
- **Zero-copy strip-K.** `rebuild_fwd_forward` advances `p += 1 + len` exactly as well as it
  ...
  `PATH` header (`fwd_frame_view.hpp:762-763`, `gather` at `:601`). No length table is rewritten.
...
- **Per-segment `TS`/`CRC` trailers.** Nothing emits them — `encode_mount_tlv`
  (`fwd_frame_view.hpp:828-852`) always writes `opt = 0` — and the `PATH`'s own trailer still
...
**Vertex-map key.** The graph key **is** the `PATH` body (`path.hpp:114,124`; `graph.cpp:1627`;
