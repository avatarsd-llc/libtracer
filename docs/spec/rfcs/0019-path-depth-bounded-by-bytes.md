...
- `core/src/frame.cpp:166-179` (`wire::path_key`) sums child lengths only to reserve, then emits.
  No cap test.
- `core/src/op_resolve_walk.hpp:624-633` (`path_lookup_key`) rejects a non-`NAME` child and counts
  nothing.
...
comparisons at `core/src/path.cpp:129` and `core/src/op_resolve_walk.hpp:379`. It almost certainly
