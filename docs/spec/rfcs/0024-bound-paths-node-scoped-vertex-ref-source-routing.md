...
vertex tree (`core/include/libtracer/graph.hpp:1223-1234`) — a tree of non-moving `unique_ptr`
...
- It is **append-only**, which the registration path already is ("vertices are added, never
  erased", `graph.hpp:1226`), so it introduces no new lifetime rule and no new invalidation event.
