...
vertex tree (`core/include/libtracer/graph.hpp:1173-1184`) — a tree of non-moving `unique_ptr`
...
- It is **append-only**, which the registration path already is ("vertices are added, never
  erased", `graph.hpp:1176`), so it introduces no new lifetime rule and no new invalidation event.
