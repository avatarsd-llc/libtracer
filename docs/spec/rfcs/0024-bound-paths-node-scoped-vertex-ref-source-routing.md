...
- Validation is **existing machinery**: a bounds check into the pinned, pointer-stable,
  ...
  validate-on-use stamps that already exist (`core/include/libtracer/graph.hpp:302`,
...
1. **It is the width of the stamp that already exists.** `graph_t::retire_generation` returns
   `std::uint32_t` (`core/include/libtracer/graph.hpp:302`); `child_registry_t::mount_generation`
...
2. **Compare the generation** against `graph_t::retire_generation(vh)`
   (`core/include/libtracer/graph.hpp:302`). Mismatch ⇒ **reject**. That doc comment already
...
ordering, `graph.hpp:298`), so a stale element can only ever compare *lower*. It never becomes
...
| --- | --- | --- |
| `retire_generation` (`graph.hpp:302`) | a retired-and-revived vertex | **on the wire** — the element's second u32 |
...
generation match authorizes nothing (`graph.hpp:298-300`).
...
operation over an already-minted binding — the property `graph.hpp:298-300` demands and the reason
...
vertex tree (`core/include/libtracer/graph.hpp:1157-1168`) — a tree of non-moving `unique_ptr`
...
- It is **append-only**, which the registration path already is ("vertices are added, never
  erased", `graph.hpp:1160`), so it introduces no new lifetime rule and no new invalidation event.
