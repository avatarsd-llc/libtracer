...
| ---- | ---- | ----------- | ------------------------- |
| CAN egress window table | `core/include/libtracer/view_can.hpp:100` — `frames_.push_back` in `view_can_frames_t::split`, reached on **every** send (`core/src/transport_can.cpp:215`) | the sender: one `push_back` per CAN frame the payload splits into | `CONFIG_LIBTRACER_TRANSPORT_CAN` is `default y`, so `transport_can.cpp` is in the archive (`libtracer/CMakeLists.txt:173`) unless you turn CAN off |
