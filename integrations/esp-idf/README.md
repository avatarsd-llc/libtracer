...
| `CONFIG_LIBTRACER_WS_CLIENT_POLL_MS` | int | 200 | `esp_ws_client_link_t`: one recv-loop poll turn — the **teardown-latency** knob (a destructor cannot join the recv thread faster than one turn), traded against how often an idle link wakes the CPU. |
