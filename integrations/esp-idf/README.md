...
Neither is a factory entry: the application constructs the link and hands it in with `transport_vertex_t::provide_link`. Consequently a chip build registers **no** `ws` kind in the built-in catalog, and a creator-endpoint SPEC carrying `kind=ws` with no staged link answers `SCHEMA_NOT_FOUND`. The `linux` target keeps the portable pair — it has glibc's `sendmsg` and no `esp_http_server`.
...
`tr::net::twai_link_t` implements the same seam `socketcan_link_t` implements, over the `esp_driver_twai` node API: **classic CAN 2.0 only** (TWAI has no FD — configure `transport_can` as `CLASSIC`), 29-bit extended IDs (the CAN-ID *is* the path, ADR-0022). The driver's RX-done callback runs in ISR context and only copies the frame into a FreeRTOS queue; a dispatch thread pops and feeds `transport_can` — user code never runs in the ISR. That dispatch thread is spawned by `start()`, the seam's second phase ([#1186](https://github.com/avatarsd-llc/libtracer/issues/1186)), not by the constructor: the controller comes up at construction and the ISR fills the queue, but nothing is delivered until the owner has registered its sink and started the link (`transport_can` does both for a link handed to it). The framing/advertise/reassembly layers above the seam are covered by the host test suite (`core/tests/transport_can_test.cpp` over a fake link); **bus-level validation on a real transceiver is an on-silicon checklist item on #183**, not a CI gate.
...
The device node wires the **one-slab recipe (ADR-0039/0042) concretely**: one static slab in THREE regions — front → `pool_t` (RX datagram segments; exhaustion = backpressure, never OOM), middle → a `tr::mem::pool_source_t` (the router's **label tables**), back → `monotonic_buffer_resource` + `synchronized_pool_resource` (the remaining pmr containers). The label region is where the label tables moved when #603 defect 1 took them off the pmr resource: a peer's `ADVERTISE` reaches that store on a receive thread, pre-ACL, and pmr cannot report exhaustion by value, so on `-fno-exceptions` it was a peer-triggerable reboot. It is a **recycling** `pool_source_t` and not a `bump_source_t` because label state is long-lived and churns (every `clear_link` frees a link's whole tables), and both of its bounds — the slab span and the `size_class_t` span — are injected and **reported at bring-up**, so the sizing is a measurement to check rather than a constant to trust (the self-proof prints `288/2048 B used, 0/12 size classes, 0 overflowed`). Since #588 the terminus **arena** is still not slab-bound — it draws from the router's nothrow `rx` block source, which this example leaves at the default heap, as are the graph's own three seams (the example default-constructs the graph). So the slab bounds the RX segments and the label tables, not yet the whole steady state; bounding the arena is a separate follow-on because ADR-0067 §3 wants a **per-child** source there rather than one shared across receive threads, which is a different topology from the single shared source the label plane wants. See `docs/reference/09-memory-substrate.md`. Connections are **config-created**: the application declares a module with `register_module`, and `write /net/<module>/conn <- SPEC{name, config{kind=udp, port}}` constructs and owns the real socket (ADR-0027, RFC-0014 §2) — same for the `tcp` kind via the built-in catalog. The superseded `/net:children[]` creation spelling was retired at RFC-0014 S7; `:children[]` here is a read-only enumeration. `ws` is not in that catalog on a chip target (see [the WebSocket plane](#the-websocket-plane-idf-native-on-chips-portable-on-linux)); it is staged with `provide_link`.
...
| ------ | ---- | ------- | ------ |
| `CONFIG_LIBTRACER_WS_CLIENT_READ_TIMEOUT_MS` | int | 100 | `esp_ws_client_link_t`: how long the recv thread waits for bytes a positive poll already promised. Bounds a stall; paces nothing. |
| `CONFIG_LIBTRACER_WS_CLIENT_POLL_MS` | int | 200 | `esp_ws_client_link_t`: one recv-loop poll turn — the **teardown-latency** knob (a destructor cannot join the recv thread faster than one turn), traded against how often an idle link wakes the CPU. Since #1058 it genuinely *is* the dominant term: a teardown that lands mid-dial no longer waits the dial out (it condemns the dial and detaches the recv thread), so the worst case is this plus one write budget — a quarter of the task-watchdog period — rather than this plus half a watchdog period of dial on top. |
| `CONFIG_LIBTRACER_WS_CLIENT_RECONNECT_BACKOFF_MS` | int | 1500 | `esp_ws_client_link_t`: upper bound on the wait before re-dialing a peer that is down (upper only — the destructor signals the wait). Also what a refused admission costs before the next attempt. |
...
**Per-LINK sizing is NOT Kconfig, deliberately** (#1160). The numbers that are the WebSocket links' per-link RAM are **constructor arguments**, because a Kconfig value is per-image and two links in one image legitimately serve different peers: `httpd_ws_link_t(…, rx_scratch_bytes, tx_pool_slots, tx_inline_bytes)` (0 = the published `kDefaultRxScratchBytes` / `kDefaultTxPoolSlots` / `kDefaultTxInlineBytes`, together ~8.4 KiB per server link) and `esp_ws_client_link_t(…, rx_bytes, tx_bytes, …)`. The effective values are readable at runtime — `rx_scratch_bytes()`, `tx_slot_capacity()`, `tx_inline_bytes()`, `buffer_bytes()` on the server; `rx_bytes()`, `tx_bytes()`, `timing()` on the client — so a node can report the ceiling that produced a drop instead of quoting a constant out of these sources.
...
| ---- | ---- | ----------- | ------------------------- |
| `try_reserve` on `-fno-exceptions` (#923, #850) | `core/include/libtracer/mem_heap.hpp:169-183` — `try_grow` catches the container's own allocation failure on a hosted build; **this component is not one**, so IDF's `-fno-exceptions` build keeps the probe-then-commit fallback | anything concurrent — and single-core is not single-threaded: a FreeRTOS context switch between the probe's free and the `reserve` opens the window. Every `try_*` helper inherits it on this profile | header-only, reached from the graph and every transport |
