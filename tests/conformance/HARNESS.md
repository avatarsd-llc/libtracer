...
| `stream/receiver-ring-flood` | **PARTIAL — the plane-isolation arm only.** RFC-0025 §7 vector 12 (Amendment 2 §4.6.1) claims three things: (i) exhausting the **net-plane** store under a flood leaves the **graph plane** still able to allocate; (ii) the receiver ring sheds under §4.4 rather than starving the node; (iii) the accounting recovers once pressure clears. Only **(i)** is bound today. | C++ `core/tests/plane_isolation_test.cpp` — a peer-shaped `ADVERTISE` flood through `fwd_router_t::on_frame` drains the injected net-plane `block_source_t` (the `#603` / `fe6adcd6` seam) to a hard `null_source()`-backed bound, and every graph-plane assertion is taken **while** that store is proven exhausted and further inbound frames are still being refused: vertex creation, an `assign` publish read back byte-exact, an edge add + dispatch, and a composed subtree read served by the graph's own injected `ctl` seam. The suite carries its own **ablation as a control arm** — the identical node with the per-plane injection deleted (both planes on one source of the same size) answers `BACKPRESSURE` to the identical composed read — so "the graph plane still allocates" is a claim about an allocation that demonstrably *can* be starved. Only the composed read is ablation-sensitive today, because ADR-0079 stage 2 (the graph placement store) is unbuilt and `vertex_t` still comes from the global heap; the file says so where the arms are. **(ii) and (iii) are unbound**: they assert against the receiving STREAM ring's `try_alloc` refusal path, which lands with the fused #1461 + #1462 PR rewriting `vertex_t::store()`'s STREAM arm. Until then there is no vector directory under `vectors/v1/stream/receiver-ring-flood/` either — the vector has no wire surface of its own, so a codec-tier `input.bin` would claim carriage of bytes this vector is not about. No Rust/TS row, and none is owed: neither binding has a graph, a router or an injected allocation store. |
