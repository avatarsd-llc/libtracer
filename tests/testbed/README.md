...
> rules, while `transport.hpp:39` declares `using peer_id_t = std::array<std::byte, 16>`
...
- ~~**No reconnect anywhere.**~~ **Landed.** RFC-0014 §4 gave `transport_vertex.hpp` a
  six-state `link_state_t` (`transport_vertex.hpp:102`) including `RECONNECTING`, plus
  `backoff_ms` (`transport_vertex.hpp:154`) and `connect_timeout_ms` (`transport_vertex.hpp:158`).
