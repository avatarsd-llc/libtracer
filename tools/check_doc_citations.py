...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1811", "graph_t::set_identity"),
    ("core/src/graph.cpp:1837", "graph_t::read_identity"),
    ("core/src/graph.cpp:2170", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:1552", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1619", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1656", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1577", "step0.wildcard"),
    ("core/src/graph.cpp:2136", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2236", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ("core/src/op_resolve_walk.hpp:333", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:958", 'field.steps[0].name != "subscribers"'),
    ...
    ("core/src/graph.cpp:1078", "!arena"),
    ("core/include/libtracer/segment.hpp:78", "struct segment_t"),
    # --- the design + module pages (#728). Every one of these had drifted. ---
    ("core/src/graph.cpp:786", "has_registered_child()"),
    ("core/src/graph.cpp:876", "void graph_t::fan_out"),
    ("core/src/graph.cpp:999", "graph_t::write_impl"),
    ("core/src/graph.cpp:1061", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1062", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1074", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1075", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1077", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1093", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1094", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1096", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1298", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1531", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1658", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ("core/include/libtracer/vertex.hpp:2439", "vertex_t* parent_"),
    ("core/include/libtracer/vertex.hpp:2439", "vertex_t* parent_"),
    ...
    ('core/include/libtracer/graph.hpp:477',
    ...
    ('core/include/libtracer/graph.hpp:564',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:651', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:669',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:707', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:821', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:845',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:942', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:944', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:946', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:948', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:959', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:991', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:997', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1003', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1004', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1007', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1011', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1158', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1167', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1218',
    ...
    ('core/include/libtracer/graph.hpp:1227', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/vertex.hpp:109',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:114', 'class value_ref_t {'),
    ('core/include/libtracer/vertex.hpp:540', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:727', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:730', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:770',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:814', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:830',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:838', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:848', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:852', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:996',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1159', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1636',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow) {'),
    ('core/include/libtracer/vertex.hpp:1640', 'const bool use_heap = subs_.size() > edge_snapshot_t::kCapacity &&'),
    ('core/include/libtracer/vertex.hpp:1645',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2497',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:2500',
    ...
    ('core/src/graph.cpp:589', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:811', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:824', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:831', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:835', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:843', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:866', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:909',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:916', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:925', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:938', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:945',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:966', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1005', '// a subtree sweep. propagate(v) is the separate accumulate-then-flush primitive.'),
    ('core/src/graph.cpp:1008', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1024', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1029', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1050',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1071',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1167', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1285',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1453', 'field_path_t field;'),
    ('core/src/graph.cpp:1713', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2267', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/op_resolve_walk.hpp:936',
     'if (!req.src.spans_intact()) return std::unexpected(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:1013', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:859', 'if (value.total_length() == 0)'),
    # core/src/path.cpp
