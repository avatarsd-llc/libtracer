...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3877",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3917", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4673", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3751", "Everything else under `settings`"),
    ("core/src/graph.cpp:3950", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3509", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3629", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3669", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3563", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:4127", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4179", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4191", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4349", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4610", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4607", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4781", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/include/libtracer/mem_heap.hpp:286",
    ...
    ('core/include/libtracer/mem_heap.hpp:248',
     '[[nodiscard]] bool try_reserve(std::vector<T, Alloc>& v, std::size_t n) noexcept {'),
    ('core/include/libtracer/mem_heap.hpp:470', '[[nodiscard]] inline std::optional<view_t> over_bytes(std::span<const std::byte> bytes,'),
    ...
    ('core/include/libtracer/graph.hpp:1777', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1842', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2400', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2404', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2414', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2417', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2421', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2432', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2527', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2533', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2539', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2540', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2564', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2568', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2862', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2878', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:3004', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:3019', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2272', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:2136", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2362", "graph_t::write_impl"),
    ("core/src/graph.cpp:2528", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2530", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2545", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2546", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2548", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2564", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2565", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2570", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2947", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3481", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3671", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1634',
    ...
    ('core/include/libtracer/graph.hpp:2990',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:2828', '    mem::source_backend_t src_backend_;'),
    ('core/include/libtracer/graph.hpp:2839', '    mem::source_resource_t src_mr_;'),
    ...
    ('core/include/libtracer/mem_heap.hpp:433',
    ...
    ('core/include/libtracer/mem_source.hpp:279', '[[nodiscard]] block_source_t& heap_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:300', '[[nodiscard]] block_source_t& null_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:325', 'class bump_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:367', 'void reset() noexcept {'),
    ('core/include/libtracer/mem_source.hpp:507', 'class pool_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:311',
     '*       `monotonic_buffer_resource` also spills past its buffer, but it spills to a'),
    ('core/include/libtracer/mem_source.hpp:316',
     '* @warning SCOPE-LIFETIME USE ONLY. A bump block is never reclaimed, so a source that'),
    ('core/include/libtracer/mem_source.hpp:319',
     '*          between operations. It is NOT a long-lived seam: an 8 KiB bump source wired as'),
    ('core/include/libtracer/mem_source.hpp:320',
     "*          a router's `rx` decoded 6 frames and rejected the next 194 — measured. A"),
    ('core/include/libtracer/mem_source.hpp:322',
     '* @note Single-threaded by contract — a bump cursor is not synchronized. Its intended use'),
    ('core/include/libtracer/mem_source.hpp:333',
     '[[nodiscard]] void* try_alloc(std::size_t bytes, std::size_t align) noexcept override {',
     ': block_source_t("bump"), buf_(buffer), upstream_(&upstream) {}'),
    ('core/include/libtracer/mem_source.hpp:524',
     'pool_source_t(std::span<std::byte> slab, std::span<size_class_t> classes) noexcept'),
    ('core/include/libtracer/mem_source.hpp:586',
     '[[nodiscard]] std::size_t classes_used() const noexcept { return n_; }'),
    ('core/include/libtracer/mem_source.hpp:597',
    ...
    ('core/include/libtracer/vertex.hpp:1089', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1933', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2814', 'const bool use_heap ='),
    ...
    ('core/src/graph.cpp:2449',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2462', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2515',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2542',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2692', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2931',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3760', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4812', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:3439', 'delivery_link.assign(split.link);'),
    ...
    ('core/include/libtracer/vertex.hpp:504', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:1987',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2732',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2909', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2994',
    ...
    ('core/src/graph.cpp:2196', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2527',
    ...
    ('core/src/graph.cpp:2545', 'std::array<std::byte, 4096> stack;'),
    ('core/src/graph.cpp:2546', 'mem::bump_source_t src(stack, *ctl_);'),
    ('core/src/graph.cpp:2762', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2859',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:3017', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3571', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3763',
    ...
    ('core/src/graph.cpp:2405',
     '// A handler stores no LKV (the user handler consumes the value), so there is no'),
    ('core/src/graph.cpp:2828', 'if (!try_build_key(v, key)) {'),
    ('core/src/graph.cpp:2461',
    ...
    ('core/include/libtracer/vertex.hpp:3063', '    std::atomic<role_t> role_;'),
    ('core/src/graph.cpp:2695', 'void graph_t::deliver_current(vertex_t* v) {'),
    ('core/src/graph.cpp:2712',
     "// The sweep root's OWN delivery is unconditional (below), and it reads the LKV"),
    ('core/src/graph.cpp:2978',
     'if (!sp) return std::unexpected(status_t::NOT_FOUND);  // never assigned'),
    ('core/src/graph.cpp:4279',
    ...
    ('core/include/libtracer/vertex.hpp:1586',
