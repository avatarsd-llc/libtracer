...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2824",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2864", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3386", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:1087", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1093", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1144", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:179", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:345", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:523", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:2478", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2597", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2637", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2531", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3069", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3118", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3130", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3276", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3323", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3320", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3466", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:351", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:1005", 'field.steps[0].name != "subscribers"'),
    ...
    ('core/src/op_resolve_walk.hpp:1099', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ('core/include/libtracer/graph.hpp:1683', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1687', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1697', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1700', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1704', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1715', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1804', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1810', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1816', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1817', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1841', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1845', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2047', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2057', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2170', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1289', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1583', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1589', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ("core/include/libtracer/transport.hpp:33", "namespace tr::net {"),
    ('core/include/libtracer/transport.hpp:37', 'using peer_id_t = std::array<std::byte, 16>;'),
    ('core/include/libtracer/transport.hpp:141', 'class bus_link_t {'),
    ...
    ("core/src/fwd_router.cpp:2760", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2798", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2799", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2801", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2254", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2273", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2266", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1739", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2168", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:539',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1504',
...
('core/include/libtracer/graph.hpp:1164',
    ...
    ('core/include/libtracer/graph.hpp:2161',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2164',
    ...
    ('core/include/libtracer/transport.hpp:522',
     'virtual void send(std::span<const std::span<const std::byte>> iov) {'),
    ('core/include/libtracer/transport.hpp:678',
    ...
    ('core/include/libtracer/vertex.hpp:721', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1422', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2230', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2247', '++drops.out_of_memory;'),
    ...
    ('core/include/libtracer/vertex.hpp:532',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1472',
    ...
    ('core/src/fwd_router.cpp:1707',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1713', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1769', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2282',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:2233', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2247', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2297', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2777',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2858',
    ...
    ('core/src/graph.cpp:2707', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3497', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/fwd_reply.hpp:109', 'void tlv_sliced(std::span<const std::byte> wire) {'),
    ('core/src/op_resolve_walk.hpp:959',
    ...
    ('core/include/libtracer/fwd_router.hpp:402',
     '[[nodiscard]] graph::result_t<void> subscribe_toward(const graph::path_t& producer,'),
    ('core/include/libtracer/fwd_router.hpp:1061',
    ...
    ('core/include/libtracer/graph.hpp:1742',
    ...
    ('core/include/libtracer/vertex.hpp:377', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:2154',
    ...
    ('core/src/graph.cpp:2539', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2710',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2896', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
