...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2892",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2932", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3454", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/transport_vertex.cpp:99", "graph_.register_child_type(",
     "!config, conn_role_t::DIAL);"),
    ("core/src/transport_vertex.cpp:133",
     "void transport_vertex_t::register_transport_type(std::string kind, transport_factory_t factory) {"),
    ("core/src/transport_vertex.cpp:138", "transport_vertex_t::register_module"),
    ...
    ("core/src/transport_vertex.cpp:176",
    ...
    ("core/src/graph.cpp:2539", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2665", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2705", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2592", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3137", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3186", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3198", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3344", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3391", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3388", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3534", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/include/libtracer/graph.hpp:51", "namespace tr::graph {"),
    ('core/include/libtracer/graph.hpp:1303', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1363', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1788', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1792', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1802', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1805', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1809', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1820', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1909', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1915', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1921', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1922', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1946', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1950', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2231', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2241', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2354', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1339', '* @param ctx Caller-owned context. It must stay alive until the first'),
    ('core/include/libtracer/graph.hpp:1688', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1694', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1360", "has_registered_child()"),
    ("core/src/graph.cpp:1494", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1632", "graph_t::write_impl"),
    ("core/src/graph.cpp:1745", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1747", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1762", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1763", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1765", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1781", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1782", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1787", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2081", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2511", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2707", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ('core/include/libtracer/config.hpp:344', 'using config_t = default_config_t;'),
    ...
    ('core/include/libtracer/config.hpp:390',
     'inline constexpr bool kSpinWaitSafe = tr::graph::config_t::kSpinWaitSafe;'),
    ('core/include/libtracer/config.hpp:311', 'static constexpr bool kWeaklyOrdered = true;'),
    ...
    ('core/include/libtracer/config.hpp:353',
...
('core/include/libtracer/graph.hpp:1214',
    ...
    ('core/include/libtracer/subscriber.hpp:403', 'class edge_snapshot_t {'),
    ('core/include/libtracer/subscriber.hpp:406', 'static constexpr std::size_t kCapacity = 8;'),
    ...
    ('core/include/libtracer/vertex.hpp:1446', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2254', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2271', '++drops.out_of_memory;'),
    ...
    ('core/src/graph.cpp:1086', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1093', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1128',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1393', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1421', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1428', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1432', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1440', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1483',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1527',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1534', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1544',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1562',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1570',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1599', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1651', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1679', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1692', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1703', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1732',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1759',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1877', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2065',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2775', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3565', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1170', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1170',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2478', 'delivery_link.assign(split.link);'),
    ...
    ('.github/workflows/core-ci.yml:234', 'matrix:', '  tsan:'),
    ...
    ('.github/workflows/core-ci.yml:243', '-DCMAKE_CXX_FLAGS="-fsanitize=thread -g -O1"'),
    ...
    ('core/include/libtracer/subscriber.hpp:500', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/subscriber.hpp:598',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1504',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2178',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2358', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2435',
    ...
    ('core/src/graph.cpp:1790', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1923', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2020',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2253', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2600', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2778',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2964', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
    ...
    ('core/src/transport_vertex.cpp:65',
