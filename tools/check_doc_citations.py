...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2459", "graph_t::set_identity"),
    ("core/src/graph.cpp:2485", "graph_t::read_identity"),
    ("core/src/graph.cpp:2904", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:834", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:840", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:886", "The forward hop, read entirely by OFFSET"),
    ...
    ("core/src/graph.cpp:2140", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2259", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2299", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2193", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2625", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2671", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2683", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2800", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2841", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2838", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2984", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1057', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1113', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1427', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1431', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1441', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1444', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1448', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1459', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1535', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1541', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1547', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1548', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1572', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1576', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1761', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1770', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1859', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1093', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1326', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1332', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1078", "has_registered_child()"),
    ("core/src/graph.cpp:1189", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1321", "graph_t::write_impl"),
    ("core/src/graph.cpp:1434", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1436", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1451", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1452", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1454", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1470", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1471", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1473", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1760", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2112", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2301", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2107", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2145", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2146", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2148", "emit_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2179", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1693", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1712", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1705", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1371", "frame.subrope(0, frame.total_length()).try_materialize"),
    ...
    ("core/src/fwd_router.cpp:1611", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:1077',
...
('core/include/libtracer/graph.hpp:970',
    ...
    ('core/include/libtracer/vertex.hpp:827', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:830', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:1085', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1109', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1119', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1123', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1460', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2123', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2913', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2930', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:1041',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:1101',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:1276',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:2173',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow,'),
    ('core/include/libtracer/vertex.hpp:2919',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2925',
     'if (src[i].active.load(std::memory_order_acquire)) ++drops.truncated;'),
    ('core/include/libtracer/vertex.hpp:3241',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3246',
    ...
    ('core/src/fwd_router.cpp:1339',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1345', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1401', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1721',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1672', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1686', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1736', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2124',
    ...
    ('core/src/graph.cpp:807', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:814', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:849',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1103', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1131', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1138', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1142', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1150', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1179', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:1222',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1229', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1239',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1257',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1265',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1288', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1340', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1368', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1381', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1392', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1421',
    ...
    ('core/src/graph.cpp:1560', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1744',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2369', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3015', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:863', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:817', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:891', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:892', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:2098', 's.target_key.reset();'),
