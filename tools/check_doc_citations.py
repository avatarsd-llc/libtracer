...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3784",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3824", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4579", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3658", "Everything else under `settings`"),
    ("core/src/graph.cpp:3856", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3437", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3557", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3597", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3491", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:4033", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4085", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4097", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4255", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4516", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4513", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4687", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/include/libtracer/mem_heap.hpp:282",
     "[[nodiscard]] inline bool try_assign(std::vector<std::byte>& dst,"),
    ('core/include/libtracer/mem_heap.hpp:169', '[[nodiscard]] inline bool try_grow(std::size_t bytes, F&& grow) noexcept {'),
    ('core/include/libtracer/mem_heap.hpp:244',
     '[[nodiscard]] bool try_reserve(std::vector<T, Alloc>& v, std::size_t n) noexcept {'),
    ('core/include/libtracer/mem_heap.hpp:442', '[[nodiscard]] inline std::optional<view_t> over_bytes(std::span<const std::byte> bytes,'),
    ...
    ('core/include/libtracer/graph.hpp:1746', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1811', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2369', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2373', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2383', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2386', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2390', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2401', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2494', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2500', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2506', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2507', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2531', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2535', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2829', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2841', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2960', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2975', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1782',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2235', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2241', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1959", "has_registered_child()"),
    ("core/src/graph.cpp:2126", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2318", "graph_t::write_impl"),
    ("core/src/graph.cpp:2484", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2486", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2501", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2502", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2504", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2520", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2521", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2526", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2876", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3409", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3599", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1603',
    ...
    ('core/include/libtracer/graph.hpp:2953',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:2795', '    mem::source_backend_t src_backend_;'),
    ('core/include/libtracer/graph.hpp:2806', '    mem::source_resource_t src_mr_;'),
    ...
    ('core/include/libtracer/mem_heap.hpp:405',
    ...
    ('core/src/graph.cpp:1627', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1634', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1669',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1992', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:2020', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:2034', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:2038', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:2046', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:2115',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2166',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2173', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2183',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2201',
    ...
    ('core/src/graph.cpp:2272', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2361',
    ...
    ('core/src/graph.cpp:2405',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2418', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2471',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2498',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2621', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2860',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3667', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4718', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1724', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1724',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3367', 'delivery_link.assign(split.link);'),
    ...
    # ('core/src/graph.cpp:2946', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3499', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3670',
