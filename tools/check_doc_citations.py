...
ANCHORS = [
    ("core/include/libtracer/tlv.hpp:99", "struct opt_t"),
    ("core/include/libtracer/tlv.hpp:32", "enum class type_t"),
    ('core/include/libtracer/tlv.hpp:51', 'ROUTER = 0x0D, /**< @brief Router-wrapped frame. */'),
    ("core/src/graph.cpp:3266",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3306", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3835", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3140", "Everything else under `settings`"),
    ("core/src/graph.cpp:3338", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ("core/src/transport_vertex.cpp:61", 'cfg.name("kind")'),
    ...
    ("core/src/transport_vertex.cpp:124", "graph_.register_child_type(",
    ...
    ("core/src/transport_vertex.cpp:168", "transport_vertex_t::register_module"),
    ...
    ("core/src/transport_vertex.cpp:206",
    ...
    ("core/src/transport_vertex.cpp:233", "transport_vertex_t::mint_module_locked"),
    ("core/src/transport_vertex.cpp:278", "transport_vertex_t::endpoint_write"),
    ("core/src/transport_vertex.cpp:431", "transport_vertex_t::provide_link"),
    ("core/src/transport_vertex.cpp:523", "routing key IS the mount path"),
    ...
    ("core/src/transport_vertex.cpp:530", "qualified += name", "routing key IS the mount path"),
    ("core/src/transport_vertex.cpp:548", "structural vertex, created lazily"),
    ...
    ("core/src/transport_vertex.cpp:556", "register_vertex_key(mod_key",
     "structural vertex, created lazily"),
    ("core/src/transport_vertex.cpp:689", "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:698", "pending_links_.erase(pl)"),
    ("core/src/transport_vertex.cpp:717", "if (constructed)"),
    ("core/src/transport_vertex.cpp:719", "? link_state_t::LISTENING"),
    ('core/src/transport_vertex.cpp:90', '[[nodiscard]] view_t link_state_value(link_state_t state) {'),
    ('core/src/transport_vertex.cpp:474', 'std::string module;'),
    ...
    ("core/include/libtracer/fwd_router.hpp:1295", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1301", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1354", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:194", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:372", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:550", "using reply_fn_t"),
    ('core/include/libtracer/fwd_router.hpp:196', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:197', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ...
    ("core/include/libtracer/child_registry.hpp:280", "bool add(std::string name"),
    ("core/include/libtracer/child_registry.hpp:538",
     "[[nodiscard]] static transport_t* resolve_peer(const child_t& child, std::string_view peer) {"),
    ("core/include/libtracer/child_registry.hpp:553", "bool erase(std::string_view name) {"),
    ("core/include/libtracer/child_registry.hpp:586", "entry_by_name"),
    ("core/include/libtracer/child_registry.hpp:607",
     "[[nodiscard]] transport_t* by_name(std::string_view name) const {"),
    ("core/include/libtracer/child_registry.hpp:648", "std::size_t size()"),
    ("core/include/libtracer/child_registry.hpp:658", "live_size"),
    ("core/src/transport_vertex.cpp:692", "return std::unexpected(status_t::BACKPRESSURE);",
     "if (!router_.add_child(qualified, *link))"),
    ("core/include/libtracer/transport_vertex.hpp:459", "result_t<void> register_module"),
    ("core/include/libtracer/transport_vertex.hpp:102", "enum class link_state_t"),
    ...
    ("core/include/libtracer/transport_vertex.hpp:130", "struct conn_settings_t {"),
    ("core/include/libtracer/transport_vertex.hpp:594",
    ...
    ("core/src/transport_vertex.cpp:630", "handlers.on_children = [bus]() -> result_t<view_t> {"),
    ("core/src/graph.cpp:2919", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3039", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3079", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2973", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3515", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3567", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3579", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3725", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3772", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3769", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3915", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:351", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:1013", 'field.steps[0].name != "subscribers"'),
    ("core/src/op_resolve_walk.hpp:164", "view_t own_wire(mem::mem_backend_t& flat)"),
    ("core/src/op_resolve_walk.hpp:550", "rope_t or_backpressure"),
    ('core/src/op_resolve_walk.hpp:1109', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ("core/include/libtracer/graph.hpp:53", "namespace tr::graph {"),
    ('core/include/libtracer/graph.hpp:1431', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1496', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2017', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2021', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2031', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2034', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2038', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2049', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2138', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2144', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2150', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2151', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2175', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2179', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2391', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2401', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2514', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1467',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1885', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1891', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1650", "has_registered_child()"),
    ("core/src/graph.cpp:1800", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1945", "graph_t::write_impl"),
    ("core/src/graph.cpp:2058", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2060", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2075", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2076", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2078", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2094", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2095", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2100", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2394", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2891", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3081", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2997", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:3035", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:3036", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:3038", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2421", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2440", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2433", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1895", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2327", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('bench/bench_libtracer.cpp:1378',
    ...
    ('core/include/libtracer/fwd_router.hpp:195',
     'mem::block_source_t* label_src = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:566',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1724',
...
('core/include/libtracer/graph.hpp:1295',
    ...
    ('core/include/libtracer/subscriber.hpp:109', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/subscriber.hpp:616', 'class edge_snapshot_t {'),
    ('core/include/libtracer/subscriber.hpp:619', 'static constexpr std::size_t kCapacity = 8;'),
    ...
    ('core/include/libtracer/vertex.hpp:769', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1492', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2303', 'const bool use_heap ='),
    ...
    ('core/src/fwd_router.cpp:1869', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1926', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2449',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2400', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2414', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2466', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:3014',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3095',
     'constexpr std::array<std::byte, 5> op_tlv{std::byte{0x01}, std::byte{0x00}, std::byte{0x01},'),
    # core/src/graph.cpp
    ('core/src/graph.cpp:474', 'const view_t& frame_view, std::vector<std::byte> key,'),
    ('core/src/graph.cpp:1373', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1380', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1415',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1683', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1711', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1725', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1729', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1737', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1789',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1840',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1847', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1857',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1875',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1883',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1912', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1964', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1992', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:2005', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:2016', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2045',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2072',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2190', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2378',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3149', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3946', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/fwd_reply.hpp:109', 'void tlv_sliced(std::span<const std::byte> wire) {'),
    ('core/src/op_resolve_walk.hpp:967',
    ...
    ('core/src/transport_vertex.cpp:50',
    ...
    ('core/src/fwd_router.cpp:976', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:914', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1460', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1460',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2858', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1638', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1653', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1654', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/subscriber.hpp:74', 'struct delivery_policy_t {'),
    ('core/include/libtracer/vertex.hpp:385', 'enum class delivery_mode_t : std::uint8_t {'),
    ('core/include/libtracer/subscriber.hpp:705', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/subscriber.hpp:808',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1546',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2221',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2398', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2475',
     '// padding — 8-byte, then 4-byte, then flag bytes), with everything the write hot'),
    ('core/src/fwd_router.cpp:850',
     "// segment, which is what keeps two buses' same-named peers distinct on the way back."),
    ('core/src/fwd_router.cpp:921', 'bus->set_peer_receiver('),
    ('core/src/fwd_router.cpp:1334',
     'transport_t* fwd_router_t::bound_egress(wire::path_ref_element_t e, std::string_view caller,'),
    ('core/src/fwd_router.cpp:1356',
    ...
    ('core/src/fwd_router.cpp:2455',
     'const wire::grammar::rope_cursor cur{frame};',
     'if (!frame.all_host()) return;'),
    ('core/src/fwd_router.cpp:2750', 'const auto payload_view = view::over_bytes(payload_bytes);'),
    ('core/src/fwd_router.cpp:2761',
    ...
    ('core/src/graph.cpp:2103', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2236', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2333',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2451', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
