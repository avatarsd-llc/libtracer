...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3611",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3651", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4394", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3485", "Everything else under `settings`"),
    ("core/src/graph.cpp:3683", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3264", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3384", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3424", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3318", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3860", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3912", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3924", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4070", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4331", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4328", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4502", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:2203", "graph_t::write_impl"),
    ("core/src/graph.cpp:2343", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2345", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2360", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2361", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2363", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2379", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2380", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2385", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2703", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3236", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3426", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1545',
    ...
    ('core/include/libtracer/vertex.hpp:919', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1763', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2621', 'const bool use_heap ='),
    ...
    ('core/include/libtracer/vertex.hpp:696',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1809',
    ...
    ('core/include/libtracer/vertex.hpp:2998',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3003',
    ...
    ('core/src/graph.cpp:2266',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2279', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2330',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2357',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2475', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2687',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3494', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4533', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:3194', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1753', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1768', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1769', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/vertex.hpp:1817',
    ...
    ('core/include/libtracer/vertex.hpp:2716', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2793',
    ...
    ('core/src/graph.cpp:2388', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2545', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2642',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2773', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3326', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3497',
