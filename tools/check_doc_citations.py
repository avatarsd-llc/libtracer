...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1982", "graph_t::set_identity"),
    ("core/src/graph.cpp:2008", "graph_t::read_identity"),
    ("core/src/graph.cpp:2384", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:1711", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1790", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1827", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1736", "step0.wildcard"),
    ("core/src/graph.cpp:2148", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2194", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2206", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2317", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2354", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2350", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2450", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ...
    ("core/src/graph.cpp:1175", "!arena"),
    ("core/include/libtracer/segment.hpp:78", "struct segment_t"),
    # --- the design + module pages (#728). Every one of these had drifted. ---
    ("core/src/graph.cpp:864", "has_registered_child()"),
    ("core/src/graph.cpp:954", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1084", "graph_t::write_impl"),
    ("core/src/graph.cpp:1158", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1159", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1171", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1172", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1174", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1190", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1191", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1193", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1395", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1690", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1829", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:857',
    ...
    ('core/include/libtracer/graph.hpp:1277', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1279', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1281', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1284', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1288', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1299', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1331', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1337', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1343', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1344', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1361', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1365', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1537', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1546', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1602',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1605',
    ...
    ('core/include/libtracer/vertex.hpp:574', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:761', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:764', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:974',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:1018', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1034',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:1042', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1052', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1056', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1209',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1388', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1927', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:1977',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow,'),
    ('core/include/libtracer/vertex.hpp:2709', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2715',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2721',
     'if (src[i].active.load(std::memory_order_acquire)) ++drops.truncated;'),
    ('core/include/libtracer/vertex.hpp:2726', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:3025',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3028',
    ...
    ('core/src/graph.cpp:889', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:902', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:909', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:913', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:921', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:944', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:987',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:994', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1004',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1022',
    ...
    ('core/src/graph.cpp:1051', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1093', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1113', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1121', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1126', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1147',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1168',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1264', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1382',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1884', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2481', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/include/libtracer/graph.hpp:1197', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1203', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ('core/src/graph.cpp:1675', 's.target_key.reset();'),
