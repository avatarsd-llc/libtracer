...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3109",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3149", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3671", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/transport_vertex.cpp:620", "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:629", "pending_links_.erase(pl)"),
    ("core/src/transport_vertex.cpp:648", "if (constructed)"),
    ("core/src/transport_vertex.cpp:650", "? link_state_t::LISTENING"),
    ...
    ("core/src/transport_vertex.cpp:623", "return std::unexpected(status_t::BACKPRESSURE);",
    ...
    ("core/src/graph.cpp:2763", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2882", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2922", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2816", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3354", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3403", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3415", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3561", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3608", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3605", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3751", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1367', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1432', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1906', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1910', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1920', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1923', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1927', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1938', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2027', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2033', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2039', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2040', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2064', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2068', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2270', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2280', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2393', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1403',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1806', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1812', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1520", "has_registered_child()"),
    ("core/src/graph.cpp:1654", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1799", "graph_t::write_impl"),
    ("core/src/graph.cpp:1912", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1914", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1929", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1930", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1932", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1948", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1949", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1954", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2248", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2735", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2924", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2929", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2967", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2968", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2970", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2353", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2372", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2365", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1827", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2259", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:1231',
    ...
    ('core/include/libtracer/vertex.hpp:735', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1453', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2261', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2278', '++drops.out_of_memory;'),
    ...
    ('core/src/fwd_router.cpp:1794',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1801', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1858', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2381',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2332', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2346', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2398', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2946',
    ...
    ('core/src/graph.cpp:1246', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1253', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1288',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1553', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1581', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1588', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1592', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1600', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1643',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1694',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1701', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1711',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1729',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1737',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1766', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1818', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1846', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1859', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1870', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1899',
    ...
    ('core/src/graph.cpp:2044', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2232',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2992', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3782', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:957', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:906', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1330', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:2702', 'delivery_link.assign(split.link);'),
    ...
    ('.github/workflows/core-ci.yml:445', 'name: tsan (slot=${{ matrix.lkv_slot }})'),
    ...
    ('.github/workflows/core-ci.yml:452', '-DCMAKE_CXX_FLAGS="-fsanitize=thread -g -O1"'),
    ...
    ('core/include/libtracer/vertex.hpp:1511',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2185',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2365', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2442',
    ...
    ('core/src/graph.cpp:1957', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2090', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2187',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2420', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2824', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2995',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:3181', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
