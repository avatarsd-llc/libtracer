...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1831", "graph_t::set_identity"),
    ("core/src/graph.cpp:1857", "graph_t::read_identity"),
    ("core/src/graph.cpp:2190", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:1572", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1639", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1676", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1597", "step0.wildcard"),
    ("core/src/graph.cpp:2156", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2256", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ...
    ("core/src/graph.cpp:1098", "!arena"),
    ("core/include/libtracer/segment.hpp:78", "struct segment_t"),
    # --- the design + module pages (#728). Every one of these had drifted. ---
    ("core/src/graph.cpp:806", "has_registered_child()"),
    ("core/src/graph.cpp:896", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1019", "graph_t::write_impl"),
    ("core/src/graph.cpp:1081", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1082", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1094", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1095", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1097", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1113", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1114", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1116", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1318", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1551", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1678", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1444", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1473", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1474", "flatten OOM"),
    ("core/src/fwd_router.cpp:1478", "try_encode_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1510", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1125", "route_flat.empty()"),
    ("core/src/fwd_router.cpp:1145", "payload_flat.empty()"),
    ("core/src/fwd_router.cpp:1138", "frame.subrope(head->child1_off, head->child1_total).materialize", "case type_t::COMPACT"),
    ("core/src/fwd_router.cpp:778", "frame.subrope(0, frame.total_length()).materialize"),
    ...
    ("core/src/fwd_router.cpp:1043", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_frame_view.hpp:423',
    ...
    ('core/include/libtracer/graph.hpp:512',
     '[[nodiscard]] result_t<value_ref_t> read(vertex_handle_t v, std::string_view caller = {}) const;'),
    ('core/include/libtracer/graph.hpp:566',
     '* makes host-side after registration — and it has **no wire surface at all**: no peer'),
    ('core/include/libtracer/graph.hpp:599',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:686', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:704',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:742', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:856', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:880',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:977', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:979', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:981', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:983', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:994', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1026', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1032', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1038', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1039', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1042', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1046', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1202', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1211', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1262',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1265',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1271', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/src/fwd_router.cpp:729',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:735', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:796', '// A REPLY that reaches its originator here is handed to the sink rope-native'),
    ('core/src/fwd_router.cpp:1091',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1104', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1117', 'const view_t route_flat ='),
    ('core/src/fwd_router.cpp:1118',
    ...
    ('core/src/fwd_router.cpp:1459', "// dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1)."),
    ('core/src/fwd_router.cpp:1494',
    ...
    ('core/src/graph.cpp:609', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:831', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:844', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:851', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:855', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:863', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:886', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:929',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:936', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:945', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:958', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:965',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:986', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1025', '// a subtree sweep. propagate(v) is the separate accumulate-then-flush primitive.'),
    ('core/src/graph.cpp:1028', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1044', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1049', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1070',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1091',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1187', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1305',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1473', 'field_path_t field;'),
    ('core/src/graph.cpp:1733', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2287', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/op_resolve_walk.hpp:1020', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
