...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3554",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3594", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4328", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3428", "Everything else under `settings`"),
    ("core/src/graph.cpp:3626", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/include/libtracer/fwd_router.hpp:1392", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1398", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1451", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:249", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:469", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:647", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:3207", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3327", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3367", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3261", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3803", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3855", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3867", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4013", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4265", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4262", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4426", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1820", "has_registered_child()"),
    ("core/src/graph.cpp:1984", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2163", "graph_t::write_impl"),
    ("core/src/graph.cpp:2300", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2302", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2317", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2318", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2320", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2336", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2337", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2342", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2660", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3179", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3369", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:3116", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:3154", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:3155", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:3157", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2533", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2552", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2545", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1952", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2407", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:663',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1839',
...
('core/include/libtracer/graph.hpp:1476',
    ...
    ('core/src/fwd_router.cpp:1919',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1926', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1987', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2561',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2512', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2526', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2578', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:3133',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3223',
     'constexpr std::array<std::byte, 5> op_tlv{std::byte{0x01}, std::byte{0x00}, std::byte{0x01},'),
    # core/src/graph.cpp
    ('core/src/graph.cpp:571', 'const view_t& frame_view, std::vector<std::byte> key,'),
    ('core/src/graph.cpp:1488', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1495', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1530',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1853', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1881', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1895', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1899', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1907', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1973',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2024',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2031', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ...
    ('core/src/graph.cpp:2067',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2130', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2182', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2210', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2223',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2236', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2287',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2314',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2432', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2644',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3437', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4457', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1585', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1585',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3137', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1698', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1713', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1714', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/fwd_router.hpp:1365',
    ...
    ('core/src/fwd_router.cpp:2379',
     'mem::block_array_t<std::span<const std::byte>> iov{rx_for(inbound_ctx)};'),
    ('core/src/fwd_router.cpp:2567',
     'const wire::grammar::rope_cursor cur{frame};',
     'if (!frame.all_host()) return;'),
    ('core/src/fwd_router.cpp:2862', 'const auto payload_view = view::over_bytes(payload_bytes);'),
    ('core/src/fwd_router.cpp:2873',
    ...
    ('core/src/graph.cpp:1813',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:2044', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2299',
    ...
    ('core/src/graph.cpp:2345', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2502', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2599',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2730', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3269', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3440',
