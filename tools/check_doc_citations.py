...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3761",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3801", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4552", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3635", "Everything else under `settings`"),
    ("core/src/graph.cpp:3833", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3414", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3534", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3574", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3468", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:4010", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4062", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4074", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4228", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4489", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4486", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4660", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1733', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1798', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2356', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2360', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2370', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2373', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2377', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2388', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2481', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2487', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2493', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2494', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2518', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2522', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2779', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2791', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2910', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2944', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1769',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2222', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2228', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1941", "has_registered_child()"),
    ("core/src/graph.cpp:2108", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2300", "graph_t::write_impl"),
    ("core/src/graph.cpp:2466", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2468", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2483", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2484", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2486", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2502", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2503", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2508", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2853", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3386", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3576", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ('bench/bench_libtracer.cpp:1486',
...
('core/include/libtracer/graph.hpp:1590',
    ...
    ('core/include/libtracer/mem_source.hpp:252', '[[nodiscard]] block_source_t& heap_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:273', '[[nodiscard]] block_source_t& null_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:298', 'class bump_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:340', 'void reset() noexcept {'),
    ('core/include/libtracer/mem_source.hpp:480', 'class pool_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:284',
     '*       `monotonic_buffer_resource` also spills past its buffer, but it spills to a'),
    ('core/include/libtracer/mem_source.hpp:289',
     '* @warning SCOPE-LIFETIME USE ONLY. A bump block is never reclaimed, so a source that'),
    ('core/include/libtracer/mem_source.hpp:292',
     '*          between operations. It is NOT a long-lived seam: an 8 KiB bump source wired as'),
    ('core/include/libtracer/mem_source.hpp:293',
     "*          a router's `rx` decoded 6 frames and rejected the next 194 — measured. A"),
    ('core/include/libtracer/mem_source.hpp:295',
     '* @note Single-threaded by contract — a bump cursor is not synchronized. Its intended use'),
    ('core/include/libtracer/mem_source.hpp:306',
     '[[nodiscard]] void* try_alloc(std::size_t bytes, std::size_t align) noexcept override {',
     ': block_source_t("bump"), buf_(buffer), upstream_(&upstream) {}'),
    ('core/include/libtracer/mem_source.hpp:497',
     'pool_source_t(std::span<std::byte> slab, std::span<size_class_t> classes) noexcept'),
    ('core/include/libtracer/mem_source.hpp:559',
     '[[nodiscard]] std::size_t classes_used() const noexcept { return n_; }'),
    ('core/include/libtracer/mem_source.hpp:570',
    ...
    ('core/include/libtracer/rope.hpp:329',
     '[[nodiscard]] bool try_to_iovec(std::vector<std::span<const std::byte>>& out) const noexcept {'),
    ('core/include/libtracer/rope.hpp:427', 'static constexpr std::size_t kInline = 2;'),
    ...
    ('core/src/graph.cpp:1609', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1616', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1651',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1974', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:2002', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:2016', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:2020', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:2028', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:2097',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2148',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2155', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2165',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2183',
    ...
    ('core/src/graph.cpp:2254', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2343',
    ...
    ('core/src/graph.cpp:2387',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2400', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2453',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2480',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2598', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2837',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3644', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4691', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1706', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1706',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3344', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:2168', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2465',
    ...
    ('core/src/graph.cpp:2511', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2668', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2765',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2923', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3476', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3647',
