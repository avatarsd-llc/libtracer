...
repo-relative path (`core/src/graph.cpp:946`), the design pages' established
BASENAME shorthand (`graph.hpp:1053`), and a bare continuation (`:304`) that
...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1846", "graph_t::set_identity"),
    ("core/src/graph.cpp:1872", "graph_t::read_identity"),
    ("core/src/graph.cpp:2205", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:646", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:652", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:664", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:538", "Slot addresses are NOT stable"),
    ("core/include/libtracer/fwd_router.hpp:153", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:212", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:262", "subscribe_toward"),
    ("core/include/libtracer/fwd_router.hpp:359", "using reply_fn_t"),
    ("core/include/libtracer/fwd_router.hpp:370", "using stale_label_fn_t"),
    ...
    ("core/src/graph.cpp:1587", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1654", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1691", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1612", "step0.wildcard"),
    ("core/src/graph.cpp:2171", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2271", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ("core/src/op_resolve_walk.hpp:333", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:965", 'field.steps[0].name != "subscribers"'),
    ...
    ("core/src/graph.cpp:1113", "!arena"),
    ("core/include/libtracer/segment.hpp:78", "struct segment_t"),
    # --- the design + module pages (#728). Every one of these had drifted. ---
    ("core/src/graph.cpp:821", "has_registered_child()"),
    ("core/src/graph.cpp:911", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1034", "graph_t::write_impl"),
    ("core/src/graph.cpp:1096", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1097", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1109", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1110", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1112", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1128", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1129", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1131", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1333", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1566", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1693", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1617", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1646", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1647", "flatten OOM"),
    ("core/src/fwd_router.cpp:1651", "try_encode_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1683", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1298", "route_flat.empty()"),
    ("core/src/fwd_router.cpp:1318", "payload_flat.empty()"),
    ("core/src/fwd_router.cpp:1311", "frame.subrope(head->child1_off, head->child1_total).materialize", "case type_t::COMPACT"),
    ("core/src/fwd_router.cpp:932", "frame.subrope(0, frame.total_length()).materialize"),
    ...
    ("core/src/op_resolve_walk.hpp:190", "view_t own_wire(mem::mem_backend_t& flat)"),
    ("core/src/op_resolve_view.cpp:146", "over_bytes(sub.only().bytes(), flat)"),
    ("core/src/op_resolve_view.cpp:254", "wire().materialize(backend())"),
    ("core/src/op_resolve_walk.hpp:567", "view::heap_alloc(head_len)"),
    ("core/src/op_resolve_walk.hpp:665", "rope_t or_backpressure"),
    ("core/src/fwd_router.cpp:1216", "decode_into(frame, rx_for(inbound_ctx))"),
    ("core/include/libtracer/vertex.hpp:2471", "vertex_t* parent_"),
    ("core/include/libtracer/vertex.hpp:2471", "vertex_t* parent_"),
    ...
    ('bench/bench_lkv_slot.cpp:192', 'class model_sp_atomic_t {'),
    ('bench/bench_lkv_slot.cpp:341', 'class model_hazard_t {'),
    ('bench/bench_lkv_slot.cpp:455', 'class model_hazard_ref_t {'),
    ...
    ('core/include/libtracer/config.hpp.in:153', 'static constexpr std::size_t kMaxVertexBytes64 = 120;'),
    ('core/include/libtracer/config.hpp.in:189', 'static constexpr std::uint32_t kPinPayloadRatio = 0;'),
    ('core/include/libtracer/config.hpp.in:198', 'using acl_policy_t = @LIBTRACER_ACL_POLICY@;'),
    ('core/include/libtracer/config.hpp.in:165', 'static constexpr std::size_t kMaxVertexBytes32 = 80;'),
    ('core/include/libtracer/config.hpp.in:211',
     '* `-DLIBTRACER_LKV_SLOT=<type>`. The named type must satisfy the policy contract in'),
    ('core/include/libtracer/config.hpp.in:214', 'using lkv_slot_t = @LIBTRACER_LKV_SLOT@;'),
    ('core/include/libtracer/config.hpp.in:223', 'using config_t = default_config_t;'),
    ('core/include/libtracer/config.hpp.in:225',
     '// ---------------------------------------------------------------------------------------------'),
    ...
    ('core/include/libtracer/fwd_frame_view.hpp:554',
     '* @brief A control frame (ADVERTISE / COMPACT / HANDLE_NACK) peeked off any'),
    # core/include/libtracer/fwd_router.hpp
    ('core/include/libtracer/fwd_router.hpp:96',
     "* @param flat  The byte backend EVERY rope flatten on the router's forward AND terminus"),
    ('core/include/libtracer/fwd_router.hpp:154',
     'std::pmr::memory_resource* mr = std::pmr::get_default_resource(),'),
    ('core/include/libtracer/fwd_router.hpp:155', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:156', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ('core/include/libtracer/fwd_router.hpp:375',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:765',
    ...
    ('core/include/libtracer/graph.hpp:723', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:741',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:779', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:893', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:917',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:1014', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1016', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1018', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1020', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1031', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1063', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1069', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1075', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1076', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1079', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1083', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1239', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1248', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1299',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1302',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1308', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/vertex.hpp:141',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:146', 'class value_ref_t {'),
    ('core/include/libtracer/vertex.hpp:572', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:759', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:762', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:802',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:846', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:862',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:870', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:880', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:884', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1028',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1191', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ...
    ('core/include/libtracer/vertex.hpp:1672', 'const bool use_heap = subs_.size() > edge_snapshot_t::kCapacity &&'),
    ('core/include/libtracer/vertex.hpp:1677',
    ...
    ('core/include/libtracer/vertex.hpp:2532',
    ...
    ('core/src/fwd_router.cpp:536',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:883',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:889', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:950', '// A REPLY that reaches its originator here is handed to the sink rope-native'),
    ('core/src/fwd_router.cpp:1264',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1277', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1290', 'const view_t route_flat ='),
    ('core/src/fwd_router.cpp:1291',
    ...
    ('core/src/fwd_router.cpp:1632', "// dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1)."),
    ('core/src/fwd_router.cpp:1667',
    ...
    ('core/src/graph.cpp:624', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:846', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:859', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:866', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:870', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:878', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:901', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:944',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:951', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:960', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:973', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:980',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:1001', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1040', '// a subtree sweep. propagate(v) is the separate accumulate-then-flush primitive.'),
    ('core/src/graph.cpp:1043', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1059', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1064', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1085',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1106',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1202', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1320',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1488', 'field_path_t field;'),
    ('core/src/graph.cpp:1748', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2302', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/op_resolve_walk.hpp:76', "*        the u16 the kind=ERROR reply's ERROR{VALUE} identity carries."),
    ('core/src/op_resolve_walk.hpp:522',
     'void tlv_sliced(std::span<const std::byte> wire) {  // trailer-sliced whole-TLV copy (§4)'),
    ('core/src/op_resolve_walk.hpp:576', 'out.tlv_sliced(reply_dst_wire);'),
    ('core/src/op_resolve_walk.hpp:943',
     'if (!req.src.spans_intact()) return std::unexpected(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:1030', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:866', 'if (value.total_length() == 0)'),
    # core/src/path.cpp
    ...
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:40',
     '* reply, and (the deep path) the whole /unit batch-apply transaction. The device'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:47', 'constexpr std::size_t kHttpdTaskStack = 12288;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:150', 'if (chunk.empty()) return true;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:156',
...
def citation_spans(context: str, filemap: dict = None) -> tuple:
    ...
    the design pages' basename shorthand `graph.hpp:1053`, a generated header
    `config.hpp.in:214`, a range `file.cpp:12-20`, a comma list `file.hpp:145,153`, and
