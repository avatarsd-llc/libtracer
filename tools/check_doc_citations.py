...
ANCHORS = [
    ...
    ('core/include/libtracer/tlv.hpp:50', 'ROUTER = 0x0D, /**< @brief Router-wrapped frame. */'),
    ("core/src/graph.cpp:2732",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2772", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3237", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/transport_vertex.cpp:98", "graph_.register_child_type(",
    ...
    ("core/include/libtracer/fwd_router.hpp:850", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:856", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:902", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:177", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:273", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:433", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:2388", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2507", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2547", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2441", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2931", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2977", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2989", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3133", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3174", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3171", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3317", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:349", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:928", 'field.steps[0].name != "subscribers"'),
    ...
    ('core/src/op_resolve_walk.hpp:994', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ('core/include/libtracer/graph.hpp:1147', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1203', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1558', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1562', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1572', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1575', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1579', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1590', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1679', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1685', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1691', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1692', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1716', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1720', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1922', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1932', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2044', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1183', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1458', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1464', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1267", "has_registered_child()"),
    ("core/src/graph.cpp:1401", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1539", "graph_t::write_impl"),
    ("core/src/graph.cpp:1652", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1654", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1669", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1670", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1672", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1688", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1689", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1694", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1988", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2360", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2549", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2213", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2251", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2252", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2254", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1795", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1814", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1807", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1467", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:1713", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('bench/bench_libtracer.cpp:1249',
    ...
    ('core/include/libtracer/fwd_router.hpp:1124',
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/include/libtracer/transport.hpp:416',
     'virtual void send(std::span<const std::span<const std::byte>> iov) {'),
    ('core/include/libtracer/transport.hpp:572',
    ...
    ('core/include/libtracer/transport_can.hpp:522', '[[nodiscard]] bus_link_t* bus() override { return this; }'),
    ('core/include/libtracer/transport_can.hpp:541',
    ...
    ('core/include/libtracer/vertex.hpp:231',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:236', 'class value_ref_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:715', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1416', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2224', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2241', '++drops.out_of_memory;'),
    ...
    ('core/src/fwd_router.cpp:741',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1435',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1441', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1497', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1823',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1774', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1788', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1838', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2230',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2311',
    ...
    ('core/src/graph.cpp:993', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1000', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1035',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1300', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1328', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1335', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1339', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1347', 'if (!try_clone_rope(clone, value)) {'),
    ...
    ('core/src/graph.cpp:1441', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1451',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1469',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1477',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1506', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1558', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1586', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1599', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1610', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1639',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1666',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1784', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1972',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2617', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3348', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/fwd_reply.hpp:109', 'void tlv_sliced(std::span<const std::byte> wire) {'),
    ('core/src/op_resolve_walk.hpp:882',
    ...
    ('core/src/fwd_router.cpp:875', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:829', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1077', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1077',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2342', 's.target_key.reset();'),
    ...
    ('core/src/route_handle.cpp:257', 't->egress.push_back(egress_entry_t{', 'std::pair<std::uint16_t, bool> route_handle_t::ensure_egress(std::string_view out_link,'),
    ...
    ('core/tests/CMakeLists.txt:1508', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1521', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1522', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('integrations/esp-idf/libtracer/CMakeLists.txt:297', 'if(CONFIG_FREERTOS_UNICORE)'),
    ...
    ('core/include/libtracer/vertex.hpp:376', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/src/graph.cpp:1454', '// nested publish must not be able to swallow this tally).'),
    ...
    ('core/src/graph.cpp:1697', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1830', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1927',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2160', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2449', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2620',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2804', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
    ('core/src/op_resolve_walk.hpp:297',
     'p.mint_request = (op_byte & kFwdOpFlagMintRequest) != 0;'),
    ('core/src/op_resolve_walk.hpp:303',
    ...
    ('core/src/op_resolve_walk.hpp:800',
     'graph.write(v, has_field ? field : field_path_t{}, value, inbound_link);'),
    ('core/src/op_resolve_walk.hpp:965',
