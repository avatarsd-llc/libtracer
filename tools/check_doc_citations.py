...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2941", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3479", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3528", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3540", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3686", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3733", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3730", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3876", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1430', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1495', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1981', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1985', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1995', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1998', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2002', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2013', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2102', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2108', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2114', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2115', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2139', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2143', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2355', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2365', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2478', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1466',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1881', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1887', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1635", "has_registered_child()"),
    ("core/src/graph.cpp:1769", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1914", "graph_t::write_impl"),
    ("core/src/graph.cpp:2027", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2029", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2044", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2045", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2047", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2063", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2064", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2069", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2363", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2860", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3049", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2993", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:3031", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:3032", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:3034", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2417", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2436", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2429", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1891", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2323", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:1294',
    ...
    ('core/src/fwd_router.cpp:1858',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1865', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1922', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2445',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2396', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2410', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2462', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:3010',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3091',
    ...
    ('core/src/graph.cpp:1668', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1696', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1703', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1707', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1715', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1758',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1809',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1816', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1826',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1844',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1852',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1881', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1933', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1961', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1974', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1985', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2014',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2041',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2159', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2347',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3117', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3907', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:972', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:910', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1445', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1445',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2827', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1618', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1633', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1634', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/src/fwd_router.cpp:2907',
     'graph_.count_external_drop(graph::graph_t::external_drop_t::OUT_OF_MEMORY, 1);',
     'const auto payload_view = view::over_bytes(payload);'),
    ('core/src/fwd_router.cpp:3033', 'if (fresh) emit_advertise(*link, label, route);'),
    ('core/src/fwd_router.cpp:3094',
    ...
    ('core/src/graph.cpp:2072', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2205', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2302',
    ...
    ('core/src/graph.cpp:2949', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3120',
