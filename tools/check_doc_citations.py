...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2777",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2817", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3282", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:878", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:884", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:930", "The forward hop, read entirely by OFFSET"),
    ...
    ("core/src/graph.cpp:2486", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2976", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3022", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:3034", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3178", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3219", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3216", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3362", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1199', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1255', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1629', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1633', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1643', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1646', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1650', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1661', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1750', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1756', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1762', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1763', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1787', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1791', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1993', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2003', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2116', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1235', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1529', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1535', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:2405", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2594", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2231", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2269", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2270", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2272", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ('core/include/libtracer/fwd_router.hpp:1152',
...
('core/include/libtracer/graph.hpp:1110',
    ...
    ('core/src/graph.cpp:2662', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3393', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2375', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1517', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1530', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1531', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/src/fwd_router.cpp:2271', 'if (fresh) emit_advertise(*link, label, route);'),
    ('core/src/fwd_router.cpp:2332',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x40}, std::byte{0x00},'),
    ('core/src/fwd_router.cpp:2333',
     'std::byte{0x00}};',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x40}, std::byte{0x00},'),
    ...
    ('core/src/graph.cpp:2494', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2665',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2849', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
