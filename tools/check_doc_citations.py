...
BASENAME shorthand (`graph.hpp:1061`), and a bare continuation (`:304`) that
...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2229", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:704", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:710", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:722", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:569", "Slot addresses are NOT stable"),
    ("core/include/libtracer/fwd_router.hpp:175", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:237", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:384", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:2157", "view::segment_alloc(*value_backend_, hdr_len(n.body_len))"),
    ("core/src/graph.cpp:2195", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2295", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ...
    ('core/include/libtracer/fwd_router.hpp:176',
     'std::pmr::memory_resource* mr = std::pmr::get_default_resource(),'),
    ('core/include/libtracer/fwd_router.hpp:177', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:178', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ('core/include/libtracer/fwd_router.hpp:400',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:833',
    ...
    ('core/include/libtracer/graph.hpp:201',
     '* @param ctl The #551 nothrow seam every FAILABLE allocation draws from — the ones a'),
    ('core/include/libtracer/graph.hpp:214',
     'explicit graph_t(std::pmr::memory_resource* mr = std::pmr::get_default_resource(),'),
    ('core/include/libtracer/graph.hpp:227',
     '[[nodiscard]] mem::block_source_t& control_source() const noexcept { return *ctl_; }'),
    ('core/include/libtracer/graph.hpp:282',
     '* already-retired or unregistered vertex succeeds and does nothing. The root cannot be'),
    ('core/include/libtracer/graph.hpp:560',
     '[[nodiscard]] result_t<value_ref_t> read(vertex_handle_t v, std::string_view caller = {}) const;'),
    ('core/include/libtracer/graph.hpp:614',
     '* makes host-side after registration — and it has **no wire surface at all**: no peer'),
    ('core/include/libtracer/graph.hpp:647',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:734', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:752',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:790', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:904', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:928',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:1025', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1027', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1029', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1031', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1042', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1074', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1080', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1086', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1087', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1090', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1094', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1250', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1259', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1310',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1313',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1319', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/src/graph.cpp:2326', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
...
def citation_spans(context: str, filemap: dict = None) -> tuple:
    ...
    the design pages' basename shorthand `graph.hpp:1061`, a generated header
