...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3110",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3150", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3672", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2764", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2883", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2923", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2817", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3355", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3404", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3416", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3562", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3609", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3606", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3752", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1357', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1422', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1896', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1900', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1910', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1913', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1917', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1928', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2017', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2023', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2029', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2030', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2054', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2058', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2260', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2270', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2383', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1393',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1796', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1802', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:2736", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2925", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ('core/include/libtracer/config.hpp:240', 'static constexpr std::uint32_t kPinPayloadRatio = 0;'),
    ('core/include/libtracer/config.hpp:249', 'using acl_policy_t = allow_only_policy_t;'),
...
('core/include/libtracer/graph.hpp:1221',
    ...
    ('core/src/graph.cpp:2993', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3783', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2703', 'delivery_link.assign(split.link);'),
    ...
    ('.github/workflows/core-ci.yml:384', 'name: tsan (slot=${{ matrix.lkv_slot }})'),
    ...
    ('.github/workflows/core-ci.yml:391', '-DCMAKE_CXX_FLAGS="-fsanitize=thread -g -O1"'),
    ...
    ('core/src/graph.cpp:2825', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2996',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:3182', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
