...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2667",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2707", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3145", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2323", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2442", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2482", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2376", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2866", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2912", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2924", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3041", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3082", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3079", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3225", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1220", "has_registered_child()"),
    ("core/src/graph.cpp:1346", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1484", "graph_t::write_impl"),
    ("core/src/graph.cpp:1597", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1599", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1614", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1615", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1617", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1633", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1634", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1636", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1923", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2295", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2484", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/src/graph.cpp:1245', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1273', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1280', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1284', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1292', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1335',
    ...
    ('core/src/graph.cpp:1386', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1396',
    ...
    ('core/src/graph.cpp:1451', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1503', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1531', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1544', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1555', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1584',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1611',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1723', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1907',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2552', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3256', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2277', 's.target_key.reset();'),
    ...
    ('core/src/graph.cpp:1198',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:1399', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:1596',
    ...
    ('core/src/graph.cpp:1639', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1765', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1862',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2095', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2384', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2555',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2739', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
