...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3771",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3811", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4562", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3645", "Everything else under `settings`"),
    ("core/src/graph.cpp:3843", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3424", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3544", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3584", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3478", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:4020", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4072", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4084", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4238", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4499", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4496", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4670", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1951", "has_registered_child()"),
    ("core/src/graph.cpp:2118", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2310", "graph_t::write_impl"),
    ("core/src/graph.cpp:2476", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2478", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2493", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2494", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2496", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2512", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2513", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2518", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2863", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3396", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3586", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1603',
    ...
    ('core/src/graph.cpp:1619', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1626', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1661',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1984', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:2012', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:2026', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:2030', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:2038', 'if (!try_clone_rope(clone, value)) {'),
    ...
    ('core/src/graph.cpp:2165', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2175',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2193',
    ...
    ('core/src/graph.cpp:2264', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2353',
    ...
    ('core/src/graph.cpp:2397',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2410', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2463',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2490',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2608', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2847',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3654', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4701', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1716', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:3354', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:1944',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:2178', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2475',
    ...
    ('core/src/graph.cpp:2521', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2678', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2775',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2933', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3486', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3657',
