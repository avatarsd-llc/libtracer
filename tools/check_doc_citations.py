...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2748", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3213", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2364", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2483", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2523", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2417", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2907", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2953", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2965", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3109", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3150", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3147", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3293", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1544', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1548', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1558', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1561', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1565', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1576', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1658', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1664', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1670', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1671', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1695', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1699', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1901', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1911', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ...
    ("core/src/graph.cpp:1243", "has_registered_child()"),
    ("core/src/graph.cpp:1377", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1515", "graph_t::write_impl"),
    ("core/src/graph.cpp:1628", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1630", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1645", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1646", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1648", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1664", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1665", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1670", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1964", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2336", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2525", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2211", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2249", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2250", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2252", "emit_compact", "fwd_router_t::deliver_remote"),
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/src/graph.cpp:969', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:976', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1011',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1276', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1304', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1311', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1315', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1323', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1366',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1410',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1417', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1427',
    ...
    ('core/src/graph.cpp:1482', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1534', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1562', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1575', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1586', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1615',
    ...
    ('core/src/graph.cpp:1760', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1948',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2593', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3324', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1053', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1053',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2318', 's.target_key.reset();'),
    ...
    ('core/include/libtracer/graph.hpp:1603',
    ...
    ('core/src/fwd_router.cpp:2143',
     'graph_.count_external_drop(graph::graph_t::external_drop_t::OUT_OF_MEMORY, 1);',
     'const auto payload_view = view::over_bytes(payload);'),
    ('core/src/fwd_router.cpp:2251', 'if (fresh) emit_advertise(*link, label, route);'),
    ('core/src/fwd_router.cpp:2312',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x40}, std::byte{0x00},'),
    ('core/src/fwd_router.cpp:2313',
     'std::byte{0x00}};',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x40}, std::byte{0x00},'),
    ('core/src/graph.cpp:630', 'return acl_allows(v.get(), caller, right);'),
    ('core/src/graph.cpp:932',
     'if (ancestor != nullptr && !acl_allows(ancestor, caller, acl_right_t::CREATE))'),
    ('core/src/graph.cpp:996',
    ...
    ('core/src/graph.cpp:1673', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1806', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1903',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2136', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2425', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2596',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2780', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
