...
ANCHORS = [
    ...
    ('core/include/libtracer/graph.hpp:390',
    ...
    ('core/include/libtracer/graph.hpp:564', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:582',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:620', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:734', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:758',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:855', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:857', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:859', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:861', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:872', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:904', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:910', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:916', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:917', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:920', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:924', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1053', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1062', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1113',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1116',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1122', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/vertex.hpp:504', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:691', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:694', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:734',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:778', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:794',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:802', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:812', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:816', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:959',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1122', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1599',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow) {'),
    ('core/include/libtracer/vertex.hpp:1603', 'const bool use_heap = subs_.size() > edge_snapshot_t::kCapacity &&'),
    ('core/include/libtracer/vertex.hpp:1608',
    ...
    ('core/include/libtracer/vertex.hpp:2448',
    ...
    ('core/src/graph.cpp:536', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:566',
     'const std::shared_lock lock(map_mutex_);',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:758', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:771', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:778', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:782', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:790', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:813', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:856',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:863', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:872', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:885', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:892',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:913', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:952', '// a subtree sweep. propagate(v) is the separate accumulate-then-flush primitive.'),
    ('core/src/graph.cpp:955', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:971', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:976', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:997',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1018',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1114', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1232',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1400', 'field_path_t field;'),
    ('core/src/graph.cpp:1660', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2214', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
