...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2801",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2841", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3306", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2457", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2576", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2616", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2510", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3000", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3046", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:3058", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3202", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3243", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3240", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3386", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1291", "has_registered_child()"),
    ("core/src/graph.cpp:1425", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1563", "graph_t::write_impl"),
    ("core/src/graph.cpp:1676", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1678", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1693", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1694", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1696", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1712", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1713", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1718", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2012", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2429", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2618", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2313", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2351", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2352", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2354", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1823", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1842", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1835", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1495", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:1741", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:1164',
    ...
    ('core/src/fwd_router.cpp:1463',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1469', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1525', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1851',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1802', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1816', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1866', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2330',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2411',
    ...
    ('core/src/graph.cpp:1017', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1024', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1059',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1324', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1352', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1359', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1363', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1371', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1414',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1458',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1465', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1475',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1493',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1501',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1530', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1582', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1610', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1623', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1634', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1663',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1690',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1808', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1996',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2686', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3417', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:837', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1101', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:2399', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:1721', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1854', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1951',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2184', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2518', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2689',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2873', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
