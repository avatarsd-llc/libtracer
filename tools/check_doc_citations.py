...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2675",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2715", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3153", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2331", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2450", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2490", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2384", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2874", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2920", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2932", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3049", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3090", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3087", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3233", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1671', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1677', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1683', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1684', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1708', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1712', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1914', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1924', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2036', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ("core/src/graph.cpp:1492", "graph_t::write_impl"),
    ("core/src/graph.cpp:1605", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1607", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1622", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1623", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1625", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1641", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1642", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1644", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1931", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2303", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2492", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/include/libtracer/graph.hpp:2027',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2030',
    ...
    ('core/include/libtracer/vertex.hpp:688', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:959', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:962', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:1219', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1243', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1253', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1257', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1594', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2286', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:3082', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:3099', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:1175',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:1235',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:1410',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:2336',
    ...
    ('core/src/graph.cpp:1459', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1511', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1539', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1552', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1563', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1592',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1619',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1731', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1915',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2560', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3264', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2285', 's.target_key.reset();'),
    ...
    ('core/include/libtracer/vertex.hpp:668', 'enum class delivery_mode_t : std::uint8_t {'),
    ('core/include/libtracer/vertex.hpp:1056', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/vertex.hpp:1154',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:2344',
    ...
    ('core/include/libtracer/vertex.hpp:3182', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:3259',
    ...
    ('core/src/graph.cpp:1647', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1773', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1870',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2103', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2392', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2563',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2747', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
