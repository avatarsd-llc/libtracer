...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2435", "graph_t::set_identity"),
    ("core/src/graph.cpp:2461", "graph_t::read_identity"),
    ("core/src/graph.cpp:2880", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:815", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:821", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:867", "The forward hop, read entirely by OFFSET"),
    ...
    ("core/src/graph.cpp:2116", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2235", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2275", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2169", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2601", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2647", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2659", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2776", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2817", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2814", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2960", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1031', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1087', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1401', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1405', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1415', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1418', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1422', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1433', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1509', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1515', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1521', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1522', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1546', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1550', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1735', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1744', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1833', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1067', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1300', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1306', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1054", "has_registered_child()"),
    ("core/src/graph.cpp:1165", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1297", "graph_t::write_impl"),
    ("core/src/graph.cpp:1410", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1412", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1427", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1428", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1430", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1446", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1447", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1449", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1736", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2088", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2277", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2010", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2048", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2049", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2051", "emit_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2082", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1596", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1615", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1608", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1288", "frame.subrope(0, frame.total_length()).try_materialize"),
    ...
    ("core/src/fwd_router.cpp:1514", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:1058',
...
('core/include/libtracer/graph.hpp:944',
    ...
    ('core/include/libtracer/graph.hpp:1827',
    ...
    ('core/include/libtracer/vertex.hpp:2069', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2859', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2876', '++drops.out_of_memory;'),
    ...
    ('core/src/fwd_router.cpp:646',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1256',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1262', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1309', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1624',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1575', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1589', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1639', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2027',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2066',
    ...
    ('core/src/graph.cpp:783', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:790', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:825',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1079', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1107', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1114', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1118', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1126', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1155', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:1198',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1205', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1215',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1233',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1241',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1264', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1316', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1344', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1357', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1368', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1397',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1424',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1536', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1720',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2345', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2991', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:780', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:734', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:867', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:868', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:2074', 's.target_key.reset();'),
