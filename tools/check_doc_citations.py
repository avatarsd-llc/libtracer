...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2374", "graph_t::set_identity"),
    ("core/src/graph.cpp:2400", "graph_t::read_identity"),
    ("core/src/graph.cpp:2819", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2055", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2174", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2214", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2108", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2540", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2586", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2598", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2715", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2756", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2753", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2899", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1350', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1354', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1364', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1367', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1371', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1382', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1458', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1464', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1470', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1471', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1495', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1499', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1677', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1686', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1754', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1016', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1249', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1255', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:993", "has_registered_child()"),
    ("core/src/graph.cpp:1104", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1236", "graph_t::write_impl"),
    ("core/src/graph.cpp:1349", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1351", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1366", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1367", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1369", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1385", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1386", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1388", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1675", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2027", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2216", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1949", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1987", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1988", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:1990", "emit_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2021", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1535", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1554", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1547", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1227", "frame.subrope(0, frame.total_length()).try_materialize"),
    ...
    ("core/src/fwd_router.cpp:1453", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:893',
    ...
    ('core/include/libtracer/graph.hpp:1745',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1748',
    ...
    ('core/src/fwd_router.cpp:1195',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1201', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1248', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1563',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1514', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1528', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1578', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:1966',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2005',
    ...
    ('core/src/graph.cpp:764',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1018', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1046', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1053', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1057', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1065', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1094', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:1137',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1144', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1154',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1172',
    ...
    ('core/src/graph.cpp:1203', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1255', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1283', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1296', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1307', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1336',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1363',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1475', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1659',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2284', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2930', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/fwd_router.cpp:707', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:806', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:807', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:2013', 's.target_key.reset();'),
