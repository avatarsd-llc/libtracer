...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2173", "graph_t::set_identity"),
    ("core/src/graph.cpp:2199", "graph_t::read_identity"),
    ("core/src/graph.cpp:2590", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:1855", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1972", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2012", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:1906", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2339", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2385", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2397", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2514", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2555", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2552", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2670", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1253", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1254", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1266", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1267", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1269", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1285", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1286", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1288", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1499", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1827", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2014", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1817", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1849", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1850", "flatten OOM"),
    ("core/src/fwd_router.cpp:1854", "try_encode_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1886", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1442", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1461", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1454", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1141", "frame.subrope(0, frame.total_length()).materialize"),
    ...
    ("core/src/fwd_router.cpp:1360", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/graph.hpp:769',
     '[[nodiscard]] result_t<value_ref_t> read(vertex_handle_t v, std::string_view caller = {}) const;'),
('core/include/libtracer/graph.hpp:856',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:943', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:999', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1282', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1284', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1286', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1289', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1293', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1304', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1336', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1342', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1348', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1349', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1366', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1370', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1542', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1551', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1607',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1610',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1616', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/vertex.hpp:3097',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3100',
    ...
    ('core/src/fwd_router.cpp:563',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1110',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1116', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1159', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1470',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1421', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1435', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1481', 'hold = frame.subrope(off, total).materialize(*flat_);'),
    ('core/src/fwd_router.cpp:1832', "// dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1)."),
    ('core/src/fwd_router.cpp:1870',
    ...
    ('core/src/graph.cpp:1208', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1216', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1221', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1242',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1263',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1359', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1486',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2075', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2701', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/include/libtracer/graph.hpp:979', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1202', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1208', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ('core/src/graph.cpp:1813', 's.target_key.reset();'),
