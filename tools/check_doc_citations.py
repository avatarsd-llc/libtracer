...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2669",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2709", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3147", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2325", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2444", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2484", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2378", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2868", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2914", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2926", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3043", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3084", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3081", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3227", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1145', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1201', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1557', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1561', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1571', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1574', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1578', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1589', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1665', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1671', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1677', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1678', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1702', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1706', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1908', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1918', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2030', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1181', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1456', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1462', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1228", "has_registered_child()"),
    ("core/src/graph.cpp:1354", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1486", "graph_t::write_impl"),
    ("core/src/graph.cpp:1599", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1601", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1616", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1617", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1619", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1635", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1636", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1638", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1925", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2297", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2486", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2207", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2245", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2246", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2248", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    # ever above this one.
    ("core/src/fwd_router.cpp:2318", "std::vector<std::span<const std::byte>> iov;", "Default: full-route"),
    ...
    ("core/src/fwd_router.cpp:1711", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('bench/bench_libtracer.cpp:1248',
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/include/libtracer/graph.hpp:2021',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2024',
    ...
    ('core/include/libtracer/vertex.hpp:209',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:214', 'class value_ref_t {'),
    ('core/include/libtracer/vertex.hpp:641', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:912', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:915', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:1172', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1196', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1206', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1210', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1547', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2239', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:3035', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:3052', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:1128',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:1188',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:1363',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:2289',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow,'),
    ('core/include/libtracer/vertex.hpp:3041',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:3047',
     'if (src[i].active.load(std::memory_order_acquire)) ++drops.truncated;'),
    ('core/include/libtracer/vertex.hpp:3364',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3369',
    ...
    ('core/src/fwd_router.cpp:739',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1433',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1439', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1495', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1821',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1772', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1786', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1836', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2224',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2302',
    ...
    ('core/src/graph.cpp:952', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:959', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:994',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1253', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1281', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1288', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1292', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1300', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1343',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1387',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1394', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1404',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1422',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1430',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1453', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1505', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1533', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1546', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1557', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1586',
    ...
    ('core/src/graph.cpp:1725', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1909',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2554', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3258', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1036', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1036',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2279', 's.target_key.reset();'),
    ...
    ('core/tests/CMakeLists.txt:1363', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1376', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1377', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/graph.hpp:1616',
    ...
    ('core/src/graph.cpp:1206',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:1407', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:1598',
    ...
    ('core/src/graph.cpp:1641', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1767', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1864',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2097', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2386', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2557',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2741', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
