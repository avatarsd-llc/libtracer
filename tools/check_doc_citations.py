...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3367",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3407", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4141", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3241", "Everything else under `settings`"),
    ("core/src/graph.cpp:3439", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3020", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3140", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3180", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3074", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3616", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3668", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3680", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3826", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4078", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4075", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4221", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1539', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1604', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2125', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2129', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2139', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2142', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2146', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2157', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2250', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2256', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2262', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2263', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2287', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2291', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2517', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2527', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2640', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2655', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1575',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1993', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1999', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1687", "has_registered_child()"),
    ("core/src/graph.cpp:1851", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2030", "graph_t::write_impl"),
    ("core/src/graph.cpp:2145", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2147", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2162", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2163", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2165", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2181", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2182", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2187", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2494", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2992", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3182", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1396',
    ...
    ('core/src/graph.cpp:1375', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1382', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1417',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1720', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1748', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1762', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1766', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1774', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1840',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1891',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1898', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1908',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1926',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1934',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1997', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2049', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2077', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2090',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2103', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2132',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2159',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2277', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2478',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3250', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4252', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1472', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1472',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2959', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:2190', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2336', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2433',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2552', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3082', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3253',
