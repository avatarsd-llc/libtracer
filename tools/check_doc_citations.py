...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1501", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1568", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1605", "field.steps.size() != 1", 'step0.name == "children"'),
    ...
    ("core/src/op_resolve_walk.hpp:315", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:724", 'field.steps[0].name != "subscribers"'),
    ...
    ("core/src/graph.cpp:735", "has_registered_child()"),
    ("core/src/graph.cpp:825", "void graph_t::fan_out"),
    ("core/src/graph.cpp:948", "graph_t::write_impl"),
    ("core/src/graph.cpp:1010", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1011", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1023", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1024", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1026", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1042", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1043", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1045", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1247", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1480", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1607", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1413", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1442", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1443", "flatten OOM"),
    ("core/src/fwd_router.cpp:1447", "try_encode_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1479", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/op_resolve_walk.hpp:548", "view::heap_alloc(head_len)"),
    ("core/src/op_resolve_walk.hpp:625", "rope_t or_backpressure"),
    ...
    ('core/include/libtracer/fwd_frame_view.hpp:418',
     '* @brief A control frame (ADVERTISE / COMPACT / HANDLE_NACK) peeked off any'),
    # core/include/libtracer/fwd_router.hpp
    ('core/include/libtracer/fwd_router.hpp:94',
     "* @param flat  The byte backend EVERY rope flatten on the router's forward AND terminus"),
    ('core/include/libtracer/fwd_router.hpp:152',
     'std::pmr::memory_resource* mr = std::pmr::get_default_resource(),'),
    ('core/include/libtracer/fwd_router.hpp:153', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:154', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ('core/include/libtracer/fwd_router.hpp:286',
    ...
    ('core/include/libtracer/graph.hpp:508',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:595', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:613',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:651', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:765', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:789',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:886', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:888', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:890', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:892', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:903', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:935', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:941', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:947', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:948', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:951', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:955', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1088', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1097', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1148',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1151',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1157', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/path.hpp:30', 'inline constexpr std::size_t kMaxSegmentBytes = 64;'),
    ('core/include/libtracer/path.hpp:32', 'inline constexpr std::size_t kMaxPathBytes = 1024;'),
    ('core/include/libtracer/path.hpp:34', 'inline constexpr std::size_t kMaxSegments = 255;'),
    ('core/include/libtracer/path.hpp:36', 'inline constexpr std::size_t kMaxFieldDepth = 8;'),
    ('core/include/libtracer/path.hpp:129', '* constructor instead.'),
    ('core/include/libtracer/path.hpp:172', 'static constexpr std::size_t kInlineBytes = 16;'),
    ...
    ('core/include/libtracer/vertex.hpp:79',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:84', 'class value_ref_t {'),
    ('core/include/libtracer/vertex.hpp:510', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:697', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:700', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:740',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:784', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:800',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:808', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:818', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:822', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:966',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:1129', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1606',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow) {'),
    ('core/include/libtracer/vertex.hpp:1610', 'const bool use_heap = subs_.size() > edge_snapshot_t::kCapacity &&'),
    ('core/include/libtracer/vertex.hpp:1615',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2452',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:2455',
    ...
    ('core/src/fwd_router.cpp:525',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:718',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:724', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:785', '// A REPLY that reaches its originator here is handed to the sink rope-native'),
    ('core/src/fwd_router.cpp:1060',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1073', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1086', 'const view_t route_flat ='),
    ('core/src/fwd_router.cpp:1087',
    ...
    ('core/src/fwd_router.cpp:1428', "// dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1)."),
    ...
    ('core/src/graph.cpp:538', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:760', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:773', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:780', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:784', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:792', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:815', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:858',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:865', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:874', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:887', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:894',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:915', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:954', '// a subtree sweep. propagate(v) is the separate accumulate-then-flush primitive.'),
    ('core/src/graph.cpp:957', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:973', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:978', '// no notify reclone of the rope on the hot write path.'),
    ...
    ('core/src/graph.cpp:1116', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1234',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1402', 'field_path_t field;'),
    ('core/src/graph.cpp:1662', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2216', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/op_resolve_walk.hpp:76', "*        the u16 the kind=ERROR reply's ERROR{VALUE} identity carries."),
    ('core/src/op_resolve_walk.hpp:504',
     'void tlv_sliced(std::span<const std::byte> wire) {  // trailer-sliced whole-TLV copy (§4)'),
    ('core/src/op_resolve_walk.hpp:557', 'out.tlv_sliced(reply_dst_wire);'),
    ('core/src/op_resolve_walk.hpp:702',
     'if (!req.src.spans_intact()) return std::unexpected(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:747', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ('core/src/op_resolve_walk.hpp:861', 'if (value.total_length() == 0)'),
    # core/src/path.cpp
    ...
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:38',
     '* reply, and (the deep path) the whole /unit batch-apply transaction. The device'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:45', 'constexpr std::size_t kHttpdTaskStack = 12288;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:102', 'struct asm_buf_t {'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:120', 'if (chunk.empty()) return true;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:126',
     'if (len_ != 0) std::memcpy(grown.get(), bytes_.get(), len_);'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:332',
     'payload.reset(new (std::nothrow) std::byte[frame.len]);'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:403',
     'if (frame.type == HTTPD_WS_TYPE_BINARY && frame.final && slot->asm_buf.empty()) {'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:467', 'peer_rx_.deliver_borrowed(peer, frame);'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:476',
     'void httpd_ws_link_t::queue_send(int fd, std::span<const std::span<const std::byte>> iov) {'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:487', 'for (const auto& part : iov) total += part.size();'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:604',
