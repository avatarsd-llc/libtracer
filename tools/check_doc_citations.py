...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2216", "graph_t::set_identity"),
    ("core/src/graph.cpp:2242", "graph_t::read_identity"),
    ("core/src/graph.cpp:2661", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/transport_vertex.cpp:253", "routing key IS the mount path"),
    ("core/src/transport_vertex.cpp:260", "qualified += name"),
    ("core/src/transport_vertex.cpp:279", "grouping vertex, created lazily"),
    ("core/src/transport_vertex.cpp:287", "register_vertex_key(mod_key"),
    ("core/src/transport_vertex.cpp:379", "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:382", "return std::unexpected(status_t::BACKPRESSURE);",
     "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:388", "pending_links_.erase(pl)"),
    ("core/src/transport_vertex.cpp:402", "if (constructed)"),
    ("core/src/transport_vertex.cpp:404", "link_state_t::LISTENING : link_state_t::UP"),
    ...
    ("core/src/graph.cpp:1891", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2008", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2048", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:1942", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2382", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2428", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2440", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2557", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2598", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2595", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2741", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1292", "!arena"),
    ("core/include/libtracer/segment.hpp:78", "struct segment_t"),
    # --- the design + module pages (#728). Every one of these had drifted. ---
    ("core/src/graph.cpp:963", "has_registered_child()"),
    ("core/src/graph.cpp:1068", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1198", "graph_t::write_impl"),
    ("core/src/graph.cpp:1275", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1276", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1288", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1289", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1291", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1307", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1308", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1310", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1521", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1863", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2050", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:1909", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1943", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1944", "flatten OOM"),
    ("core/src/fwd_router.cpp:1946", "emit_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1977", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1527", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1546", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1539", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1226", "frame.subrope(0, frame.total_length()).materialize"),
    ...
    ("core/src/fwd_router.cpp:1445", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:879',
    ...
    ('core/include/libtracer/graph.hpp:1318', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1320', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1322', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1325', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1329', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1340', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1382', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1388', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1394', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1395', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1412', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1416', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1589', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1598', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1657',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1660',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1666', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/rope.hpp:161',
    ...
    ('core/include/libtracer/rope.hpp:180',
     '[[nodiscard]] view_t materialize(mem::mem_backend_t& backend = mem::heap_backend()) const {'),
    ('core/include/libtracer/rope.hpp:216', '[[nodiscard]] rope_t subrope(std::size_t off, std::size_t len) const {'),
    ('core/include/libtracer/rope.hpp:245',
    ...
    ('core/include/libtracer/rope.hpp:262',
     '[[nodiscard]] bool try_to_iovec(std::vector<std::span<const std::byte>>& out) const noexcept {'),
    ('core/include/libtracer/rope.hpp:313', 'static constexpr std::size_t kInline = 2;'),
    ...
    ('core/include/libtracer/vertex.hpp:1447', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2015', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2065',
     'std::size_t snapshot_edges(edge_snapshot_t& inline_buf, std::vector<edge_view_t>& overflow,'),
    ('core/include/libtracer/vertex.hpp:2817', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2823',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2829',
     'if (src[i].active.load(std::memory_order_acquire)) ++drops.truncated;'),
    ('core/include/libtracer/vertex.hpp:2834', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:3144',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3149',
    ...
    ('core/src/fwd_router.cpp:1244', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1555',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1506', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1520', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1566', 'hold = frame.subrope(off, total).materialize(*flat_);'),
    ('core/src/fwd_router.cpp:1926',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:1961',
    ...
    ('core/src/graph.cpp:721', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:728', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:748',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:988', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1016', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1023', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1027', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1035', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1058', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:1101',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1108', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1118',
    ...
    ('core/src/graph.cpp:1165', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1207', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1230', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1238', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1243', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1264',
    ...
    ('core/src/graph.cpp:1381', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1508',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2118', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2772', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:776', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:777', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:1849', 's.target_key.reset();'),
