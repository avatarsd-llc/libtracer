...
ANCHORS = [
    ...
    ("core/src/fwd_router.cpp:1793", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1825", "value.materialize(*flat_)"),
    ("core/src/fwd_router.cpp:1826", "flatten OOM"),
    ("core/src/fwd_router.cpp:1830", "try_encode_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:1862", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1429", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1448", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1441", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1128", "frame.subrope(0, frame.total_length()).materialize"),
    ...
    ("core/src/fwd_router.cpp:1347", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:856',
    ...
    ('core/src/fwd_router.cpp:553',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1097',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1103', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1146', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1457',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1408', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1422', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1468', 'hold = frame.subrope(off, total).materialize(*flat_);'),
    ('core/src/fwd_router.cpp:1808', "// dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1)."),
    ('core/src/fwd_router.cpp:1846',
    ...
    ('core/src/fwd_router.cpp:660', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:614', 'bus->set_peer_rope_receiver('),
