...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2829",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2869", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3391", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2483", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2602", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2642", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2536", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3074", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3123", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3135", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3281", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3328", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3325", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3471", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1253', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1309', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1694', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1698', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1708', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1711', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1715', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1726', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1815', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1821', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1827', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1828', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1852', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1856', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2058', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2068', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2181', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1289', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1594', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1600', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1312", "has_registered_child()"),
    ("core/src/graph.cpp:1446", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1584", "graph_t::write_impl"),
    ("core/src/graph.cpp:1697", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1699", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1714", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1715", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1717", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1733", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1734", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1739", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2033", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2455", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2644", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1164',
    ...
    ('core/include/libtracer/vertex.hpp:1430', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2238', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2255', '++drops.out_of_memory;'),
    ...
    ('core/include/libtracer/vertex.hpp:2622',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:2627',
    ...
    ('core/src/graph.cpp:1038', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1045', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1080',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1345', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1373', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1380', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1384', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1392', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1435',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1479',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1486', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1496',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1514',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1522',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1551', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1603', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1631', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1644', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1655', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1684',
    ...
    ('core/src/graph.cpp:1829', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2017',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2712', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3502', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2422', 'delivery_link.assign(split.link);'),
    ...
    ('core/include/libtracer/vertex.hpp:1488',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2162',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2342', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2419',
    ...
    ('core/src/graph.cpp:1742', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1875', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:1972',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2205', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2544', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2715',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2901', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
