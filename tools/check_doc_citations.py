...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1937", "graph_t::set_identity"),
    ("core/src/graph.cpp:1963", "graph_t::read_identity"),
    ("core/src/graph.cpp:2339", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/transport_vertex.cpp:337", "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:340", "return std::unexpected(status_t::BACKPRESSURE);",
     "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:346", "pending_links_.erase(pl)"),
    ("core/src/transport_vertex.cpp:351", "if (constructed)"),
    ("core/src/transport_vertex.cpp:353", "link_state_t::LISTENING : link_state_t::UP"),
    ...
    ("core/src/graph.cpp:1666", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1745", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1782", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1691", "step0.wildcard"),
    ("core/src/graph.cpp:2103", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2149", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2161", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2272", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2309", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2305", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2405", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ...
    ("core/src/graph.cpp:838", "has_registered_child()"),
    ("core/src/graph.cpp:928", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1051", "graph_t::write_impl"),
    ("core/src/graph.cpp:1113", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1114", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1126", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1127", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1129", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1145", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1146", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1148", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1350", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1645", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:1784", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:857',
    ...
    ('core/include/libtracer/graph.hpp:1271', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1273', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1275', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1277', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1288', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1320', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1326', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1332', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1333', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1336', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1340', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1512', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1521', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1576',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1579',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1585', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/transport_can.hpp:413', '[[nodiscard]] bus_link_t* bus() override { return this; }'),
    ('core/include/libtracer/transport_can.hpp:432',
    ...
    ('core/src/graph.cpp:863', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:876', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:883', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:887', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:895', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:918', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:961',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:968', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:977', 'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf);'),
    ('core/src/graph.cpp:990', 'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf);'),
    ('core/src/graph.cpp:997',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t value) {'),
    ('core/src/graph.cpp:1018', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1060', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1076', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1081', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1102',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1123',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1219', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1337',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:1839', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2436', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1630', 's.target_key.reset();'),
