...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3353",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3393", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3922", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3227", "Everything else under `settings`"),
    ("core/src/graph.cpp:3425", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/transport_vertex.cpp:384", "transport_vertex_t::endpoint_write"),
    ("core/src/transport_vertex.cpp:549", "transport_vertex_t::provide_link"),
    ("core/src/transport_vertex.cpp:648", "routing key IS the mount path"),
    ...
    ("core/src/transport_vertex.cpp:655", "qualified += name", "routing key IS the mount path"),
    ("core/src/transport_vertex.cpp:673", "structural vertex, created lazily"),
    ...
    ("core/src/transport_vertex.cpp:681", "register_vertex_key(mod_key",
     "structural vertex, created lazily"),
    ("core/src/transport_vertex.cpp:814", "if (!router_.add_child(qualified, *link))"),
    ("core/src/transport_vertex.cpp:828", "pending_links_.erase(pl)"),
    ("core/src/transport_vertex.cpp:847", "if (constructed)"),
    ("core/src/transport_vertex.cpp:850", "? link_state_t::LISTENING"),
    ('core/src/transport_vertex.cpp:92', '[[nodiscard]] view_t link_state_value(link_state_t state) {'),
    ('core/src/transport_vertex.cpp:592', 'std::string module;'),
    ...
    ("core/src/transport_vertex.cpp:822", "return std::unexpected(status_t::BACKPRESSURE);",
    ...
    ("core/src/transport_vertex.cpp:755", "handlers.on_children = [bus]() -> result_t<view_t> {"),
    ("core/src/graph.cpp:3006", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3126", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3166", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3060", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3602", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3654", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3666", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3812", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3859", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3856", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4002", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1504', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1569', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2090', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2094', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2104', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2107', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2111', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2122', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2215', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2221', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2227', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2228', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2252', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2256', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2468', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2478', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2591', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2606', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1540',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1958', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1964', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1686", "has_registered_child()"),
    ("core/src/graph.cpp:1850", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2029", "graph_t::write_impl"),
    ("core/src/graph.cpp:2144", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2146", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2161", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2162", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2164", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2180", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2181", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2186", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2480", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2978", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3168", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ('bench/bench_libtracer.cpp:1407',
...
('core/include/libtracer/graph.hpp:1361',
    ...
    ('core/include/libtracer/vertex.hpp:897', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1741', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2599', 'const bool use_heap ='),
    ...
    ('core/src/graph.cpp:1374', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1381', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1416',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1719', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1747', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1761', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1765', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1773', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1839',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1890',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1897', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1907',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1925',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1933',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1996', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2048', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2076', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2102', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2131',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2158',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2276', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2464',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3236', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4033', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/transport_vertex.cpp:665',
    ...
    ('core/src/graph.cpp:1471', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1471',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2945', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1661', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1676', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1677', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/vertex.hpp:389', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:1795',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2517',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2694', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2771',
    ...
    ('core/src/fwd_router.cpp:3099',
     'std::byte{0x00}};',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x00}, std::byte{0x00},'),
    ('core/src/graph.cpp:893', 'return acl_allows(v.get(), caller, right);'),
    ('core/src/graph.cpp:1322',
     'if (ancestor != nullptr && !acl_allows(ancestor, caller, acl_right_t::CREATE))'),
    ('core/src/graph.cpp:1401',
    ...
    ('core/src/graph.cpp:1664',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:1910', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2143',
    ...
    ('core/src/graph.cpp:2189', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2322', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2419',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2538', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3068', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3239',
    ...
    ('core/src/transport_vertex.cpp:557',
