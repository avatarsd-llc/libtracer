...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2496", "graph_t::set_identity"),
    ("core/src/graph.cpp:2522", "graph_t::read_identity"),
    ("core/src/graph.cpp:2941", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:844", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:850", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:896", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:177", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:267", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:427", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:2177", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2296", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2336", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2230", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2662", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2708", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2720", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2837", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2878", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2875", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3021", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:345", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:917", 'field.steps[0].name != "subscribers"'),
    ("core/src/op_resolve_walk.hpp:163", "view_t own_wire(mem::mem_backend_t& flat)"),
    ("core/src/op_resolve_walk.hpp:544", "rope_t or_backpressure"),
    ('core/src/op_resolve_walk.hpp:982', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ('core/include/libtracer/graph.hpp:1089', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1145', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1467', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1471', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1481', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1484', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1488', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1499', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1575', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1581', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1587', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1588', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1612', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1616', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1801', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1810', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1899', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1125', '* @param ctx Caller-owned context; must outlive every possible delivery (edges are'),
    ('core/include/libtracer/graph.hpp:1366', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1372', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/include/libtracer/grammar.hpp:360", "receiver-resource depth bound"),
    ...
    ('core/include/libtracer/grammar.hpp:370',
     '`TLV_NESTING_TOO_DEEP` ("exceeds this receiver'),
    ("core/src/graph.cpp:1488", "!arena"),
    ...
    ("core/src/graph.cpp:1105", "has_registered_child()"),
    ("core/src/graph.cpp:1222", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1354", "graph_t::write_impl"),
    ("core/src/graph.cpp:1467", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1469", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1484", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1485", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1487", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1503", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1504", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1506", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1793", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2149", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2338", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2205", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2243", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2244", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2246", "emit_compact", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2316", "std::vector<std::span<const std::byte>> iov;", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:1791", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1810", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:1803", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1463", "frame.subrope(0, frame.total_length()).try_materialize"),
    ...
    ("core/src/fwd_router.cpp:1709", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/grammar.hpp:458',
...
('core/include/libtracer/graph.hpp:1002',
    ...
    ('core/include/libtracer/vertex.hpp:166',
     "* Holding one keeps that value alive, exactly as the reader's own reference did before. Under"),
    ('core/include/libtracer/vertex.hpp:171', 'class value_ref_t {'),
    ('core/include/libtracer/vertex.hpp:598', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/vertex.hpp:846', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:849', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:1106', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1130', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1140', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1144', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1481', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2173', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2965', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2982', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:1062',
     '// kVertexLockStripes and kCacheLineBytes are ordinary constexprs shared by every TU'),
    ('core/include/libtracer/vertex.hpp:1122',
     'inline constinit std::array<vertex_stripe_t, kVertexLockStripes> vertex_stripes{};'),
    ('core/include/libtracer/vertex.hpp:1297',
     'static constexpr std::size_t kInlineFanout = edge_snapshot_t::kCapacity;'),
    ('core/include/libtracer/vertex.hpp:2223',
    ...
    ('core/src/fwd_router.cpp:737',
     'bool fwd_router_t::add_child(std::string name, transport_t& link, mem::block_source_t* rx) {'),
    ('core/src/fwd_router.cpp:1431',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1437', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1493', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:1819',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame) {'),
    ('core/src/fwd_router.cpp:1770', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:1784', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:1834', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2222',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2300',
    ...
    ('core/src/graph.cpp:834', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:841', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:876',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1130', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1158', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1165', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1169', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1177', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1211',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1255',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1262', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1272',
    ...
    ('core/src/graph.cpp:1321', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1373', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1401', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1414', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1425', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1454',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1481',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1593', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1777',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2406', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3052', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/fwd_reply.hpp:109', 'void tlv_sliced(std::span<const std::byte> wire) {'),
    ('core/src/op_resolve_walk.hpp:871',
    ...
    ('core/include/libtracer/fwd_frame_view.hpp:880', 'inline constexpr std::size_t kFwdMaxIov = 10;'),
    ...
    ('core/src/fwd_router.cpp:871', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:825', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:918', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:919', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:2131', 's.target_key.reset();'),
    ...
    ('core/tests/CMakeLists.txt:1319', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1332', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1333', '    LIBTRACER_NO_ATOMIC'),
