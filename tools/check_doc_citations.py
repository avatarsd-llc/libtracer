...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3393",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3433", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4167", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3267", "Everything else under `settings`"),
    ("core/src/graph.cpp:3465", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/include/libtracer/fwd_router.hpp:1303", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1309", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1362", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:202", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:380", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:558", "using reply_fn_t"),
    ('core/include/libtracer/fwd_router.hpp:204', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:205', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ...
    ("core/src/graph.cpp:3046", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3166", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3206", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3100", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3642", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3694", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3706", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3852", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4104", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4101", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4247", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1592', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1657', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2178', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2182', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2192', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2195', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2199', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2210', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2303', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2309', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2315', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2316', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2340', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2344', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2570', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2580', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2693', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2708', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1628',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2046', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2052', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1704", "has_registered_child()"),
    ("core/src/graph.cpp:1868", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2047", "graph_t::write_impl"),
    ("core/src/graph.cpp:2162", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2164", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2179", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2180", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2182", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2198", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2199", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2204", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2511", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3018", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3208", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:3047", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:3085", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:3086", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:3088", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2464", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2483", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2476", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1938", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2370", "decode_into(frame, rx_for(inbound_ctx))"),
...
('core/include/libtracer/graph.hpp:1449',
    ...
    ('core/include/libtracer/mem_source.hpp:171', '[[nodiscard]] block_source_t& heap_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:192', '[[nodiscard]] block_source_t& null_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:217', 'class bump_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:257', 'void reset() noexcept { used_ = 0; }'),
    ('core/include/libtracer/mem_source.hpp:349', 'class pool_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:203',
     '*       `monotonic_buffer_resource` also spills past its buffer, but it spills to a'),
    ('core/include/libtracer/mem_source.hpp:208',
     '* @warning SCOPE-LIFETIME USE ONLY. A bump block is never reclaimed, so a source that'),
    ('core/include/libtracer/mem_source.hpp:211',
     '*          between operations. It is NOT a long-lived seam: an 8 KiB bump source wired as'),
    ('core/include/libtracer/mem_source.hpp:212',
     "*          a router's `rx` decoded 6 frames and rejected the next 194 — measured. A"),
    ('core/include/libtracer/mem_source.hpp:214',
     '* @note Single-threaded by contract — a bump cursor is not synchronized. Its intended use'),
    ('core/include/libtracer/mem_source.hpp:225',
     '[[nodiscard]] void* try_alloc(std::size_t bytes, std::size_t align) noexcept override {',
     ': block_source_t("bump"), buf_(buffer), upstream_(&upstream) {}'),
    ('core/include/libtracer/mem_source.hpp:366',
     'pool_source_t(std::span<std::byte> slab, std::span<size_class_t> classes) noexcept'),
    ('core/include/libtracer/mem_source.hpp:417',
     '[[nodiscard]] std::size_t classes_used() const noexcept { return n_; }'),
    ('core/include/libtracer/mem_source.hpp:421',
    ...
    ('core/include/libtracer/subscriber.hpp:153', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/subscriber.hpp:660', 'class edge_snapshot_t {'),
    ('core/include/libtracer/subscriber.hpp:663', 'static constexpr std::size_t kCapacity = 8;'),
    ...
    ('core/include/libtracer/vertex.hpp:916', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1760', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2618', 'const bool use_heap ='),
    ...
    ('core/src/fwd_router.cpp:1905',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1912', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1969', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2492',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2443', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2457', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2509', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:3064',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3145',
     'constexpr std::array<std::byte, 5> op_tlv{std::byte{0x01}, std::byte{0x00}, std::byte{0x01},'),
    # core/src/graph.cpp
    ('core/src/graph.cpp:475', 'const view_t& frame_view, std::vector<std::byte> key,'),
    ('core/src/graph.cpp:1392', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1399', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1434',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1737', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1765', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1779', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1783', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1791', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1857',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1908',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1915', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1925',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1943',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1951',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2014', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2066', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2094', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2107',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2120', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2149',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2176',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2294', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2495',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3276', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4278', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1489', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:2976', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1672', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1687', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1688', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/subscriber.hpp:104', 'struct delivery_policy_t {'),
    ('core/include/libtracer/vertex.hpp:389', 'enum class delivery_mode_t : std::uint8_t {'),
    ('core/include/libtracer/subscriber.hpp:749', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/subscriber.hpp:852',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1814',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2536',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2713', 'const std::size_t doff = off;'),
    ...
    ('core/src/graph.cpp:2207', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2353', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2450',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2569', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3108', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3279',
