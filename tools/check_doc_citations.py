...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3012",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3052", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3574", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:1160", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1166", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1219", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:194", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:366", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:544", "using reply_fn_t"),
    ...
    ("core/include/libtracer/child_registry.hpp:268", "bool add(std::string name"),
    ("core/include/libtracer/child_registry.hpp:518",
     "[[nodiscard]] static transport_t* resolve_peer(const child_t& child, std::string_view peer) {"),
    ("core/include/libtracer/child_registry.hpp:533", "bool erase(std::string_view name) {"),
    ("core/include/libtracer/child_registry.hpp:566", "entry_by_name"),
    ("core/include/libtracer/child_registry.hpp:587",
    ...
    ("core/src/graph.cpp:2825", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2719", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3257", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3306", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3318", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3464", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3511", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3508", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3654", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:351", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:1007", 'field.steps[0].name != "subscribers"'),
    ...
    ('core/src/op_resolve_walk.hpp:1103', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ('core/include/libtracer/graph.hpp:1785', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1789', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1799', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1802', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1806', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1817', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1906', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1912', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1918', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1919', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1943', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1947', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2149', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2159', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2272', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1315',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1685', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1691', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ("core/include/libtracer/transport.hpp:34", "namespace tr::net {"),
    ...
    ("core/src/graph.cpp:1442", "has_registered_child()"),
    ("core/src/graph.cpp:1576", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1721", "graph_t::write_impl"),
    ("core/src/graph.cpp:1834", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1836", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1851", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1852", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1854", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1870", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1871", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1876", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2170", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2638", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2827", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:2925", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2963", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2964", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2966", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2349", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2368", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2361", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1823", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2255", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('bench/bench_libtracer.cpp:1245',
    ...
    ('core/include/libtracer/config.hpp:87', '* Override fragment: `static constexpr std::size_t kVertexLockStripes = 8;`; ESP-IDF:'),
    ('core/include/libtracer/config.hpp:97', 'static constexpr std::size_t kVertexLockStripes = 16;'),
    # core/include/libtracer/config.hpp
    ('core/include/libtracer/config.hpp:71', '* struct my_node_config_t : default_config_t {'),
    ('core/include/libtracer/config.hpp:83', 'struct default_config_t {'),
    ('core/include/libtracer/config.hpp:148', 'static constexpr std::size_t kHazardReaderSlots = 64;'),
    ('core/include/libtracer/config.hpp:197', 'static constexpr std::size_t kMaxVertexBytes64 = 96;'),
    ('core/include/libtracer/config.hpp:239', 'static constexpr std::uint32_t kPinPayloadRatio = 0;'),
    ('core/include/libtracer/config.hpp:248', 'using acl_policy_t = allow_only_policy_t;'),
    ('core/include/libtracer/config.hpp:215', 'static constexpr std::size_t kMaxVertexBytes32 = 72;'),
    ('core/include/libtracer/config.hpp:264', 'using lkv_slot_t = sp_atomic_slot_t;',
     'A many-core host is the case for rebinding this'),
    ('core/include/libtracer/config.hpp:393', 'using config_t = default_config_t;'),
    ('core/include/libtracer/config.hpp:97',
     'static constexpr std::size_t kVertexLockStripes = 16;'),
    ('core/include/libtracer/config.hpp:121',
     'static constexpr std::size_t kCacheLineBytes = 64;'),
    ('core/include/libtracer/config.hpp:261',
     '* fragment: `using lkv_slot_t = hazard_slot_t;`. The named type must satisfy the contract in'),
    ('core/include/libtracer/config.hpp:443',
     'inline constexpr bool kSpinWaitSafe = tr::graph::config_t::kSpinWaitSafe;'),
    ('core/include/libtracer/config.hpp:360', 'static constexpr bool kWeaklyOrdered = true;'),
    ...
    ('core/include/libtracer/config.hpp:402',
    ...
    ('core/include/libtracer/fwd_router.hpp:560',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1582',
...
('core/include/libtracer/graph.hpp:1190',
    ...
    ('core/include/libtracer/transport_vertex.hpp:151',
    ...
    ('core/include/libtracer/subscriber.hpp:105', 'using subscriber_fn_t = void (*)(void* ctx, const rope_t& value);'),
    ('core/include/libtracer/subscriber.hpp:376', 'class edge_snapshot_t {'),
    ('core/include/libtracer/subscriber.hpp:379', 'static constexpr std::size_t kCapacity = 8;'),
    ...
    ('core/include/libtracer/vertex.hpp:729', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1436', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2244', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2261', '++drops.out_of_memory;'),
    ...
    ('core/src/fwd_router.cpp:1790',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1797', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1854', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2377',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2328', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2342', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2394', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2942',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3023',
     'constexpr std::array<std::byte, 5> op_tlv{std::byte{0x01}, std::byte{0x00}, std::byte{0x01},'),
    # core/src/graph.cpp
    ('core/src/graph.cpp:396', 'const view_t& frame_view, std::vector<std::byte> key,'),
    ('core/src/graph.cpp:1168', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1175', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1210',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1475', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1503', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1510', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1514', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1522', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1565',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1616',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1623', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1633',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1651',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1659',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1688', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1740', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1768', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1781', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1792', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1821',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:1848',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1966', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2154',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2895', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3685', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    # core/src/op_resolve_walk.hpp
    ('core/src/fwd_reply.hpp:109', 'void tlv_sliced(std::span<const std::byte> wire) {'),
    ('core/src/op_resolve_walk.hpp:961',
    ...
    ('core/src/posix_endpoint.cpp:652', 'std::array<std::byte, 4096> chunk;',
    ...
    ('core/src/fwd_router.cpp:953', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:902', 'bus->set_peer_rope_receiver('),
    ('core/src/graph.cpp:1252', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1252',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2605', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1575', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1588', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1589', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('.github/workflows/core-ci.yml:279', 'matrix:', '  tsan:'),
    ...
    ('.github/workflows/core-ci.yml:288', '-DCMAKE_CXX_FLAGS="-fsanitize=thread -g -O1"'),
    ...
    ('core/include/libtracer/config.hpp:161',
    ...
    ('core/src/graph.cpp:1879', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2012', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2109',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2342', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2727', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2898',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:3084', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
    ...
    ('core/src/posix_endpoint.cpp:724',
     'return false;',
