...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3648",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3688", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4431", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3522", "Everything else under `settings`"),
    ("core/src/graph.cpp:3720", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3301", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3421", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3461", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3355", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3897", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3949", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3961", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4107", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4368", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4365", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4539", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1689', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1754', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2312', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2316', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2326', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2329', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2333', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2344', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2437', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2443', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2449', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2450', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2474', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2478', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2710', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2720', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2838', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2853', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1725',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2178', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2184', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1857", "has_registered_child()"),
    ("core/src/graph.cpp:2024", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2233", "graph_t::write_impl"),
    ("core/src/graph.cpp:2380", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2382", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2397", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2398", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2400", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2416", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2417", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2422", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2740", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3273", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3463", "acl_right_t::CREATE", 'step0.name == "children"'),
    ...
    ('core/include/libtracer/graph.hpp:1340',
...
('core/include/libtracer/graph.hpp:1546',
    ...
    ('core/include/libtracer/graph.hpp:2829',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2832',
    ...
    ('core/include/libtracer/vertex.hpp:961', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1805', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2663', 'const bool use_heap ='),
    ...
    ('core/src/graph.cpp:1525', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1532', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1567',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1890', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1918', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1932', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1936', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1944', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:2013',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2064',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2071', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2081',
    ...
    ('core/src/graph.cpp:2170', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2259',
    ...
    ('core/src/graph.cpp:2303',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2316', '// no notify reclone of the rope on the hot write path.'),
    ...
    ('core/src/graph.cpp:2512', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2724',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3531', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4570', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1622', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1622',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3231', 'delivery_link.assign(split.link);'),
    ...
    ('core/include/libtracer/vertex.hpp:431', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:1859',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2581',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2758', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2837',
    ...
    ('core/src/fwd_router.cpp:3279',
     'std::byte{0x00}};',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x00}, std::byte{0x00},'),
    ('core/src/graph.cpp:1027', 'return acl_allows(v.get(), caller, right);'),
    ('core/src/graph.cpp:1473',
    ...
    ('core/src/graph.cpp:2425', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2582', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2679',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2810', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3363', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3534',
