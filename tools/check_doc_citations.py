...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3571",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3611", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4345", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3445", "Everything else under `settings`"),
    ("core/src/graph.cpp:3643", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3224", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3344", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3384", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3278", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3820", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3872", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3884", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4030", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4282", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4279", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4443", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:1837", "has_registered_child()"),
    ("core/src/graph.cpp:2001", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2180", "graph_t::write_impl"),
    ("core/src/graph.cpp:2317", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2319", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2334", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2335", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2337", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2353", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2354", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2359", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2677", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3196", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3386", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1476',
    ...
    # core/src/graph.cpp
    ('core/src/graph.cpp:588', 'const view_t& frame_view, std::vector<std::byte> key,'),
    ('core/src/graph.cpp:1505', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1512', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1547',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1870', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1898', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1912', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1916', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1924', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1990',
    ...
    ('core/src/graph.cpp:2048', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2058',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2076',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:2084',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2147', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2199', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2227', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2240',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2253', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2304',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2331',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2449', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2661',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3454', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4474', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1602', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:3154', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:1830',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:2061', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2316',
    ...
    ('core/src/graph.cpp:2362', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2519', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2616',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2747', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3286', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3457',
