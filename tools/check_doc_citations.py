...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2626",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:2666", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3104", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2282", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2401", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2441", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2335", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2825", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2871", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2883", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3000", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3041", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3038", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3184", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1907', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1917', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2029', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ("core/src/graph.cpp:1185", "has_registered_child()"),
    ("core/src/graph.cpp:1311", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1443", "graph_t::write_impl"),
    ("core/src/graph.cpp:1556", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1558", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1573", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1574", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1576", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1592", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1593", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1595", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1882", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2254", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2443", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1058',
    ...
    ('core/include/libtracer/graph.hpp:2020',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2023',
    ...
    ('core/include/libtracer/vertex.hpp:889', 'class edge_snapshot_t {'),
    ('core/include/libtracer/vertex.hpp:892', 'static constexpr std::size_t kCapacity = 8;'),
    ('core/include/libtracer/vertex.hpp:1149', 'static_assert(alignof(vertex_stripe_t) == kStripeAlign,'),
    ('core/include/libtracer/vertex.hpp:1173', 'static std::array<vertex_stripe_t, kVertexLockStripes> stripes{};'),
    ('core/include/libtracer/vertex.hpp:1183', 'return (h >> 6) % kVertexLockStripes;'),
    ('core/include/libtracer/vertex.hpp:1187', 'inline vertex_stripe_t& vertex_stripe_of(const void* v) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1524', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:2216', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:3012', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:3029', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:1105',
    ...
    ('core/src/graph.cpp:909', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:916', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:951',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1210', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1238', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1245', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1249', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1257', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1300',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1344',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1351', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1361',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1379',
    ...
    ('core/src/graph.cpp:1410', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1462', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1490', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1503', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1514', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1543',
    ...
    ('core/src/graph.cpp:1682', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1866',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2511', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3215', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:993', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:993',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2236', 's.target_key.reset();'),
    ...
    ('core/include/libtracer/vertex.hpp:986', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/vertex.hpp:1084',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:2274',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2948',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:3112', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:3189',
    ...
    ('core/src/graph.cpp:1598', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:1724', 'if (!detail::try_assign(copy, k)) return false;'),
    ...
    ('core/src/graph.cpp:2343', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2514',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:2698', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
