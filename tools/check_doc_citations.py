...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3574",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3614", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4348", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3448", "Everything else under `settings`"),
    ("core/src/graph.cpp:3646", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3227", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3347", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3387", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3281", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3823", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3875", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3887", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4033", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4285", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4282", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4446", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:2207', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2211', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2221', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2224', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2228', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2239', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2332', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2338', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2344', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2345', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2369', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2373', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2605', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2615', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2728', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2743', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ...
    ("core/src/graph.cpp:2320", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2322", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2337", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2338", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2340", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2356", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2357", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2362", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2680", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3199", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3389", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1476',
    ...
    ('core/include/libtracer/graph.hpp:2719',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2722',
    ...
    ('core/src/graph.cpp:2243',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2256', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2307',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2334',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2452', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2664',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3457', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4477', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:3157', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:2365', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2522', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2619',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2750', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3289', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3460',
