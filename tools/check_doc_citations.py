...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3704",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3744", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4495", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3578", "Everything else under `settings`"),
    ("core/src/graph.cpp:3776", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3357", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3477", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3517", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3411", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3953", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4005", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4017", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4171", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4432", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4429", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4603", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ("core/src/graph.cpp:2436", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2438", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2453", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2454", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2456", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2472", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2473", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2478", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2796", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3329", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3519", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1563',
    ...
    ('core/include/libtracer/vertex.hpp:3063',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3068',
    ...
    ('core/src/graph.cpp:2359',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2372', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2423',
    ...
    ('core/src/graph.cpp:2568', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2780',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3587', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4634', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:3287', 'delivery_link.assign(split.link);'),
    ...
    ('core/src/graph.cpp:2481', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2638', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2735',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2866', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3419', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3590',
    ...
    ('core/src/graph.cpp:2315',
     '// A handler stores no LKV (the user handler consumes the value), so there is no'),
    ('core/src/graph.cpp:2704', 'if (!try_build_key(v, key)) {'),
    ('core/src/graph.cpp:2371',
