...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2035", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2075", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:1969", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2401", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2447", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2459", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2576", "folded_point_header(hdr_backend, n.body_len)"),
    ...
    ("core/src/graph.cpp:1282", "value.materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1283", "head.empty() && value.total_length()", "graph_t::write_branch"),
    ("core/src/graph.cpp:1295", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1296", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1298", "decode_into(head.bytes(), src)"),
    ("core/src/graph.cpp:1314", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1315", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1317", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1552", "value.materialize(*value_backend_)", "field_write read it back"),
    ("core/src/graph.cpp:1890", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2077", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:893',
    ...
    ('core/include/libtracer/vertex.hpp:2824', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2830',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2836',
     'if (src[i].active.load(std::memory_order_acquire)) ++drops.truncated;'),
    ('core/include/libtracer/vertex.hpp:2841', '++drops.out_of_memory;'),
    ('core/include/libtracer/vertex.hpp:3151',
    ...
    ('core/src/graph.cpp:1292',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:1382', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1539',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2145', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2791', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1876', 's.target_key.reset();'),
