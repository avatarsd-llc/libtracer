...
ANCHORS = [
    ...
    ("core/src/graph.cpp:1856", "graph_t::set_identity"),
    ("core/src/graph.cpp:1882", "graph_t::read_identity"),
    ("core/src/graph.cpp:2215", 'field.steps[0].name == "identity"'),
    ...
    ("core/include/libtracer/fwd_router.hpp:702", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:708", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:720", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:567", "Slot addresses are NOT stable"),
    ("core/include/libtracer/fwd_router.hpp:173", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:235", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:285", "subscribe_toward"),
    ("core/include/libtracer/fwd_router.hpp:382", "using reply_fn_t"),
    ("core/include/libtracer/fwd_router.hpp:393", "using stale_label_fn_t"),
    ...
    ("core/src/graph.cpp:1597", "field.steps.size() != 1", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:1664", "field.steps.size() != 1 || !plain_step(step0)"),
    ("core/src/graph.cpp:1701", "field.steps.size() != 1", 'step0.name == "children"'),
    ("core/src/graph.cpp:1622", "step0.wildcard"),
    ("core/src/graph.cpp:2181", '"children" && !field.steps[0].wildcard'),
    ("core/src/graph.cpp:2281", "!field.steps[0].wildcard", 'field.steps[0].name == "subscribers"'),
    ("core/src/op_resolve_walk.hpp:333", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:994", 'field.steps[0].name != "subscribers"'),
    ...
    ("core/src/op_resolve_walk.hpp:580", "view::segment_alloc(egress, head_len)"),
    ("core/src/op_resolve_walk.hpp:683", "rope_t or_backpressure"),
    ...
    ('core/include/libtracer/fwd_router.hpp:174',
     'std::pmr::memory_resource* mr = std::pmr::get_default_resource(),'),
    ('core/include/libtracer/fwd_router.hpp:175', 'mem::block_source_t* rx = &mem::heap_source(),'),
    ('core/include/libtracer/fwd_router.hpp:176', 'mem::mem_backend_t* flat = &mem::heap_backend(),'),
    ('core/include/libtracer/fwd_router.hpp:398',
    ...
    ('core/include/libtracer/graph.hpp:552',
     '[[nodiscard]] result_t<value_ref_t> read(vertex_handle_t v, std::string_view caller = {}) const;'),
    ('core/include/libtracer/graph.hpp:606',
     '* makes host-side after registration — and it has **no wire surface at all**: no peer'),
    ('core/include/libtracer/graph.hpp:639',
     '[[nodiscard]] result_t<value_ref_t> await(vertex_handle_t v, std::chrono::nanoseconds timeout,'),
    ('core/include/libtracer/graph.hpp:726', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:744',
     '*               into exactly that child here so the two doors stay byte-identical.'),
    ('core/include/libtracer/graph.hpp:782', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:896', 'std::function<void(const remote_delivery_t&, const rope_t&)> sink);'),
    ('core/include/libtracer/graph.hpp:920',
     '* Called by the FWD resolver on an inbound `:subscribers[]` WRITE (#59/#136); it'),
    ('core/include/libtracer/graph.hpp:1017', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1019', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1021', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1023', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1034', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1066', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1072', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1078', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1079', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1082', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1086', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1242', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1251', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1302',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1305',
     '*         LAST on purpose: no hot path reads it, so declaring it here keeps'),
    ('core/include/libtracer/graph.hpp:1311', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/src/graph.cpp:1498', 'field_path_t field;'),
    ('core/src/graph.cpp:1758', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2312', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/op_resolve_walk.hpp:589', 'out.tlv_sliced(reply_dst_wire);'),
    ('core/src/op_resolve_walk.hpp:972',
    ...
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:66',
     '* reply, and (the deep path) the whole /unit batch-apply transaction. The device'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:73', 'constexpr std::size_t kHttpdTaskStack = 12288;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:192', 'if (chunk.empty()) return true;'),
    ('integrations/esp-idf/libtracer/httpd_ws_link.cpp:198',
