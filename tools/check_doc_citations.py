...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3458",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3498", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4232", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3332", "Everything else under `settings`"),
    ("core/src/graph.cpp:3530", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3111", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3231", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3271", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3165", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3707", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3759", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3771", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3917", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4169", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4166", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4312", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1619', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1684', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2205', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2209', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2219', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2222', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2226', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2237', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2330', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2336', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2342', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2343', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2367', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2371', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2603', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2613', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2726', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2741', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1655',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2073', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2079', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1724", "has_registered_child()"),
    ("core/src/graph.cpp:1888", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2067", "graph_t::write_impl"),
    ("core/src/graph.cpp:2204", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2206", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2221", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2222", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2224", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2240", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2241", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2246", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2564", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3083", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3273", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1476',
    ...
    ('core/src/graph.cpp:1757', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1785', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1799', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1803', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1811', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1877',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1928',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1935', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1945',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1963',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1971',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2034', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2086', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:2114', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
    ...
    ('core/src/graph.cpp:2127',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2140', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2191',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2218',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2336', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2548',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3341', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4343', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:3041', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1680', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1695', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1696', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/src/graph.cpp:1717',
     'if (!acl_allows(v, caller, acl_right_t::READ))',
     'result_t<value_ref_t> graph_t::read(vertex_handle_t vh, std::string_view caller) const {'),
    ('core/src/graph.cpp:1948', '// nested publish must not be able to swallow this tally).'),
    ('core/src/graph.cpp:2203',
    ...
    ('core/src/graph.cpp:2249', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2406', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2503',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2634', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3173', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3344',
