...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3089",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3129", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3651", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:2743", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2862", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2902", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2796", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3334", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3383", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3395", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3541", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3588", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3585", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3731", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1310', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1375', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:1849', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1853', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1863', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:1866', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:1870', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:1881', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:1970', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1976', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1982', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1983', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2007', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2011', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2213', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2223', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2336', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1346',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:1749', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:1755', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:2715", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2904", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1221',
    ...
    ('core/include/libtracer/graph.hpp:2327',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2330',
    ...
    ('core/src/graph.cpp:2972', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3762', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:2682', 'delivery_link.assign(split.link);'),
    ...
    ('.github/workflows/core-ci.yml:317', 'name: tsan (slot=${{ matrix.lkv_slot }})'),
    ...
    ('.github/workflows/core-ci.yml:324', '-DCMAKE_CXX_FLAGS="-fsanitize=thread -g -O1"'),
    ...
    ('core/src/graph.cpp:2804', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:2975',
     '// (NAME key, NAME/SETTINGS value), read through the ONE pair-consuming walk,'),
    ('core/src/graph.cpp:3161', 'result_t<view_t> graph_t::read_settings(vertex_t* v) const {'),
