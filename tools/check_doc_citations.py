...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3910",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3950", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4706", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3784", "Everything else under `settings`"),
    ("core/src/graph.cpp:3983", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:3540", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3660", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3700", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:3594", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:4160", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4212", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4224", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4382", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4643", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4640", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4814", "field_selector(field) == field_sel_t::SLOT"),
    ("core/src/op_resolve_walk.hpp:366", "enum class index_mode_t"),
    ("core/src/op_resolve_walk.hpp:1083", 'field.steps[0].name != "subscribers"'),
    ...
    ('core/src/op_resolve_walk.hpp:1179', 'if (!req.dst.spans_intact()) return reply_error(status_t::BACKPRESSURE);'),
    ...
    ("core/src/graph.cpp:1990", "has_registered_child()"),
    ("core/src/graph.cpp:2167", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2393", "graph_t::write_impl"),
    ("core/src/graph.cpp:2559", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2561", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2576", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2577", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2579", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2595", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2596", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2601", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2978", "value.try_materialize(*value_backend_)",
...
('core/include/libtracer/graph.hpp:1634',
    ...
    ('core/include/libtracer/vertex.hpp:1081', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1925', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2806', 'const bool use_heap ='),
    ...
    ('core/src/graph.cpp:1658', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1665', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1700',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:2023', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:2051', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:2065', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:2069', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:2077', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:2156',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2207',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2214', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2224',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2242',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:2250',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2347', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ...
    ('core/src/graph.cpp:2493', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2546',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2573',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2723', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2962',
    ...
    ('core/src/graph.cpp:1755',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3470', 'delivery_link.assign(split.link);'),
    ...
    ('core/include/libtracer/vertex.hpp:500', 'enum class delivery_mode_t : std::uint8_t {'),
    ...
    ('core/include/libtracer/vertex.hpp:2724',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2901', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2978',
    ...
    ('core/src/graph.cpp:2793', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2890',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:3048', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3602', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3796',
    ...
    ('core/src/graph.cpp:2938',
     ' * DOCTRINE — write-vs-retire (#1477, the ruling). `write(vertex_handle_t)` takes NO map lock,'),
    ('core/include/libtracer/vertex.hpp:3047', '    std::atomic<role_t> role_;'),
    ('core/src/graph.cpp:2726', 'void graph_t::deliver_current(vertex_t* v) {'),
    ('core/src/graph.cpp:2743',
     "// The sweep root's OWN delivery is unconditional (below), and it reads the LKV"),
    ('core/src/graph.cpp:3009',
     'if (!sp) return std::unexpected(status_t::NOT_FOUND);  // never assigned'),
    ('core/src/graph.cpp:4312',
