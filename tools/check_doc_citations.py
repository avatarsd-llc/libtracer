...
ANCHORS = [
    ...
    ("core/include/libtracer/fwd_router.hpp:1145", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1151", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1204", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:179", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:351", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:529", "using reply_fn_t"),
    ...
    ("core/src/fwd_router.cpp:2822", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:2860", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:2861", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:2863", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2302", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2321", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2314", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ("core/src/fwd_router.cpp:1776", "frame.subrope(0, frame.total_length()).try_materialize",
     "if (hit.rejected) {"),
    ...
    ("core/src/fwd_router.cpp:2208", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:545',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1567',
...
('core/include/libtracer/graph.hpp:1190',
    ...
    ('core/src/fwd_router.cpp:1743',
     'void fwd_router_t::on_frame_rope_impl(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:1750', 'if (frame.link_count() == 1) {'),
    ('core/src/fwd_router.cpp:1807', '// A REPLY that reaches its originator here is handed to the sink'),
    ('core/src/fwd_router.cpp:2330',
     'void fwd_router_t::on_control_rope(std::string_view inbound_name, view::rope_t frame,'),
    ('core/src/fwd_router.cpp:2281', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2295', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2347', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:2839',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:2920',
    ...
    ('core/src/fwd_router.cpp:906', 'link.set_rope_receiver('),
    ('core/src/fwd_router.cpp:855', 'bus->set_peer_rope_receiver('),
    ...
    ('core/include/libtracer/fwd_router.hpp:408',
     '[[nodiscard]] graph::result_t<void> subscribe_toward(const graph::path_t& producer,'),
    ('core/include/libtracer/fwd_router.hpp:1118',
