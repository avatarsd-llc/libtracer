...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3741",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3781", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:4532", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3615", "Everything else under `settings`"),
    ("core/src/graph.cpp:3813", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ("core/src/transport_vertex.cpp:55", 'cfg.name("kind")'),
    ...
    ("core/include/libtracer/fwd_router.hpp:1521", "Terminus over a MULTI-LINK rope"),
    ("core/include/libtracer/fwd_router.hpp:1527", "64 KB / 2 links"),
    ("core/include/libtracer/fwd_router.hpp:1580", "The forward hop, read entirely by OFFSET"),
    ("core/include/libtracer/fwd_router.hpp:249", "explicit fwd_router_t"),
    ("core/include/libtracer/fwd_router.hpp:479", "bool add_child"),
    ("core/include/libtracer/fwd_router.hpp:744", "using reply_fn_t"),
    ...
    ("core/src/graph.cpp:3448", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3990", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:4042", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:4054", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:4208", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:4469", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:4466", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:4640", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1706', '[[nodiscard]] result_t<rope_t> read_subtree_folded(vertex_handle_t v,'),
    ('core/include/libtracer/graph.hpp:1771', 'template <typename F>'),
    ('core/include/libtracer/graph.hpp:2329', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:2333', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:2343', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2346', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2350', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2361', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2454', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2460', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2466', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2467', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2491', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2495', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2746', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2756', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2874', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ('core/include/libtracer/graph.hpp:2889', 'mem::block_source_t* ring_ = &mem::heap_source();'),
    ('core/include/libtracer/graph.hpp:1742',
     '* @param ctx Caller-owned context, passed back to @p fn on every delivery. Its lifetime'),
    ('core/include/libtracer/graph.hpp:2195', '[[nodiscard]] result_t<value_ref_t> read(const path_t& path) const;'),
    ('core/include/libtracer/graph.hpp:2201', '[[nodiscard]] result_t<value_ref_t> await(const path_t& path, std::chrono::nanoseconds timeout);'),
    ...
    ("core/src/graph.cpp:1921", "has_registered_child()"),
    ("core/src/graph.cpp:2088", "void graph_t::fan_out"),
    ("core/src/graph.cpp:2280", "graph_t::write_impl"),
    ("core/src/graph.cpp:2446", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2448", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2463", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2464", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2466", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2482", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2483", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2488", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2833", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:3366", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3556", "acl_right_t::CREATE", 'step0.name == "children"'),
    ("core/src/fwd_router.cpp:3316", "fwd_router_t::deliver_remote"),
    ("core/src/fwd_router.cpp:3354", "value.try_materialize(*flat_)"),
    ("core/src/fwd_router.cpp:3355", "if (!flat) return;", "A REFUSED materialize drops the delivery"),
    ("core/src/fwd_router.cpp:3357", "emit_compact", "fwd_router_t::deliver_remote"),
    ...
    ("core/src/fwd_router.cpp:2733", "if (route.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2752", "if (payload.empty() && head->child1_total != 0) return;"),
    ("core/src/fwd_router.cpp:2745", "const std::span<const std::byte> payload = contig(head->child1_off, head->child1_total);"),
    ...
    ("core/src/fwd_router.cpp:2607", "decode_into(frame, rx_for(inbound_ctx))"),
    ...
    ('core/include/libtracer/fwd_router.hpp:760',
     "* Invoked (with the `FWD{REPLY}` frame as a @ref view::rope_t) when a REPLY's first"),
    ('core/include/libtracer/fwd_router.hpp:1968',
...
('core/include/libtracer/graph.hpp:1563',
    ...
    ('core/include/libtracer/mem_source.hpp:232', '[[nodiscard]] block_source_t& heap_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:253', '[[nodiscard]] block_source_t& null_source() noexcept;'),
    ('core/include/libtracer/mem_source.hpp:278', 'class bump_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:320', 'void reset() noexcept {'),
    ('core/include/libtracer/mem_source.hpp:460', 'class pool_source_t final : public block_source_t {'),
    ('core/include/libtracer/mem_source.hpp:264',
     '*       `monotonic_buffer_resource` also spills past its buffer, but it spills to a'),
    ('core/include/libtracer/mem_source.hpp:269',
    ...
    ('core/include/libtracer/vertex.hpp:982', '[[nodiscard]] bool has_registered_child() const noexcept {'),
    ('core/include/libtracer/vertex.hpp:1826', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2698', 'const bool use_heap ='),
    ...
    ('core/src/fwd_router.cpp:2712', 'const auto head = peek_control(cur, wire::grammar::crc_check_t::VERIFY);'),
    ('core/src/fwd_router.cpp:2726', 'const std::span<const std::byte> route = contig(head->child1_off, head->child1_total);'),
    ('core/src/fwd_router.cpp:2778', 'frame.subrope(off, total).try_materialize(*flat_);'),
    ('core/src/fwd_router.cpp:3333',
     "// else. A dropped fresh ADVERTISE self-heals via the peer's HANDLE_NACK (§E.1). NOT yet"),
    ('core/src/fwd_router.cpp:3423',
    ...
    ('core/src/graph.cpp:1589', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1596', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1631',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1954', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1982', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1996', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:2000', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:2008', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:2077',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:2128',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:2135', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:2145',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:2163',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:2171',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:2234', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:2323',
    ...
    ('core/src/graph.cpp:2367',
     "// Drain this RECEIVER's ring and advance its cursor, so a later propagate over the"),
    ('core/src/graph.cpp:2380', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2433',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2460',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2578', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2817',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3624', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:4671', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1686', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1686',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:3324', 'delivery_link.assign(split.link);'),
    ...
    ('core/tests/CMakeLists.txt:1773', 'add_executable(substrate_test_no_atomic'),
    ('core/tests/CMakeLists.txt:1788', 'target_compile_definitions(substrate_test_no_atomic PRIVATE'),
    ('core/tests/CMakeLists.txt:1789', '    LIBTRACER_NO_ATOMIC'),
    ...
    ('core/include/libtracer/fwd_router.hpp:1494',
    ...
    ('core/include/libtracer/vertex.hpp:1880',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2616',
     '*        displaced one. Call with the stripe lock held; the caller runs'),
    ('core/include/libtracer/vertex.hpp:2793', 'const std::size_t doff = off;'),
    ('core/include/libtracer/vertex.hpp:2870',
    ...
    ('core/src/fwd_router.cpp:2767',
     'const wire::grammar::rope_cursor cur{frame};',
     'if (!frame.all_host()) return;'),
    ('core/src/fwd_router.cpp:3062', 'const auto payload_view = view::over_bytes(payload_bytes);'),
    ...
    ('core/src/fwd_router.cpp:3356', 'if (fresh) emit_advertise(*link, label, route);'),
    ('core/src/fwd_router.cpp:3426',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x00}, std::byte{0x00},'),
    ('core/src/fwd_router.cpp:3427',
     'std::byte{0x00}};',
     'constexpr std::array<std::byte, 4> empty_src{std::byte{0x06}, std::byte{0x00}, std::byte{0x00},'),
    ('core/src/graph.cpp:1091', 'return acl_allows(v.get(), caller, right);'),
    ...
    ('core/src/graph.cpp:2491', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2648', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2745',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2903', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:3456', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
    ('core/src/graph.cpp:3627',
