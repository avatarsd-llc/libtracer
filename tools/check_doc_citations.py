...
ANCHORS = [
    ...
    ("core/src/graph.cpp:3254",
     "result_t<void> graph_t::set_identity(std::uint8_t kind, std::span<const std::byte> key) {"),
    ("core/src/graph.cpp:3294", "result_t<view_t> graph_t::read_identity() const {"),
    ("core/src/graph.cpp:3816", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:3128", "Everything else under `settings`"),
    ("core/src/graph.cpp:3326", "result_t<view_t> graph_t::read_settings(vertex_t* v) const {"),
    ...
    ("core/src/graph.cpp:2907", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:3027", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:3067", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2961", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:3499", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:3548", "folded_member_header(hdr_backend, body, seg.size())"),
    ("core/src/graph.cpp:3560", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:3706", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:3753", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:3750", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:3896", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1985', 'struct delivery_drops_t {'),
    ('core/include/libtracer/graph.hpp:1989', 'std::uint64_t no_target = 0;'),
    ('core/include/libtracer/graph.hpp:1999', 'std::uint64_t denied = 0;'),
    ('core/include/libtracer/graph.hpp:2002', 'std::uint64_t out_of_memory = 0;'),
    ('core/include/libtracer/graph.hpp:2006', 'std::uint64_t fan_out_truncated = 0;'),
    ('core/include/libtracer/graph.hpp:2017', '[[nodiscard]] delivery_drops_t delivery_drops() const noexcept;'),
    ('core/include/libtracer/graph.hpp:2106', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2112', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2118', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2119', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2143', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2147', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:2359', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:2369', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:2482', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ("core/src/graph.cpp:1638", "has_registered_child()"),
    ("core/src/graph.cpp:1788", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1933", "graph_t::write_impl"),
    ("core/src/graph.cpp:2046", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:2048", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:2063", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:2064", "bump_source_t src(stack"),
    ("core/src/graph.cpp:2066", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:2082", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:2083", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:2088", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:2382", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:2879", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:3069", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:1295',
    ...
    ('core/include/libtracer/graph.hpp:2473',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:2476',
    ...
    ('core/include/libtracer/subscriber.hpp:613', 'class edge_snapshot_t {'),
    ('core/include/libtracer/subscriber.hpp:616', 'static constexpr std::size_t kCapacity = 8;'),
    ...
    ('core/include/libtracer/vertex.hpp:1458', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2263', 'const bool use_heap ='),
    ...
    ('core/include/libtracer/vertex.hpp:2269',
     '// OOM fallback (reserve failed on a wide list): the inline prefix delivers,'),
    ('core/include/libtracer/vertex.hpp:2275',
    ...
    ('core/src/graph.cpp:1361', 'graph_t::delivery_drops_t graph_t::delivery_drops() const noexcept {'),
    ('core/src/graph.cpp:1368', 'void graph_t::count_drop(drop_reason_t why, std::uint64_t n) noexcept {'),
    ('core/src/graph.cpp:1403',
     'void graph_t::count_snapshot_drops(const vertex_t::snapshot_drops_t& drops) noexcept {'),
    ('core/src/graph.cpp:1671', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1699', 'if (target == nullptr) {',
     'target = find_ptr(*e.target_key);'),
    ('core/src/graph.cpp:1713', 'if (!acl_allows(target, e.caller(), acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1717', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1725', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1777',
     '[[gnu::always_inline]] inline void graph_t::dispatch_edge(const edge_view_t& e,'),
    ('core/src/graph.cpp:1828',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1835', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1845',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1863',
     'const std::size_t n = v->snapshot_edges(inline_buf, heap_buf, drops);'),
    ('core/src/graph.cpp:1871',
     'result_t<std::shared_ptr<const rope_t>> graph_t::store_value(vertex_t* v, rope_t&& value,'),
    ('core/src/graph.cpp:1900', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1952', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1980', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());',
     'frame that abandons the delivery'),
    ('core/src/graph.cpp:1993', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:2004', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:2033',
     'result_t<void> graph_t::write_branch(vertex_t* v, const rope_t& value, std::string_view caller,'),
    ('core/src/graph.cpp:2060',
     "// The overflow leg draws from the graph's injected control seam, not the global heap:"),
    ('core/src/graph.cpp:2178', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:2366',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:3137', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:3927', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:1448', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ...
    ('core/src/graph.cpp:1448',
     'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:2846', 'delivery_link.assign(split.link);'),
    ...
    ('core/include/libtracer/graph.hpp:2044',
    ...
    ('core/include/libtracer/subscriber.hpp:702', 'struct alignas(pub_edge_t) edge_pub_t {'),
    ('core/include/libtracer/subscriber.hpp:805',
     'inline void scan_retired_edges(edge_block_t& b) noexcept {'),
    ('core/include/libtracer/vertex.hpp:1512',
     'if (!pin.valid()) {  // domain exhausted: the pre-#635 path, for these threads only'),
    ('core/include/libtracer/vertex.hpp:2181',
    ...
    ('core/src/graph.cpp:1848', '// nested publish must not be able to swallow this tally).'),
    ...
    ('core/src/graph.cpp:2091', 'if (!parsed) return std::unexpected(parsed.error());'),
    ('core/src/graph.cpp:2224', 'if (!detail::try_assign(copy, k)) return false;'),
    ('core/src/graph.cpp:2321',
     '// Empty-set fast path (the per-eager-write case when nobody uses assign+propagate):'),
    # ('core/src/graph.cpp:2439', '// status (ADR-0060 §3), …') — anchor DROPPED (#1271). It
    ...
    ('core/src/graph.cpp:2969', 'if (!tlv) return std::unexpected(status_t::TYPE_MISMATCH);'),
