...
ANCHORS = [
    ...
    ("core/src/graph.cpp:2285", "graph_t::set_identity"),
    ("core/src/graph.cpp:2311", "graph_t::read_identity"),
    ("core/src/graph.cpp:2730", 'field.steps[0].name == "identity"'),
    ...
    ("core/src/graph.cpp:1966", "sel == field_sel_t::TAIL", 'step0.name == "subscribers"'),
    ("core/src/graph.cpp:2085", "!whole_field(field)", 'step0.name == "acl"'),
    ("core/src/graph.cpp:2125", "field_selector(field) != field_sel_t::APPEND"),
    ("core/src/graph.cpp:2019", "sel == field_sel_t::WILDCARD"),
    ("core/src/graph.cpp:2451", "view::segment_alloc(backend, folded_hdr_len(body_len))"),
    ("core/src/graph.cpp:2497", "folded_point_header(hdr_backend, name.size())"),
    ("core/src/graph.cpp:2509", "folded_point_header(hdr_backend, members_len)"),
    ("core/src/graph.cpp:2626", "folded_point_header(hdr_backend, n.body_len)"),
    ("core/src/graph.cpp:2667", "return read_children_folded(vh);"),
    ("core/src/graph.cpp:2664", "sel == field_sel_t::WHOLE || sel == field_sel_t::APPEND"),
    ("core/src/graph.cpp:2810", "field_selector(field) == field_sel_t::SLOT"),
    ...
    ('core/include/libtracer/graph.hpp:1451', 'void fan_out(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1457', 'void dispatch_edge(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1463', 'void dispatch_edge_target(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1464', 'void dispatch_edge_remote(const edge_view_t& e, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1481', 'void bubble_up(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1485', 'void deliver_vertex(vertex_t* v, const rope_t& value);'),
    ('core/include/libtracer/graph.hpp:1663', 'std::pmr::memory_resource* mr_ = std::pmr::get_default_resource();'),
    ('core/include/libtracer/graph.hpp:1672', 'mem::mem_backend_t* value_backend_ = &mem::heap_backend();'),
    ('core/include/libtracer/graph.hpp:1740', 'mem::block_source_t* ctl_ = &mem::heap_source();'),
    ...
    ("core/src/graph.cpp:979", "has_registered_child()"),
    ("core/src/graph.cpp:1084", "void graph_t::fan_out"),
    ("core/src/graph.cpp:1214", "graph_t::write_impl"),
    ("core/src/graph.cpp:1309", "value.try_materialize(*value_backend_)", "graph_t::write_branch"),
    ("core/src/graph.cpp:1311", "flatten_err_t::NO_MEMORY", "graph_t::write_branch"),
    ("core/src/graph.cpp:1326", "std::array<std::byte, 4096> stack;"),
    ("core/src/graph.cpp:1327", "bump_source_t src(stack"),
    ("core/src/graph.cpp:1329", "decode_into(head->bytes(), src)"),
    ("core/src/graph.cpp:1345", "std::vector<std::byte> root_key;"),
    ("core/src/graph.cpp:1346", "try_build_key(v, root_key)"),
    ("core/src/graph.cpp:1348", "try_assign(parse_key, root_key)"),
    ("core/src/graph.cpp:1586", "value.try_materialize(*value_backend_)",
     "field_write read an empty head"),
    ("core/src/graph.cpp:1938", "result_t<void> graph_t::field_write"),
    ("core/src/graph.cpp:2127", "acl_right_t::CREATE", 'step0.name == "children"'),
...
('core/include/libtracer/graph.hpp:893',
    ...
    ('core/include/libtracer/graph.hpp:1731',
     '*         to migrate. Kept a DIFFERENT type from `mr_` on purpose (see'),
    ('core/include/libtracer/graph.hpp:1734',
    ...
    ('core/include/libtracer/vertex.hpp:2028', 'struct snapshot_drops_t {'),
    ('core/include/libtracer/vertex.hpp:2837', 'const bool use_heap ='),
    ('core/include/libtracer/vertex.hpp:2854', '++drops.out_of_memory;'),
    ...
    ('core/include/libtracer/vertex.hpp:3164',
     'static_assert(sizeof(void*) != 8 || sizeof(vertex_t) <= config_t::kMaxVertexBytes64,'),
    ('core/include/libtracer/vertex.hpp:3169',
    ...
    ('core/src/graph.cpp:1004', '[[nodiscard]] bool try_clone_rope(rope_t& dst, const rope_t& src) noexcept {'),
    ('core/src/graph.cpp:1032', 'if (target == nullptr) {'),
    ('core/src/graph.cpp:1039', 'if (!acl_allows(target, e.caller, acl_right_t::WRITE)) {'),
    ('core/src/graph.cpp:1043', '// Delivery TERMINATES at the target (ADR-0051 / RFC-0007): apply exactly the'),
    ('core/src/graph.cpp:1051', 'if (!try_clone_rope(clone, value)) {'),
    ('core/src/graph.cpp:1074', 'inline void graph_t::dispatch_edge(const edge_view_t& e, const rope_t& value) {'),
    ('core/src/graph.cpp:1117',
     '// snapshot_edges re-checks the width under the lock, so a race on the count only costs a'),
    ('core/src/graph.cpp:1124', 'static thread_local std::vector<edge_view_t> tls_buf;'),
    ('core/src/graph.cpp:1134',
     'const std::size_t n = v->snapshot_edges(inline_buf, tls_buf, drops);'),
    ('core/src/graph.cpp:1152',
    ...
    ('core/src/graph.cpp:1181', 'void graph_t::bubble_up(vertex_t* v, const rope_t& value) {'),
    ('core/src/graph.cpp:1233', '// A handler stores no LKV (the user handler consumes the value), so the'),
    ('core/src/graph.cpp:1257', 'count_drop(drop_reason_t::OUT_OF_MEMORY, v->own_subs());'),
    ('core/src/graph.cpp:1268', '// Deliver the just-appended ring entry and advance the drain cursor, so a later'),
    ('core/src/graph.cpp:1273', '// no notify reclone of the rope on the hot write path.'),
    ('core/src/graph.cpp:1296',
    ...
    ('core/src/graph.cpp:1428', 'if (v->listeners_above() > 0) bubble_up(v, value);', 'fan_out(v, value);'),
    ('core/src/graph.cpp:1570',
     'result_t<void> graph_t::write(vertex_handle_t v, rope_t value, std::string_view caller) {'),
    ('core/src/graph.cpp:2195', 'result_t<void> graph_t::create_child(vertex_t* parent, const view_t& spec_value) {'),
    ('core/src/graph.cpp:2841', 'result_t<void> graph_t::write(const path_t& path, rope_t value) {'),
    ...
    ('core/src/graph.cpp:792', 'vertex_t* graph_t::find_ptr(std::span<const std::byte> key) const {'),
    ('core/src/graph.cpp:793', 'const std::shared_lock lock(map_mutex_);'),
    ('core/src/graph.cpp:1924', 's.target_key.reset();'),
