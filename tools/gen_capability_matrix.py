...
def render():
    ...
    L.append("scanning the source tree, so a catalog entry with no source at all — neither a")
    L.append("`core/src` translation unit nor a `core/include` header — shows as *planned*, not")
    L.append("*done*. A header-only module counts as implemented, with an empty")
    L.append("translation-unit cell. A CI check fails if this page drifts from that evidence.")
    L.append("")
    ...
    L.append("  overclaim: a listed module shows as *planned* until its source lands. The")
    L.append("  future-tier `transport_i2c` / `transport_spi` carry no design commitment and are")
    L.append("  deliberately absent from this table; they are listed in the module catalog's")
    L.append("  `future` tier.")
